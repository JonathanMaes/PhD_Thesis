...
% Q: should I put all the information of this section earlier already, at points where these subjects are discussed? I could e.g. create a custom "API" environment to explain different parts of the package in to show code outside the main text.
...
\paragraph{GUI}
A graphical user interface (GUI) is available for \hotspice, which allows the user to directly interact with the ASI and observe changes in real-time. It can be run by calling \python{hotspice.gui.show(mm)}, with \python{mm} the ASI object, resulting in the window shown in~\cref{fig:2:GUI}.
...
\subsubsection{Grid} % TODO: write more nicely and add stuff
Throughout this section, it has become clear that the usage of a grid has far-reaching implications to the workings of \hotspice, in both positive and negative sense.
On the one hand, the grid allows efficient calculation of the magnetostatic interaction by using kernels, and can do so in a memory-efficient manner while accounting for all the interactions throughout the ASI.
The computational cost of taking all interactions into account is limited when using the GPU; for example, performance only starts to decline for systems larger than $800 \times 800$ on the setup used for this thesis.
On the other hand, the grid restricts the usage of \hotspice to ASI which can be put on a regular grid.
Irregular ASI, like the evolutionary ASI developed by \textit{Penty and Tufte}~\cite{ASI_Evolutionary_ALife}, can not be simulated with this software.
Furthermore, it is unclear whether the grid has a net positive or negative impact on the calculation efficiency of the magnetostatic interaction.
While the grid easily allows the magnetostatic interaction between all magnets to be taken into account, limiting the magnetostatic interaction to a maximum interaction distance may not noticeably affect ASI dynamics.
For example, the flatspin~\cite{flatspin} simulator does not employ a grid, but instead stores the interaction strength between a magnet and its neighbours that lay within a certain radius.
This approach seems to yield comparable performance.
On the other hand, selecting multiple sufficiently spaced magnets in an irregular ASI may be more cumbersome and require a virtual underlying grid anyway, while the grid in \hotspice readily supports multi-sampling with a minimal distance requirement.
\subsubsection{Input/output RC}
\hotspice contains a module \python{hotspice.io} which provides various basic input and readout methods.
These were mostly written before several aspects of \hotspice were finished, and are therefore limited in their scope.
During development of \hotspice, the PRCpy package~\cite{PRCpy} has become available, which can determine reservoir computing metrics.
This package appears preferable over the builtin \python{hotspice.experiments} module, as it is more broadly applicable.
\subsubsection{GPU/CPU}
In hindsight, it was not necessary to port \hotspice to the GPU for the purposes of this thesis.
In~\cref{ch:Applications}, large systems are rarely used, if ever.
Most often, small systems are simulated with the N\'eel algorithm, which does not benefit from the multi-switching implementation.
Therefore, the vast majority of calculations in that chapter will be performed on CPU.
Still, the efforts of implementing GPU operation and a multi-switching procedure were not wasted as they have expanded the envelope of situations where \hotspice can be used, particularly when using the Metropolis-Hastings algorithm. \par
One particular issue that remains, is that the current implementation requires making the choice to use GPU or CPU before the \python{import hotspice} statement.
While this presents no issues in the vast majority of cases, it is a limitation that is not strictly necessary.
