...
While it is possible to obtain an exact solution for the energy barrier in OOP magnets, the assumption of a sinusoidal energy landscape was found to be inaccurate, likely due to non-coherent reversal in real OOP nanomagnets. \par % TODO: we don't know that yet before Ch. 3
...
It was verified that this technique yields correct results for a set of test cases on various lattices, and the accuracy of the first-switch method was later assessed in pinwheel and kagome ASI through comparison with experimental results. \par
...
We have shown that different systems call for different model variants or update algorithms, of which we provided several notable examples: for instance, changing from a point dipole to a dumbbell model was found to have a significant impact on kagome ASI, whereas pinwheel ASI is barely affected by this. % not every model improvement is relevant to every lattice, nor does a single update scheme suit all simulation goals.
