...
Using such higher-level approximations enables the study of collective behaviour in much larger systems and over far longer timescales than is feasible with micromagnetic codes, though at the cost of no longer simulating the internal magnetisation structure of individual nanomagnets in detail.
...
The magnetostatic interaction energy between the pair of magnets, divided by $\mu^2$ to be independent of magnet volume, is shown as a function of their normalised centre-to-centre distance.
...
\xfig[1.0]{2_Hotspice/MS_distance.pdf}{
	Magnitude of the magnetostatic interaction between two magnets as a function of their normalised centre-to-centre distance, for the three \hotspice{} calculation methods (point dipole, second-order correction for dipoles, and dumbbell) compared to a micromagnetic \mumax{} calculation.
...
The two common choices for the acceptance probability, $P_\mathrm{MH}$ and $P_\mathrm{GD}$, are compared in~\cref{fig:2:RejectionKMC}.
\sidefig{2_Hotspice/RejectionKMC.pdf}{
	\label{fig:2:RejectionKMC}
	Acceptance probability used by Metropolis-Hastings and Glauber Dynamics, as a function of the switching energy $\Delta E$.
...
\begin{itemize}
	...
	Think for example of an exchange-coupled OOP square ASI in the uniform state: a regularly-spaced quarter of all magnets would switch simultaneously, without introducing additional spatial variation, which would be non-physical.
...
The three energy contributions in \eqref{eq:2:E} are provided in the \python{hotspice.energies} module and can be accessed from the main \python{hotspice} namespace.
Choosing between the dipole and monopole representations for the magnetostatic energy is currently done by using the \python{DipolarEnergy} and \python{DiMonopolarEnergy} classes, respectively.
...
The second mode displays individual magnets' magnetisation direction $s_i \vc{u}_i$ or the effective field $\vc{B}_{\mathrm{eff},i}$ they experience.
In the third mode, all energy contributions ($E_\mathrm{MS}$, $E_\mathrm{Z}$, $E_\mathrm{Exch}$, their sum $E_i$ and resulting $\EBeff$) are shown for each magnet, along both the easy and hard axis.
...
The simulations in that chapter most often use the N\'eel algorithm, which does not benefit from the multi-switching implementation either, so the majority of computations are performed on the CPU. \par
...
For these cluster algorithms, the dynamical critical exponent $z \approx 0$ in the 2D exchange-coupled Ising system~\cite{NumericalDynamicalNiedermayer}, as compared to $z\approx2.17$ with the Metropolis-Hastings algorithm~\cite{DynamicExponentMetropolis}.
...
Meanwhile, pinwheel ASI exhibits superferromagnetic order, where all magnets with similarly-oriented easy axes are magnetised in the same direction~\cite{ApparentFMpinwheel}.
