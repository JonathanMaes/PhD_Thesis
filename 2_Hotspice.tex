...
This approach has the advantage that PBC do not impact performance, as they are baked into the kernel, contrary to the alternative of using a \xlabel{circular convolution}.
...
this finally gives
\begin{equation}
	r_{12} \geq \sqrt[3]{\frac{2 \mu_0 M_{sat}^2 V^2}{\pi Q k_B T}} = \rmin \mathrm{.}
\end{equation}
...
\xfig[1.0]{2_Hotspice/MultiSwitch_select_Poisson.pdf}{
	...
	Characteristics of \link{Bridson's algorithm}{Bridson's} \xref{Poisson disc sampling} algorithm, based on $\approx \SI{e6}{}$ samples.
	...
	\textbf{(d)} \link{periodogram}{Periodogram}, i.e. the spatial Fourier transform of simultaneously sampled magnets, averaged over all iterations of the selection algorithm.
}
...
Finally, \crefSubFigRef{fig:2:MultiSwitch_select_Poisson}{d} shows the average spatial Fourier spectrum of a set of sampled points, called a \idx{periodogram}.\indexlabel{fast Fourier transform}
...
Normal SJG sampling divides the domain into strata --- typically squares --- and selects a random point in each \xlabel{stratum}.
...
\xfig[1.0]{2_Hotspice/MultiSwitch_select_Grid.pdf}{
	...
	Characteristics of the restricted \xref{stratified jittered grid} selection algorithm, based on $\approx \SI{e6}{}$ samples.
	\textbf{(a)} Probability density of \link{multi-switching}{simultaneously sampling} the magnet at the centre of the figure (white circle) and at any other point in the figure.
	The imposed minimal distance $\rmin$ is indicated by the dashed white circle.
	\textbf{(b)} Distribution of nearest-neighbour distances between sampled points, alongside the corresponding cumulative probability density.
	\textbf{(c)} Number of times each cell in the ASI was sampled, which must not display any obvious pattern.
...
\begin{itemize}
	...
	With PBC, this may result in \link{multi-sampling}{simultaneous sampling} of magnets which are physically closer than $\rmin$, but computationally separated by the edge.
...
\xfig[1.0]{2_Hotspice/MultiSwitch_select_Hybrid.pdf}{
	...
	Characteristics of the hybrid grid-Poisson selection algorithm, based on $\approx \SI{e6}{}$ samples.
	\textbf{(a)} Probability density of \link{multi-switching}{simultaneously sampling} the magnet at the centre of the figure (white circle) and at any other point in the figure.
	The imposed minimal distance $\rmin$ is indicated by the dashed white circle.
	\textbf{(b)} Distribution of nearest-neighbour distances between sampled points, alongside the corresponding cumulative probability density.
	\textbf{(c)} Number of times each cell in the ASI was sampled, which must not display any obvious pattern.
...
For instance, the \texttt{flatspin} ASI simulator~\cite{flatspin} has no underlying grid, so it only stores interaction strengths within a certain radius, which does not significantly affect dynamics.
...
Nonetheless, GPU support has broadened the range of systems that \hotspice can efficiently simulate, in particular for the \link{Metropolis-Hasting sampling}{Metropolis-Hastings algorithm}.
