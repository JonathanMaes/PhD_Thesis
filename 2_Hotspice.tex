...
For each magnet $i$, a kernel $\vc{K}_i(j)$ stores the strength of the magnetostatic interaction between itself and all other magnets $j$. This enables the quick calculation of their magnetostatic interaction energy as $E_{\mathrm{MS},i,j}= s_i s_j \vc{K}_i(j) \mu_i \mu_j$ by pre-calculating all $\vc{K}_i(j)$ values once when the ASI is created.
