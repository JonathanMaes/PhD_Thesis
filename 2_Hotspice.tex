...
The \xref{magnetic moment} and \link{shape anisotropy}{energy barrier} of each magnet can be derived from the geometry of the $470\times170\times\qty{10}{\nano\meter}$ stadium-shaped magnets and the permalloy (\ce{Ni_80 Fe_20}) \xref{saturation magnetisation} $M_\mathrm{sat}=\qty{800}{\kilo\ampere\per\meter}$.
...
In-plane kagome ASI (\crefSubFigRef{fig:2:ASIs}{g}) is a typical example of a \link{frustration}{frustrated} system: every vertex is formed by three magnets that must obey the two-in/one-out or two-out/one-in ice rule.
