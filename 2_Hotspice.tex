...
Note that all terms in \cref{eq:2:E} simply change sign when magnet $i$ switches ($\vc{\mu}_i \rightarrow -\vc{\mu}_i$).
The magnetostatic self-energy is not present in \cref{eq:2:E} because it only contributes a constant offset and does not change when the magnet switches.
...
Recall that this quantity is distinct from the shape anisotropy $E_\mathrm{B} = K_u V$: the effective energy barrier  $\EBtilde$ is a modification of $\EB$ caused by the interaction with other magnets.
...
All of them use the energy difference $\Delta E_i$ between the current and opposite state of the magnet, referred to as its \textit{switching energy}.
Recall that the terms in \cref{eq:2:E} simply change sign when the magnetisation of the magnet is reversed: if magnet $i$ reverses at time $t$, then $E_i(t^+) = -E_i(t^-)$.
Therefore, it is computationally cheap to calculate the switching energy, as $\Delta E_i = -2 E_i$. \par
...
An isolated single-domain nanomagnet often has two stable magnetisation states separated by energy barriers.
In IP ASI, these states typically originate from shape anisotropy: due to the demagnetising field, there is an energy cost associated with pointing the magnetisation along any direction.
This usually results in a preferential direction along the longest axis of the magnet's geometry~\cite{PhD_Leliaert}.
This anisotropy is quantified by the uniaxial shape anisotropy constant $K_\mathrm{u}$, leading to an energy barrier $\EB = K_\mathrm{u} V$.
Note that this expression is valid under the assumption that switching occurs by coherent rotation.
Similar to the calculation of the magnetic moment $\mu$ in \cref{fn:2:moment_integral}, edge relaxation effects may cause the effective volume, and therefore the energy barrier, to be slightly smaller than this. \par
Magnets in OOP ASI are usually still very flat, so one would expect the shape anisotropy to result in a preferential in-plane direction.
This can be avoided by choosing materials such as Co/Pt that exhibit a strong interfacial anisotropy which creates a preferential out-of-plane magnetisation direction.
...
However, this does not account for the extreme cases where the interactions are so strong that the energy barrier effectively disappears.
This happens when $\abs{\Delta E}$ exceeds twice the shape anisotropy $\EB$, leaving only one global minimum.
...
\begin{equation}
	\widetilde{E_\mathrm{B}} = \begin{cases}
		E_\mathrm{B} + \frac{\Delta E}{2} & \text{if } \abs{\frac{\Delta E}{2}} < E_\mathrm{B}, \\
		\Delta E & \text{otherwise}.
	\end{cases}
\end{equation}
This way, $\Delta E$ serves as the barrier when the original energy barrier disappears.

\subsubsection{IP: asymmetric barrier}
The simple mean-barrier approximation is insufficient for many IP ASI.
In real nanomagnets, the magnetisation must rotate for switching to occur: either clockwise ($\clockwise$) or counter-clockwise ($\counterclockwise$)~\cite{DirectionalEnergyBarrier}.
...
In the dipole model, this can be accounted for by keeping track of the energy of each magnet in these transitional `perpendicular' states.
We therefore introduce $E_\perp$, which represents the energy of a magnet if it would point \ang{90} counter-clockwise from its normal magnetization direction, but without taking into account the shape anisotropy ($E_\mathrm{B}$) associated with that orientation.
Note that we are not deviating from the two-state Ising model used by \hotspice: we are simply putting ``test dipoles'' in the perpendicular orientation, but the magnets are never actually put in these states. \par
Using this additional knowledge, the equation for the effective barrier along the two rotation pathways ($\pm$) then becomes
\begin{equation}
	\label{eq:2:E_B_asymmetric}
	\widetilde{E_\mathrm{B}} = \begin{cases}
		E_\mathrm{B} \pm \rho E_\perp + \frac{\Delta E}{2} & \text{if } \abs{\frac{\Delta E}{2}} < E_\mathrm{B} \pm \rho E_\perp, \\
		\Delta E & \text{otherwise}, \\
	\end{cases}
\end{equation}
...
The parameter $\rho = \mu_\perp/\mu_\parallel > 0$ was introduced in \cref{eq:2:E_B_asymmetric} to account for non-coherent magnetisation reversal processes like domain wall nucleation and propagation, which result in an effective reduction of the magnetic moment during reversal.~\cite{leo2021chiral,TimeResolvedDynamicsSOT}
Using a value $\rho < 1$ improves correspondence with experimental observations, as we will show in~\cref{sec:3:IP_Pinwheel_reversal}.
% TODO: mention that the perpendicular energy can be used to determine H_eff

\subsubsection{OOP: exact solution}
If an analytical expression for the energy as a function of magnetisation angle $\theta$ relative to the magnet's easy axis is known, then $\EBtilde$ can be calculated exactly.
...
}, the magnetostatic and Zeeman interactions add a term proportional to $\cos(\theta - \phi)$, with $\phi$ the angle of their combined effective field.
Thus, the total profile is a sum of two sines and can be fully characterised if $E$ and $E_\perp$ are known.
However, in the general case, this results in a transcendental equation requiring numerical approximation to solve, which is not done in \hotspice for performance reasons. \par
In OOP ASI, an explicit expression can still be obtained because the effective field aligns with the easy axis.
This causes the perpendicular energy to vanish ($E_\perp=0$) such that the equation is no longer transcendental.
Instead, this leads to a quadratic relation as described in~\ccite{StonerWohlfarth2008}:
\begin{equation}
	\widetilde{E_\mathrm{B}} = \begin{cases}
		E_\mathrm{B} \ab(\frac{\Delta E}{4 E_\mathrm{B}} + 1)^2 & \text{if } \abs{\frac{\Delta E}{2}} < E_\mathrm{B}, \\
		\Delta E & \text{otherwise}. \\
	\end{cases}
\end{equation}
