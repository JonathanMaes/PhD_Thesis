...
\hotspice includes the \python{hotspice.io} module, which provides basic input and readout methods.
These routines were developed in the early stages of development and therefore have a limited scope.
The same can be said for the \python{hotspice.experiments} module, which can determine reservoir computing metrics.
Since then, the \texttt{PRCpy} package~\cite{PRCpy} has become available, which focuses on calculating reservoir computing metrics.
This package is more broadly applicable and is now preferable over the built-in \python{hotspice.experiments} module.
