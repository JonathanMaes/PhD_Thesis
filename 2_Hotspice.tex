...
Since any magnet in the ASI may spontaneously switch to the opposite magnetisation state due to thermal fluctuations, \hotspice evaluates the time evolution of the ASI in a stepwise manner using an update algorithm that determines which magnet should switch next.
The two update algorithms implemented in \hotspice are examples of two distinct classes of kinetic Monte Carlo (KMC) algorithms --- rejection-free and rejection KMC.
In the context of ASI, we refer to these as \textit{N\'eel-Arrhenius switching} and \textit{Metropolis-Hastings}, respectively~\cite{PhysicalTimeKMC}.
...
\paragraph{Algorithm}
A single iteration of the N\'eel-Arrhenius algorithm is presented in~\cref{alg:NeelArrheniusSingle}.
Each iteration will increment the elapsed time by a certain amount $t \leq t_\mathrm{max}$, where the maximum time $t_\mathrm{max}$ can be set by the user to prevent excessively long switching times.
When using the N\'eel-Arrhenius update scheme, this iteration is simply performed as many times as is required to reach a desired elapsed time. \par
...
For example, when a sinusoidal signal of frequency $f$ is applied to the lattice, using $t_{\mathrm{max}}=20/f$ will ensure that the waveform is captured in sufficient detail.
