...
Three energy contributions are implemented in \hotspice, supporting both open and periodic boundary conditions (PBC).\footnote{
...
The first is the \textit{magnetostatic interaction energy} between magnets $i$ and $j$
\begin{equation}
	...
	\label{eq:E_MS}
\end{equation}
with $\mu_0$ the vacuum permeability and $\vc{r}_{ij} = \vc{r}_i - \vc{r}_j$ the vector connecting the two magnetic dipoles $\vc{\mu}_i$ and $\vc{\mu}_j$.
By default, this is the only interaction considered by \hotspice when an ASI is created, to avoid unnecessary calculations.
Any other energy contributions must explicitly be added to an ASI when it is created; see \cref{sec:2:API_energies}. \par
Secondly, the \textit{Zeeman energy} of an external field $\vc{B}_\mathrm{ext}$ interacting with magnet $i$
\begin{equation}
	E_{\mathrm{Z},i} = -\vc{\mu}_i \bcdot \vc{B}_\mathrm{ext} \mathrm{,} \label{eq:E_Z}
\end{equation}
is often needed when simulating ASI, as it provides a simple means to interact with the system, which is indispensable for RC purposes. \par
Finally, there is the \textit{exchange coupling energy} between nearest neighbours (NN) $i$ and $j$
\begin{equation}
	E_{\mathrm{exch},i,j} = J \frac{\vc{\mu}_i \bcdot \vc{\mu}_j}{\mu_i \mu_j} \mathrm{,} \label{eq:Eexch}
\end{equation}
with $J$ the exchange coupling constant.
This interaction is rarely present in ASI, but can be relevant in e.g. interconnected ASI -- whether by design or due to limited lithographic accuracy.
We will encounter an example of the latter in \cref{sec:3:OOP:MFM}. \\\par
...
This is called the \textit{switching energy}, and we will later see (\cref{sec:2:Dynamics}) that it takes a central place in the algorithms used for simulating dynamics.
It is therefore very advantageous to have such a simple method of calculating the switching energy. \par
When the simulation is initialized, the energy contributions are calculated for every magnet.
...
\subsection{API} % Structure of Hotspice, explain all modules (core, ASI, energies, (config? with GPU), io, experiments, gui and mostly deprecated plottools, utils but only those relevant to users)
 By default, \hotspice runs on the CPU. To run on the GPU, the environment variable \python{HOTSPICE_USE_GPU} must be set to \python{"true"} before the the \hotspice package is loaded (\python{import hotspice}) in a Python script.
\subsubsection{ASI}
\paragraph{\python{hotspice.ASI}}
