...
For a broader overview of Monte Carlo methods, we refer to Ref.~\cite{IntroductionMC}, which may also clarify the at times confusing naming present throughout literature. % TODO: combine this mention of IntroductionMC and Wolff etc. with the Critical Slowing Down discussion
...
However, just above $T_c$, the average magnetisation $\langle M \rangle$ evolves only slowly towards the expected value.
Increasing the number of MCS per temperature step brings the system closer to the theoretical equilibrium and reduces the temperature range above $T_c$ where the simulated system is not at equilibrium.
However, this has diminishing returns, as every additional MC step converges more slowly towards the equilibrium.
This slow convergence near the critical point is a symptom of the well-known phenomenon called ``critical slowing down'', which makes simulations at or near second-order phase transitions very inefficient~\cite{NumericalDynamicalNiedermayer,StatisticalMechanicsAlgorithmsComputations}.
