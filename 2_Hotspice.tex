...
To assess the potential of \xref{Reservoir Computing} in (perpendicular-anisotropy) \xref{Artificial Spin Ice}, as is the main topic of this thesis, a simulation framework is needed to efficiently explore the impact of various system parameters and input methods on reservoir performance. \\\par
...
An example of this is the flatspin simulator~\cite{flatspin}, which implements deterministic spin flipping via a \idx{Stoner-Wohlfarth} model~\cite{StonerWohlfarth2008}.
...
Interactions with other magnets or \xref{external fields} modify the energy landscape, leading to an effective barrier which we denote as $\EBeff$~\cite{leo2021chiral}.
...
\begin{enumerate}
	...
	\item The \idx{Zeeman energy} of an \xlabel{external field} $\vc{B}_\mathrm{ext}$ interacting with magnet $i$
	\begin{equation}
		\label{eq:2:E_Z}
...
\xfig[1.0]{2_Hotspice/EB_meanbarrier.pdf}{
	...
	Due to the limited information available to a \xref{mean-barrier model} (only $\EB$ and $\Delta E$), the exact solution assumes a sinusoidal shape anisotropy and a uniform effective \xref{external field} along the \xref{easy axis}.
...
Assuming a uniform magnetisation in each magnet\footnote{
	Only perfectly ellipsoidal magnets have uniform magnetisation in a uniform \xref{external field}~\cite{EllipsoidDemag,MaxwellElectricityMagnetism}.
...
It can also be used to apply time-dependent \xref{external fields}.
...
This is the reason why there often exists confusion between the Metropolis-Hastings algorithm and the so-called \idx{Glauber dynamics}\footnote{
...
This construction can be used to efficiently update the magnetostatic energy $E_\mathrm{MS}$ of all magnets whenever a magnet switches.
...
Such a convolution can be calculated very efficiently by using the fast Fourier transform (FFT). \\\par

The exact values stored in the kernel depend on whether any finite-size corrections to the magnetostatic interaction (\cref{sec:2:finite}) were used and whether the ASI is of the IP or OOP type. For example, an IP kernel using the \link{point dipole model}{point dipole approximation} contains elements
\begin{equation}
...
\xfig[1.0]{2_Hotspice/Kernel_PBC.pdf}{
	...
	Comparison of the magnetostatic interaction kernel for open (left) and periodic (right) boundary conditions, for a $16 \times 16$ pinwheel ASI (\crefSubFigRef{fig:2:ASIs}{a}) using the \link{point dipole model}{point dipole approximation}.
	The magnet of the unit cell site associated to this particular kernel is shown in the centre with a black outline.
...
The reason for this is that, whenever a magnet switches back, the error introduced by its original switch gets cancelled, leaving only some residual floating-point error in its wake.
...
\xfig[1.0]{2_Hotspice/Kernel_cutoff.pdf}{
	...
	Accumulation of error in the magnetostatic energy $E_\mathrm{MS}$ when using a truncated $41 \times 41$ kernel for a simulation of $100 \times 100$ pinwheel ASI at high temperature, starting from a uniform state.
...
This wrapping can instead also be achieved by performing a circular convolution of the truncated kernel with an $L_x \times L_y$ array whose only non-zero element is at the position of the switching magnet. \par
For a single switch, convolution with a truncated kernel is far slower than the original pointwise multiplication of \eqref{eq:2:Kernel_update}.
...
\end{equation}
Recall that we are using the Metropolis-Hastings acceptance probability in \hotspice's rejection-based kinetic Monte Carlo algorithm (\cref{sec:2:Dynamics_MH}) rather than the Glauber dynamics acceptance probability $P_\mathrm{GD}(\Delta E, T) = \frac{\exp(-\Delta E/k_B T)}{1+\exp(-\Delta E/k_B T)}$, because they both satisfy ergodicity and detailed balance but the Glauber acceptance probability is always more likely to reject a switch~\cite{bit-player_MCvsGlauber}.
...
\xfig[1.0]{2_Hotspice/MultiSwitch_select_Poisson.pdf}{
	...
	Characteristics of Bridson's Poisson disc sampling algorithm, based on $\approx \SI{e6}{}$ samples.
	\textbf{(a)} Probability density of simultaneously sampling the magnet at the center of the figure (white circle) and at any other point in the figure.
	The imposed minimal distance $\rmin$ is indicated by the dashed white circle.
	\textbf{(b)} Distribution of nearest-neighbour distances between sampled points, alongside the corresponding cumulative probability density.
	\textbf{(c)} Number of times each cell in the ASI was sampled, which must not display any obvious pattern.
	\textbf{(d)} Periodogram, i.e. the spatial Fourier transform of simultaneously sampled magnets, averaged over all iterations of the selection algorithm.
}
...
Finally, \crefSubFigRef{fig:2:MultiSwitch_select_Poisson}{d} shows the average spatial Fourier spectrum of a set of sampled points, called a periodogram.
...
Since this procedure does not guarantee a minimal distance between samples, a restricted SJG must be used instead, where samples are only taken from from the centre area of each square-shaped stratum, as discussed and illustrated in~\ccite{SamplingPolyominoes}. \par % Fig. 3b in SamplingPolyominoes is restricted SJG
...
\xfig[1.0]{2_Hotspice/MultiSwitch_select_Grid.pdf}{
	...
	Characteristics of the Jittered Grid selection algorithm, based on $\approx \SI{e6}{}$ samples.
	\textbf{(a)} Probability density of simultaneously sampling the magnet at the centre of the figure (white circle) and at any other point in the figure.
	The imposed minimal distance $\rmin$ is indicated by the dashed white circle.
	\textbf{(b)} Distribution of nearest-neighbour distances between sampled points, alongside the corresponding cumulative probability density.
	\textbf{(c)} Number of times each cell in the ASI was sampled, which must not display any obvious pattern.
	\textbf{(d)} Periodogram, i.e. the spatial Fourier transform of simultaneously sampled magnets, averaged over all iterations of the selection algorithm.
...
In some sense, this method is the opposite of Poisson disc sampling: it is less versatile, sparser and highly anisotropic, but is efficient to compute in parallel --- the number of sequential operations is independent of the number of simultaneous samples --- and synergises well with our grid-based ASI implementation.
...
\xfig[1.0]{2_Hotspice/MultiSwitch_select_Hybrid.pdf}{
	...
	Characteristics of the hybrid Poisson-grid selection algorithm, based on $\approx \SI{e6}{}$ samples.
	\textbf{(a)} Probability density of simultaneously sampling the magnet at the center of the figure (white circle) and at any other point in the figure.
	The imposed minimal distance $\rmin$ is indicated by the dashed white circle.
	\textbf{(b)} Distribution of nearest-neighbour distances between sampled points, alongside the corresponding cumulative probability density.
	\textbf{(c)} Number of times each cell in the ASI was sampled, which must not display any obvious pattern.
...
The sample distribution shown in~\crefSubFigRef{fig:2:MultiSwitch_select_Hybrid}{a} is in every way a hybrid of the spatial distribution of both earlier methods.
...
Two tables are shown:~\cref{tab:2:perf_init} for the initialisation time, and~\cref{tab:2:perf_switch} for the time it takes to perform 5000 switches using the \xref{first-switch method}, taking into account the magnetostatic interaction between all magnets.
...
Still, in all situations it is beneficial not to use the unoptimised kernel. \\\par
...
The \xref{first-switch method}, used in the preceding discussion about performance, does not implement a multi-switching procedure.
...
Even though the time required to perform a single switch with the first-switch method stays roughly constant up to $L<400$, the time per Monte Carlo sweep will scale linearly with the number of magnets if each iteration only switches a single magnet.
...
Around $70 \times 70$, the sampling rate increases rapidly, resulting in an increased number of Monte Carlo sweeps per second (MCS/s) that reach a local maximum for $200 \times 200$ magnets.
...
For GPU calculations, NVIDIA's \code{cuRAND} library provides several random number generators, of which we use\footnote{
...
\subsection{Package structure} % Q: should I put all the information of this section earlier already, at points where these subjects are discussed? I could e.g. create a custom "API" environment to explain different parts of the package in to show code outside the main text.
...
\subsubsection{ASI}
The \python{hotspice.Magnets} class implements all the core functionality of a simulation.
It controls the various technical aspects of the topics discussed earlier, like the multi-sampling methods, choice of KMC algorithm and the calculation of $\EBeff$, as well as more technical parameters such as the size of the truncated kernel.
...
To this end, the \python{hotspice.ASI} module provides two abstract classes, \python{IP_ASI} and \python{OOP_ASI}, which can be used to define a particular ASI lattice through inheritance, as is done for the built-in lattices shown in~\cref{fig:2:ASIs}.
Two parameters suffice to make a rudimentary ASI: the lattice parameter $a$ (the red indicator in~\cref{fig:2:ASIs}) and the size of the underlying grid $L_x \times L_y$.

\paragraph{Energies}\label{sec:2:API_energies}
The three \xref{energy contributions} in \eqref{eq:2:E} are provided in the \python{hotspice.energies} module and can be accessed from the main \python{hotspice} namespace.
Choosing between the dipole and \link{dumbbell model}{monopole} representations for the magnetostatic energy is currently done by using the \python{DipolarEnergy} and \python{DiMonopolarEnergy} classes, respectively.
...
By manually setting the \python{local_interaction} field of an \python{ExchangeEnergy}, longer-range \link{exchange coupling}{exchange interactions} can be applied.
\subsubsection{Reservoir computing} % io and experiments modules
...
The same is true for the \python{hotspice.experiments} module, which provides routines to determine kernel-rank and task-agnostic RC metrics.
...
A graphical user interface (GUI) is available for \hotspice, which allows the user to directly interact with the ASI and observe changes in real-time.
It can be run by calling \python{hotspice.gui.show(mm)}, with \python{mm} the ASI object, resulting in the window shown in~\cref{fig:2:GUI}.
...
The state of the ASI is prominently displayed, with some useful statistics shown in the bottom left panel.
The ASI display is controlled by the central bottom panel, which changes between 4 display modes.
...
The last mode shows the spatial distribution of some parameters like the temperature $T$, shape anisotropy $\EB$ and size of the magnetic moment $\mu$. \par
...
Specific magnets can be switched by clicking on the ASI plot with the mouse. Finally, the red box in the bottom right controls the update algorithm.

\paragraph{\python{hotspice.utils}}
While most functions in the \python{utils} module are meant for internal use, some can be useful in scripts.
...
\subsubsection{Grid}
Throughout this section, we have seen that the use of a regular grid has significant implications to the efficiency, applicability and implementation of \hotspice.
On the positive side, the grid enables fast and memory‐efficient calculation of the magnetostatic interaction because all unit cells can use common kernels.
With this approach, the magnetostatic interaction need not be truncated at a certain distance to prevent excessive memory usage, though the benefit of considering interactions across the entire system may be negligible.
For instance, the \texttt{flatspin} ASI simulator~\cite{flatspin} has no underlying grid, and therefore only stores interaction strengths within a certain radius, which does not significantly affect dynamics.
...
On the other hand, the grid restricts the usage of \hotspice to periodic ASI configurations which can be put on a regular lattice.
...
Multi-sampling in the Metropolis-Hastings scheme benefits greatly from the grid, as it enables efficient selection algorithms that maintain a minimal distance between samples.
...
In retrospect, porting \hotspice to the GPU was unnecessary for this thesis, as the ASI in \cref{ch:Applications} are relatively small.
The simulations in that chapter most often use the \xref{first-switch method}, which does not benefit from the multi-switching implementation either, so the majority of computations are performed on the CPU. \par
Nonetheless, GPU support has broadened the range of systems that \hotspice can efficiently simulate, in particular for the Metropolis-Hastings algorithm.
...
When an external magnetic field of magnitude $B$ is applied to a non-interacting ensemble of Ising spins, it follows directly from the partition function that the average magnetisation follows the relation
\begin{equation}
...
\xfig{2_Hotspice/Verification/OOP_Exchange.pdf}{
	...
	Markers show the \hotspice result for an $800 \times 800$ OOP square-lattice \link{exchange coupling}{exchange-coupled} Ising system using the Metropolis-Hastings algorithm with maximal multi-switching.
	The simulation was performed 3 times --- for different amounts of MCS (Monte Carlo steps per site) for each temperature step.
	Discrepancies due to critical slowing down above $T_c$ improve with increasing MCS.
}
...
This slow convergence near the critical point is due to a well-known phenomenon known as ``critical slowing down''.

\paragraph{Critical slowing down}
As the correlation length $\xi$ diverges at a second-order phase transition, the autocorrelation time $\tau$ will increase as $L^z$ in finite systems of linear size $L$~\cite{NumericalDynamicalNiedermayer}.
The dynamical critical exponent $z$ is typically $\gtrsim 2$ for local dynamics --- \textit{Niedermayer}~\cite{niedermayer1988general} notes that this is intuitively analogous to the problem of a random walk, where $\order{R^2}$ steps are required to traverse a distance $R$.
...
This phenomenon, termed \textit{critical slowing down} in second-order phase transitions (supercritical slowing down is an analogous phenomenon for discontinuous transitions), is particularly problematic for single-spin flip algorithms like Metropolis-Hastings, as single-spin flips are no longer physically relevant in this region~\cite{PhD_Reynal}.
...
Consequently, even after many MCS, the number of statistically independent configurations remains low, with the effective number of independent samples given by $N_\mathrm{eff} = N_\mathrm{sweep}/\tau$~\cite{niedermayer1988general,BeatCriticalSlowingDown1990}. \par

While CSD is to some degree an inherent property of the physical system --- physical quantities like $\xi$ and $\tau$ diverge near the phase transition --- the choice of simulation algorithm also has a significant impact~\cite{PhD_Reynal}.
Because CSD originates from long-range correlations, the Metropolis-Hastings algorithm exacerbates the issue due to its single-spin flip nature.
Other algorithms have been devised that alleviate CSD by nonlocal spin updates, thereby acting on collective modes~\cite{BeatCriticalSlowingDown1990}.
Among these are the group of cluster algorithms --- first the Swendsen-Wang~\cite{SwendsenWang} and later the Niedermayer~\cite{niedermayer1988general} and Wolff~\cite{Wolff} algorithms --- which construct clusters of magnets and flip all magnets in the cluster simultaneously. % Also multi-grid Monte Carlo~\cite{edwards1991multi}
...
However, in a system with long-range interactions, like the magnetostatic interaction, all magnets are ``neighbours'', and it generating an appropriate cluster becomes nontrivial.
...
Since CSD will not present significant issues in this thesis, the Wolff algorithm has only been implemented in \hotspice for exchange-coupled systems.
...
Analytical predictions remain possible in this system at zero temperature: for $\delta < 0.85$, the magnetostatic coupling dominates, leading to a checkerboard state~\cite{AgingIsingDipolar}.
As $\delta$ increases, the ever-stronger exchange coupling leads to the formation of ferromagnetic domains, which organise into stripes due to the magnetostatic interaction, with the stripe width determined by $\delta$~\cite{StripedDipolarIsing}. \par
...
Meanwhile, pinwheel ASI exhibits superferromagnetic order, where all magnets with similarly-oriented \link{easy axis}{easy axes} are magnetised in the same direction~\cite{ApparentFMpinwheel}.
