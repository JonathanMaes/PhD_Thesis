...
The number of magnets in the ASI has a major impact on performance, and is often the determining factor as to whether calculation on the GPU or CPU will result in a faster simulation.
Throughout the development of \hotspice, calculation on the GPU was made possible and the efficiency of calculating the magnetostatic interaction was improved drastically.

We start with a summarizing table of these various improvements, and explain them in more detail in the following sections.
...
% Q: should we talk about L or N? I would prefer L because we talk about 2D indexation, so L makes this clearer.
\xtable[tab:2:perf_init]{\textbf{Initialization time} for various simulation sizes $L$ (i.e. $L \times L$ magnets). `Mem' indicates excessive memory consumption, `?' means a simulation of that size takes prohibitively long ($\gtrsim \SI{1000}{\second}$).}{
...
\xtable[tab:2:perf_switch]{\textbf{5000 switches time} for various simulation sizes $L$ (i.e. $L \times L$ magnets). `Mem' indicates excessive memory consumption, `?' means a simulation of that size takes prohibitively long ($\gtrsim \SI{1000}{\second}$).}{
	\begin{tabular}{r|c|c|c|c|c|c}
		Simulation size $L$ & 50 & 100 & 150 & 200 & 400 & 1000 \\
		\hline \hline
...
Originally, \hotspice was written to calculate on CPU only.
...
By simply switching to GPU-based calculations using the \python{CuPy} library, this upper limit was increased significantly. The limiting factor instead became memory consumption of the unoptimized kernel. \par
% TODO: explain the unoptimized kernel here, and only explain the optimized kernel in the 'kernels' section.
After various kernel improvements, as detailed in~\cref{sec:2:Kernels}, the upper limit for feasible calculation time was increased to $L \approx 1000$ for GPU and $L \approx 400$ for CPU, with GPU outperforming CPU for $L \gtrapprox 300$ (last two rows of \cref{tab:2:perf_switch}).
The same goes for the initialisation time (\cref{tab:2:perf_init}), where the GPU already outperforms CPU starting at $L > 150$.

For small systems, the GPU still maintains the advantage when using the Metropolis-Hastings algorithm, due to the ability to perform multiple switches in that system and the speed at which such convolutions can be calculated on GPU.
...
\subsection{Multi-switching in Metropolis-Hastings}
