...
While the procedure detailed in~\cref{alg:NeelArrheniusSingle} is not the same, its yields an identical result as the BKL algorithm. % TODO REF
...
There often exists confusion between the Metropolis-Hastings algorithm and so-called ``Glauber dynamics''. % TODO: discuss acceptance probabilities and why they are correct (ergodicity, detailed balance). Can extend the small mention of detailed balance and ergodicity below the algorithm environment as well. Must already be mentioned here since we use it later in the minimal distance discussion.
...
Both update algorithms rely on generating a large amount of random numbers to select the next magnet(s) to switch. % TODO: expand by talking about XORWOW etc. and how choice of RNG actually doesn't really impact performance
