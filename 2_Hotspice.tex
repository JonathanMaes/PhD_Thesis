...
% TODO: apply changes applied in response letter, when response letter is sent
...
\hotspice explicitly accounts for the \link{magnetostatic interaction}{magnetostatic (MS) interaction} between all magnets.
...
\xfig[1.0]{2_Hotspice/MS_distance.pdf}{
	...
	Positions of north and south \link{magnetic monopoles}{monopoles} used in the dumbbell model are shown as red {\color{red}$\odot$} and blue {\color{blue}$\odot$} dots and are a distance $d=0.9l$ apart.
...
For the best correspondence with \mumax, the monopole-monopole distance $d$ should be set slightly shorter than the length $l$ of a magnet, typically around $d/l\approx0.9$. % Analytical: pi/4?
...
\sidefig{2_Hotspice/Unitcells.pdf}{
	...
	Construction of kagome ASI on a \xref{rectilinear grid}, as indicated by dotted lines.
...
For some magnets, though, the relative error exceeds \SI{15}{\percent} (red curve).
...
\subsubsection{Selection algorithms}\indexlabel{selection algorithm}\label{sec:2:MultiSwitch:Algorithms}
...
% API details are further discussed in~\cref{sec:2:API}.
...
For the \xref{dumbbell model}, the transition angle depends on the distance $d$ between \xref{magnetic monopoles}, but is always larger than for the dipole model~\cite{AFM-FM-transition-Pinwheel}.
