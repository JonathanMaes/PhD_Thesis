...
The rectilinear grid allows the straightforward application of first-order periodic boundary conditions (PBC) --- accounting for the 8 nearest copies of the ASI.
By adding 8 offset copies of an open-boundary kernel $\vc{\mathcal{K}}^{(q)}$ to itself, it can be transformed into a PBC kernel.
This is illustrated in~\cref{fig:2:Kernel_PBC} for an example of pinwheel ASI. \par
More specifically, 4 copies of the kernel are shifted along the axes by $\pm L_x$ or $\pm L_y$, and 4 copies are shifted diagonally by $(\pm L_x, \pm L_y)$.
This way, the adjacent ASI copies can be felt by magnets at the boundaries.
This construction works because kernel elements at the edges are only ever used to calculate the interaction between distant magnets on opposite sides of the ASI, so adjusting these elements can create the effect of PBC.

\xfig[1.0]{2_Hotspice/Kernel_PBC.pdf}{
	\label{fig:2:Kernel_PBC}
	Comparison of the magnetostatic interaction kernel for open (left) and periodic (right) boundary conditions, for a magnet in $16 \times 16$ pinwheel ASI.
}

This approach has the advantage that PBC do not impact performance, as they are baked into the kernel.
...
\xfig[1.0]{2_Hotspice/GUI.png}{
	...
	This example shows an $80 \times 50$ pinwheel ASI.
	\label{fig:2:GUI}
}
