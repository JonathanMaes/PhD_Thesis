...
% TODO: the final performance for metropolis-hastings

\xfigs[0.48]{2_Hotspice/Performance_CPU.pdf}{CPU}{2_Hotspice/Performance_GPU.pdf}{GPU}{
	\label{fig:2:Performance}Performance of OOP square ASI as a function of system size, using the Metropolis-Hastings algorithm with multi-switching ($Q=0.05$). Parameters: lattice spacing $a = \SI{1}{\micro\metre}$, $T = \SI{100}{\kelvin}$ and $\EB = 0$.
}
It is unclear what effect is causing the cliff in the CPU graph; it is not a lack of garbage collection, because starting the simulation at $L=100$ is exceedingly slow. Perhaps at $N=8000$ magnets the cache is full, making operations far less efficient from then on? % TODO: figure this out.
