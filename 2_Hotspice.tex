...
An increasing magnetic field $\vc{B}_\mathrm{ext}$ was applied downwards with a slight \ang{-3.6} offset to break the symmetry between sloped magnets, resulting in the creation of \xlabel{Dirac strings} of switched magnets. \par
...
The balance between these contributions results in a \xlabel{critical energy barrier} $E_\mathrm{B}^\mathrm{c}$: when the energy barrier $E_\mathrm{B}$ is lower (higher) than this value, a sloped (vertical) magnet will flip first, as illustrated in~\cref{fig:2:kagome_reversal_EBc}.
...
\sidefig[0.45]{2_Hotspice/Hysteresis_Kagome_EBc.pdf}{
	...
	\textbf{(b)} The effect of an \xref{external magnetic field} applied at \ang{-3.6} relative to the vertical axis on a vertical and sloped magnet in kagome ASI for different values of the energy barrier.
...
\xfig{2_Hotspice/Hysteresis_Kagome.pdf}{
	...
	Comparison between the \link{dumbbell model}{dumbbell} and \link{point dipole model}{point dipole} models at different external field magnitudes $B$.
	...
	The magnets have a \xref{magnetic moment} $\mu=\SI{1.1}{\femto\ampere\meter^2}$, and an \link{shape anisotropy}{energy barrier} $\EB = \SI{120}{\electronvolt} \pm \SI{5}{\percent}$.
