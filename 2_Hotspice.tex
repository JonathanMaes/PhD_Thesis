...
\newpage
\section{Artificial spin ice model}\indexlabel{point dipole model}
...
For an isolated nanomagnet, the energy barrier $\EB$ originates from its \xref{shape anisotropy} and can, for instance, be estimated from micromagnetic simulations.
However, interactions with other magnets or \xref{external magnetic fields} modify the energy landscape, leading to an effective barrier which we will denote as $\EBeff$~\cite{leo2021chiral}.
...
Due to the periodic nature of many ASI, \hotspice chooses to perform the simulation on a rectilinear grid as this allows efficient matrix and array manipulations, as well as straightforward implementation of both open and \xref{periodic boundary conditions} (PBC).
...
Therefore, \hotspice assumes the magnets in an ASI to be round (OOP ASI, e.g.~\cite{PerpendicularMagnetizationASI}) or elliptical (IP ASI), and allows the user to set the length $l$ and width $w$ of the magnets comprising an ASI.
...
Instead of representing a magnet as a \link{point dipole model}{point dipole}, one may instead choose to represent it as a pair of opposite \indexlabel{magnetic charge}magnetic charges~\cite{MagneticMonopoles2008,MagneticMonopoleDynamics}.
...
This is illustrated in~\crefSubFigRef{fig:2:EB_meanbarrier}{a}, where \indexlabel{energy landscape}energy landscapes are shown for various values of the \xref{switching energy} $\Delta E$.
...
Shape anisotropy is quantified by a \xlabel{uniaxial anisotropy} constant $K_\mathrm{u} = \frac{\mu_0}{2} M_\mathrm{sat}^2 \Delta \mathcal{N}$, with $\Delta \mathcal{N}$ the difference between the demagnetising factors along the IP easy and hard axes of a magnet~\cite{AdvancesASI,VogelFulcherTammannFreezing,andersson2016thermally}. % (when normalised as $\mathcal{N}_x + \mathcal{N}_y + \mathcal{N}_z = 1$)
...
\subsubsection{Asymmetric barrier}\indexlabel{asymmetric energy barrier}\label{sec:2:E_B_asymm}
...
It must also be noted that the attempt frequency can significantly depend on other system parameters like temperature~\cite{AttemptFreqTemperature}. \par % Ref says: ``When this energy barrier is the result of a collective statistical behavior of many constituents, it may contain an intrinsic temperature dependence which has to be carefully taken into account, in particular when the prefactor is interpreted in terms of an attempt frequency.''
The values for $\nu_0$ on the order of \SI{e10}{\hertz} are valid for reversal by \xref{coherent rotation}, as was already assumed during the calculation of the \link{effective energy barrier}{effective barriers}.
...
One of its main benefits is the existence of unit cells, which enables efficient \textit{kernels} for computing the \xref{magnetostatic interaction}. \par
...
\subsection{Grid}\indexlabel{unit cell}\label{sec:2:Implementation:Grid}
In \hotspice, an ASI is represented on a tiled \xlabel{rectilinear grid} of unit cells --- where the grid spacing need not be uniform --- with magnets positioned at selected grid points.
...
Each occupied grid point in the unit cell is assigned a unique index $q = 1,\dots,\widetilde{N}$, with $\widetilde{N}$ the number of magnets in a single unit cell. % TODO: maybe use \kappa instead of q? But it appears a lot in the following text.
...
Due to this equivalence, and the fact that $- N_x < \Delta x < N_x$ and $- N_y < \Delta y < N_y$, all possible interactions that a magnet at site $q$ can experience can be stored in a single $(2N_x-1) \times (2N_y-1)$ matrix $\vc{\mathcal{K}}^{(q)}_{ab}$, a \idx{unit cell kernel}.

\xfig[1.0]{2_Hotspice/Kernel_IP_Pinwheel.pdf}{
	\label{fig:2:Kernel_IP_Pinwheel}
	Two examples showing the kernels $\vc{\mathcal{K}}^{(q)}$ shifting with respect to the ASI grid for element-wise multiplication when a magnet switches.
...
The concept is illustrated a second time, now for the red magnet in the pinwheel ASI, which instead occupies position $q=\mathrm{B}$ in the \xref{unit cell} and therefore uses a different kernel; the red kernel $\vc{\mathcal{K}}^\mathrm{(B)}$.
...
\begin{equation}
	...
	\Delta \vc{E_\mathrm{MS}} = 2\mu_{xy} s_{xy}\vc{\bigmu} \odot \vc{S} \odot \ab(\vc{\mathcal{K}}^{(q)}_{ab})_{\substack{N_x < a + x \leq 2N_x \\ N_y < a + y \leq 2N_y}} \quad\mathrm{,}
\end{equation}
...
\end{equation}
where only the central $N_x \times N_y$ area of the convolution is calculated and the matrix $\vc{Q_q}$ contains 1 at sites with unit cell index $q$, otherwise 0.
...
\begin{equation}
	...
	E_{\mathrm{MS},i,j} = (s_i \mu_i) (s_j \mu_j) \vc{\mathcal{K}}^{(q_i)}_{N_x+x, N_y+y} = (s_i \mu_i) (s_j \mu_j) \vc{\mathcal{K}}^{(q_j)}_{N_x-x, N_y-y} \quad \mathrm{,}
\end{equation}
...
Since the middle region of the open-boundary \link{unit cell kernel}{kernel} already stores the interaction between nearby magnets, PBC can be applied by re-using these central values and moving the kernel by $\pm N_x$ and/or $\pm N_y$. \\\par
...
\xfig[1.0]{2_Hotspice/Kernel_PBC.pdf}{
	...
	The magnet of the \xref{unit cell} site associated to this particular kernel is shown in the centre with a black outline --- its 8 copies at $\pm 16$ lay just outside the kernel.
	The colour of other magnets indicates the magnitude of the magnetostatic interaction between the central and the coloured magnets if they both point `up'.
...
Since the kernel is nearly 4 times larger than the simulation domain, a straightforward performance improvement presents itself: \indexlabel{truncated kernel}{truncating} the \xref{magnetostatic interaction} at a certain distance.
Due to the underlying grid, the most natural way of achieving this is to reduce the size of the kernel from $(2N_x-1) \times (2N_y-1)$ to a smaller $(2R-1) \times (2R-1)$ central region, with $1 < R < \min(N_x,N_y)$.
...
Only the last 3 rows use the \link{unit cell kernel}{magnetostatic kernel} $\vc{\mathcal{K}}^{(q)}$ from~\cref{sec:2:Kernels}; the previous rows use the less efficient kernels considered earlier in that section.
...
\xtable[tab:2:perf_init]{
	...
	\begin{tabular}{r|c|c|c|c|c|c}
		...
		\hline
		GPU | \makecell{Unit cell kernel (see \S\ref{sec:2:Kernels})} & 2.4$\,$s & 4.6$\,$s & 9.7$\,$s & 16.3$\,$s & 66.2$\,$s & 730.4$\,$s \\
		\hline \hline
		GPU | \makecell{Optimised implementation\\(convolution initialisation)} & 1.0$\,$s & 1.0$\,$s & 1.0$\,$s & 1.0$\,$s & 1.6$\,$s & 23.9$\,$s \\
		\hline
		CPU | \makecell{Optimised implementation\\(convolution initialisation)} & 14$\,$ms & 0.2$\,$s & 1.0$\,$s & 3.2$\,$s & 53.1$\,$s & ? \\
		\hline
	\end{tabular}
}
...
\xtable[tab:2:perf_switch]{
	...
	\begin{tabular}{r|c|c|c|c|c|c}
		...
		\hline
		GPU | \makecell{Unit cell kernel (see \S\ref{sec:2:Kernels})} & 7.8$\,$s & 7.9$\,$s & 8.3$\,$s & 8.9$\,$s & 11.8$\,$s & 36.9$\,$s \\
		\hline \hline
		GPU | \makecell{Optimised implementation\\(2D indexation)} & 6.8$\,$s & 7.0$\,$s & 7.2$\,$s & 7.6$\,$s & 10.2$\,$s & 35.9$\,$s \\
		\hline
		CPU | \makecell{Optimised implementation\\(2D indexation)} & 0.56$\,$s & 1.3$\,$s & 2.5$\,$s & 4.3$\,$s & 41$\,$s & ? \\
		\hline
	\end{tabular}
} % Other options besides "optimised implementation": "current implementation", "production version", "improved version"?

The first two rows \textit{``Pairwise kernel''} in the tables use the na\"ive kernels $\vc{k}^{(i)}$ (or, equivalently, $\vc{K}^{(i)}$) that were described at the start of \cref{sec:2:Kernels} and used in \cref{eq:2:Kernel_k}.
...
First, a \xref{unit cell kernel} as described earlier in~\cref{sec:2:Kernels} had to be implemented, which requires far less memory by using the periodic nature of most ASI to only store interactions for a single \xref{unit cell}.
...
\xfigsnocap[0.47]{2_Hotspice/Performance_CPU.pdf}{2_Hotspice/Performance_GPU.pdf}{
	...
	The Metropolis-Hastings algorithm was used with \xref{multi-switching} $Q=0.05$, lattice spacing $a = \SI{1}{\micro\metre}$, temperature $T = \SI{100}{\kelvin}$ and energy barrier $\EB = 0$.
...
In retrospect, porting \hotspice to the GPU was not necessary for the work published in this thesis, as the ASI needed for the work presented in \cref{ch:Applications} are relatively small and are most often simulated using the \xref{first-switch method}.
...
\xfig{2_Hotspice/Verification/OOP_Exchange.pdf}{
	...
	The simulation was performed 3 times for different amounts of MCS, which is the number of \link{Monte Carlo sweep}{Monte Carlo steps per magnet} per temperature step --- i.e., the amount of attempted switches divided by the number of magnets $N$.
...
By employing algorithms that make abstraction of the time variable, we need not be limited by the divergent autocorrelation time. % TODO: fully address Jonathan's comment on this
...
\newpage
\section{Applications} % TODO: add any symbols and abbreviations here to those lists
% TODO: add the applications from the paper here, except the clocking in OOP ASI
...
Hence, a hysteresis in pinwheel ASI will be used to investigate the importance of the parameter $\rho = \mu_\perp/\mu_\parallel > 0$ that models \xref{non-coherent magnetisation reversal} in~\cref{eq:2:EB_asymmetric}.
...
Note that hysteresis can not be simulated using \xref{Metropolis-Hastings sampling}, because it ignores the uniaxial anisotropy (i.e., the energy barrier $\EB$) to sample the equilibrium state space.
...
% TODO: Make sure this section fits into the thesis, without loose references to things from the paper.
% TODO: Discuss the impact of the energy barrier approximations on the Pinwheel reversal.
...
Li~\etal~\cite{li2018pinwheel} performed an experimental study to observe the reversal of `diamond'-edge pinwheel ASI (\crefSubFigRef{fig:2:ASIs}{a}) under the influence of an external field.
Notably, when Li~\etal applied the field at an oblique angle (\ang{30}) to the ASI edges, the reversal occurred in two distinct steps: the sublattice which was more aligned with the field (\ang{15} to the easy axis) reversed first, followed by the second sublattice for which the field was at \ang{75}.
...
The magnetic moment and energy barrier of each magnet can be derived from the geometry of the $470\times170\times\qty{10}{\nano\meter}$ stadium-shaped magnets and the permalloy (\ce{Ni_80 Fe_20}) saturation magnetisation $M_\mathrm{sat}=\qty{800}{\kilo\ampere\per\meter}$.
...
\xfig{2_Hotspice/Hysteresis_Pinwheel.pdf}{
	...
	Hysteresis of the pinwheel ASI for an external magnetic field applied at \ang{30} with respect to the system edges.
	The parameter $\rho = \mu_\perp/\mu_\parallel$ modulates the effect of the asymmetric barrier as described in~\cref{sec:2:E_B_asymm}.
	\textbf{(a)} Component of the average magnetisation along the direction of the external field.
...
Note that the experimental hysteresis curve is not symmetric, in contrast to our simulations, which Li~\etal attributed to a small sample movement during their measurement which changed the applied field angle with respect to the array. \par
...
When the asymmetry of the energy barrier is neglected ($\rho=0$), the field magnitude required for the second step is significantly overestimated, as seen in the blue curve of~\crefSubFigRef{fig:2:Hysteresis_Pinwheel}{a} ($\approx\qty{60}{\milli\tesla}$ vs. experiment $\approx\qty{25}{\milli\tesla}$).
Increasing $\rho$ reduces the field magnitude at which the second step occurs but has little effect on the first step.
Notably, for $\rho \gtrapprox 0.6$, the second step disappears entirely, and the reversal instead occurs by domain nucleation at the edges of the ASI, as illustrated in \crefSubFigRef{fig:2:Hysteresis_Pinwheel}{e}. \par
...
\subsection{Kagome reversal} % TODO: This can show some interesting things about the dumbbell vs. point dipole models. Make sure this fits with the rest of the story now that this is in the thesis.
The in-plane kagome ASI (\crefSubFigRef{fig:2:ASIs}{g}) is a typical example of a \xref{frustrated} system: every vertex is formed by three magnets that must obey the two-in/one-out or two-out/one-in ice rule.
...
% At each vertex on the ASI, three magnets come together. This results in $2^3$ different vertex configurations. However, some configurations are equivalent, so only two classes are distinguished: those with 2 magnets pointing inwards or outwards, and those with 3.
%The saturated state of the ASI has all magnetic moments pointing in the positive y-direction, and hence has an average magnetic moment of $(1 + cos(\ang{60}) + cos(\ang{60})) m/3 = 2m/3$. This state can be achieved by applying a strong magnetic field in the positive y-direction.
% \cite{heyderman2013artificial,nisoli2013colloquium} also mention the experiment that Diego reproduced.
...
They studied the hysteresis loop using XMCD to image the evolution of the system's microstate during a gradual increase of the external field near the \xref{coercive field}.
We simulated a kagome ASI consisting of 173 magnets whose \link{shape anisotropy}{energy barrier} $\EB = \SI{120}{\electronvolt}$ with a standard deviation $\sigma(\EB)/\langle \EB \rangle=\SI{5}{\percent}$.
An increasing magnetic field $\vc{B}_\mathrm{ext}$ was applied at an angle of \ang{-93.6} relative to the horizontal axis.
This corresponds to a reverse field in the negative y-direction with a slight \ang{-3.6} offset to break symmetry between sloped magnets, resulting in the creation of \xlabel{Dirac strings} of flipped magnets. \par
As the field strength increases, the \xref{Zeeman energy} of the magnets rises, but this increase is faster for the vertical magnets than for the sloped ones.
However, the \link{magnetostatic interaction}{magnetostatic energy} is lowest for the vertical magnets in the initial state.
The balance between these contributions results in a \xlabel{critical energy barrier} $E_\mathrm{B}^\mathrm{c}$: when the energy barrier $E_\mathrm{B}$ is lower (higher) than this value, a sloped (vertical) magnet will flip first, as shown in~\crefSubFigRef{fig:2:kagome_reversal}{b}.
...
\xfig{2_Hotspice/Hysteresis_Kagome.pdf}{ % TODO: split in two
	\label{fig:2:kagome_reversal}
	Reversal of a kagome ASI under an \xref{external magnetic field} applied at \ang{-93.6} relative to the horizontal axis.
	\textbf{(a)} Comparison between the \link{dumbbell model}{dumbbell} and \link{point dipole model}{point dipole} models at different external field magnitudes $B$.
	Note that the critical field $B_C$ differs between the models:  $B_C = \SI{26.7}{\milli\tesla}$ for the dumbbell model and $B_C = \SI{23.9}{\milli\tesla}$ for the point dipole model.
	The NN centre-to-centre distance is \SI{500}{\nano\meter}, with a length $d=\SI{470}{\nano\meter}$ for the dumbbell model.
	The magnets have a \xref{magnetic moment} $\mu=\SI{1.1}{\femto\ampere\meter^2}$, and their \link{shape anisotropy}{energy barrier} is normally distributed with a mean of \SI{120}{\electronvolt} and \SI{5}{\percent} standard deviation.
	...
	\textbf{(b)} The effect of an externally applied magnetic field on a vertical and sloped magnet for different values of the energy barrier.
	$E_\mathrm{MS}$ is the same for each $E_\mathrm{B}$, however it is higher for the sloped magnets compared to the vertical ones.
	When $E_\mathrm{B}<E_\mathrm{B}^c$ ($E_\mathrm{B}>E_\mathrm{B}^c$), the sloped (vertical) magnet will flip first since the Zeeman energy $E_\mathrm{Z}$ increases faster for the vertical magnets.
}

The results, shown in~\crefSubFigRef{fig:2:kagome_reversal}{a}, reveal a qualitative difference between the dumbbell and dipole models.
In the dumbbell model, sloped magnets on the boundaries of the ASI flip first, while for the dipole method vertical magnets in the bulk flip first~\cite{DDG_Masterproef}.
In the experiment of Mengotti~\etal~\cite{mengotti2011kagome}, Dirac strings were observed to originate from sloped magnets, which is in agreement with the dumbbell simulation. % But they saw them in the bulk, which is not in agreement with the dumbbells.
...
Additionally, researchers using the `flatspin' ASI simulator, which employs a dipole-based model, observed Dirac strings to originate from vertical magnets, similar to our results with a point dipole model, highlighting the merit of the dumbbell model in capturing distinct dynamics~\cite{flatspin}.

\newpage
\section{Conclusion}
% TODO: check if this conclusion is sufficiently concluding for a thesis, i.e. not just a summary but also discussing the lessons learnt.
Throughout this chapter, the underlying model and implementation of \hotspice have been discussed in detail, with several model variants extending beyond the basic Ising model typically used for ASI simulations.
A first variant on this basic model considered more accurate calculations for magnetostatic interactions.
For OOP systems, it was found that a second-order correction on the magnetostatic energy $E_\mathrm{MS}$ brought it closer to its ideal value, while for IP systems a dumbbell model is preferred instead.
Their influence on the magnetostatic energy can also yield qualitatively different dynamics, as was for instance observed in the kagome lattice.
The second type of model variants considered improvements to the calculation of the energy barrier.
There, accounting for asymmetric switching channels proved key to e.g., correctly reproduce coercive fields in pinwheel ASI, provided that the reduced magnetisation during non-coherent reversal processes was also accounted for.
While it is possible to obtain an exact solution for the energy barrier in OOP magnets, the assumption of a sinusoidal energy landscape was found to be inaccurate, likely due to non-coherent reversal in real OOP nanomagnets. \par 
Furthermore, two distinct algorithms have also been implemented to handle switching events: the first-switch method and the Metropolis-Hastings algorithm.
...
In particular, the MH algorithm can benefit from multiple simultaneous switching events, which is justifiable if these switching magnets are sufficiently distantly spaced.
This technique was verified to yield correct results on a set of test cases using various lattices, and the accuracy of the first-switch method was later assessed in pinwheel and kagome ASI. \par
The usage of a grid to represent ASI brought with it several advantages, at the cost of losing complete freedom to place magnets wherever.
In particular, the grid enables the efficient calculation of the magnetostatic interaction between all magnets in the system, whereas other codes typically implement a truncation distance to improve performance.
It was found that truncation, even at a distance as far as 20 grid cells, incurs a \SIrange{5}{10}{\percent} error on the total magnetostatic interaction energy of a magnet, but it can be argued that the other approximations made are of similar accuracy, rendering such truncation justifiable.
Additionally, the grid enables an efficient simultaneous sampling of sufficiently distant magnets during the MH algorithm. \par
All of this makes \hotspice complementary to traditional micromagnetic simulations: by sacrificing the internal magnetisation details of individual nanomagnets, these higher-level approximations enable the study of complex ASI dynamics in much larger systems an over significantly longer timescales.
This opens new opportunities to model ASI for applied machine learning tasks like \xref{reservoir computing} (RC).
The ability of \hotspice to quickly sweep parameters and evaluate RC metrics facilitates the optimization of ASI configurations and the identification of suitable input protocols, an ability that will be put to good use in the next chapter.
