...
Note that hysteresis can not be simulated using \xref{Metropolis-Hastings sampling}, because it ignores the \xref{uniaxial anisotropy} (i.e., the energy barrier $\EB$) to sample the equilibrium state space.
...
This lattice exhibits \xlabel{hysteresis}: when a strong field drives it towards a uniform state, it will remain in that state after the field is removed since this is a ground state of the system. \par
...
\xfig{2_Hotspice/Hysteresis_Pinwheel.pdf}{
	...
	Hysteresis of the pinwheel ASI for an \xref{external magnetic field} applied at \ang{30} with respect to the system edges.
	...
	The experimental \xref{hysteresis} by Li~\etal~\cite{li2018pinwheel} is shown in black.
...
When the asymmetry of the energy barrier is neglected (i.e., $\rho=0$, equivalent to using the \xref{mean-barrier model}), the field magnitude required for the second step is significantly overestimated, as seen in the blue curve of~\crefSubFigRef{fig:2:Hysteresis_Pinwheel}{a} ($\approx\qty{60}{\milli\tesla}$ vs. experiment $\approx\qty{25}{\milli\tesla}$).
...
A possible explanation for this is that the switching of magnets in the experimental ASI does not occur by uniform rotation, as is assumed by the N\'eel-Arrhenius switching law (\cref{eq:2:Néel}).
...
They studied the hysteresis loop using \link{X-ray magnetic circular dichroism}{XMCD} to image the evolution of the system's microstate during a gradual increase of the \link{external magnetic field}{external field} near the \xref{coercive field}.
...
This corresponds to a reverse field in the negative y-direction with a slight \ang{-3.6} offset to break the symmetry between sloped magnets, resulting in the creation of \xlabel{Dirac strings} of switched magnets. \par
...
In the \xref{dumbbell model}, sloped magnets on the boundaries of the ASI flip first, while for the \xref{point dipole model} vertical magnets in the bulk flip first~\cite{DDG_Masterproef}.
In the experiment of Mengotti~\etal~\cite{mengotti2011kagome}, \xref{Dirac strings} were observed to originate from sloped magnets, which is in agreement with the dumbbell simulation. % But they saw them in the bulk, which is not in agreement with the dumbbells.
Furthermore, using other ASI software that employs a Stoner-Wohlfarth model (a type of point dipole model), Dirac strings were observed to originate from vertical magnets~\cite{flatspin}, similar to our results with a point dipole model.
...
A first variant considered more accurate calculations for magnetostatic interactions.
In OOP systems, it was found that a \link{finite dipole model}{second-order correction} on the magnetostatic energy $E_\mathrm{MS}$ yielded more realistic values, while for IP systems a \xref{dumbbell model} is more effective in reproducing experimental results.
...
The second type of model variants considered improvements to the calculation of the \xref{effective energy barrier}.
...
It was verified that this technique yields correct results for a set of test cases on various lattices, and the accuracy of the first-switch method was later assessed in pinwheel and kagome ASI by comparing to experimental results. \par
The usage of a grid to represent ASI brought with it several advantages, at the cost of some geometrical restriction.
