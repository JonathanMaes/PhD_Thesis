...
\xfig[1.0]{2_Hotspice/Kernel_IP_Pinwheel.pdf}{
	...
	Thin lines indicate individual cells in the simulation grid, thick lines delineate the unit cells.
...
% TODO: show in figure for visual example for pinwheel like I already tried back in the day for debugging? Explain in more detail how this offset works in the text? Can probably rework the following two sentences into the explanation of that future figure, because the offset kernels do not reach all the way through the original kernels so they are cut off already so must mention the 1/r³ decline already then.
...
\xfigs[0.47]{2_Hotspice/Performance_CPU.pdf}{CPU}{2_Hotspice/Performance_GPU.pdf}{GPU}{
	Performance of OOP square ASI as a function of system size, using the Metropolis-Hastings algorithm with multi-switching ($Q=0.05$).
	Lattice spacing $a = \SI{1}{\micro\metre}$, temperature $T = \SI{100}{\kelvin}$ and energy barrier $\EB = 0$.
