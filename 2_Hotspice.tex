...
Assuming that switching occurs by coherent rotation\footnote{
	Depending on the magnet's properties, switching by domain wall nucleation may be energetically cheaper and result in a lower energy barrier. We ignore these cases as they require micromagnetic simulations for an accurate energy barrier estimate.
}, this leads to an energy barrier $\EB = K_\mathrm{u} V$, with $V$ the magnet's volume.
Similar to the calculation of the magnetic moment $\mu$ in \cref{fn:2:moment_integral}, relaxation of the magnetisation profile $\vc{M}(\vc{r})$ near the surface of the magnet may cause the actual energy barrier to be slightly smaller. \par
