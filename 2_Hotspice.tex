...
% TODO: write down the mathematical form of the kernel elements (you know, with o_i and o_j and u_i etc. etc.)
\begin{equation}
	\label{eq:2:Kernel_unitcell}
	E_{\mathrm{MS},i,j}= (s_i \mu_i) (s_j \mu_j) \vc{K}^{(i)}_{ab} \mathrm{,} % TODO: this equation is ancient, adjust it using the new indexation paradigm and explain any necessary extra symbols required in the text
\end{equation}
...
Note that, if the asymmetric energy barrier is to be accounted for, two additional kernels are needed for each magnet in the unit cell: one where the central magnet is rotated \ang{90} (for initial calculation of $E_\perp$), and one where all other magnets are rotated \ang{90} (for updating $E_\perp$ when a magnet switches). \par % TODO: explain more, add formulae
