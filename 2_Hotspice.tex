...
The \xref{magnetic moment} and \link{shape anisotropy}{energy barrier} of each magnet can be derived from the geometry of the $470\times170\times\SI{10}{\nano\meter}$ stadium-shaped magnets and the \xref{permalloy} \xref{saturation magnetisation} $M_\mathrm{sat}=\SI{800}{\kilo\ampere\per\meter}$.
This yields a magnetic moment $\mu = M_\mathrm{sat} V = \SI{5.9e-16}{\ampere\meter\squared}$, and a \mumax~\cite{mumax3} simulation reveals that the energy barrier of such magnets is $\approx \SI{60}{\electronvolt}$.
