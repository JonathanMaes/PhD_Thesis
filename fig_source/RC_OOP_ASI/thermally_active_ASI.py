...
def get_thermal_mm(E_EA_ratio: float = 1, E_MC_ratio: float = 1, J_ratio: float = 0, E_B_std: float = 0.05, gradient: float = 0, size: int = 20, size_y: int = None, magnet_size_ratio: float = 0, MS_exponent: float = -3, T_factor: float = 1, pattern: str = 'AFM'):
...
def limit_MS_orthogonal(energy: hotspice.DipolarEnergy, horizontal=True, vertical=True):
    """ This removes all the dipole interactions between magnets which do not have the same x- or y- coordinates.
        This mimics the permalloy striplines that can be added to the system to reduce the MS decay with distance.
...
if __name__ == "__main__":
    # TODO: Next quest: see how the decay time changes as the 1/r^3 MS interaction is changed to extend further.

    ## MS INTERACTION AND DISTANCE-DEPENDENCE
    # 1) Is the MS interaction 1/r³ or relatively constant?
    ...
    mm = get_thermal_mm(E_EA_ratio=20, E_MC_ratio=2.5, E_B_std=0.0, size=20, gradient=0.1, pattern='uniform')
    inputter = UniformInputter(hotspice.io.RandomScalarDatastream(), magnitude=(0, 3e-4), n=2, frequency=1e4)
    # limit_MS_orthogonal(mm.get_energy('dipolar'))
