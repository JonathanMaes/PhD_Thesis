...
field_min, field_max = 40e-3, 90e-3
field_range = tuple(np.linspace(field_min, field_max, 51))

# E_B = (hotspice.utils.eV_to_J(110)*np.random.normal(1, 0.05, size=(nx, ny)),)
E_B = hotspice.utils.eV_to_J(110)
# E_B_std = 0
E_B_std = np.linspace(0, 0.2, 41)
...
if __name__ == "__main__":
    ...
    outdir = args.outdir if args.outdir is not None else "Clocking_binary_TA_EBstd"
    ...
    if args.iteration is None: # This gets run if the file is called without command line arguments
        plot_kwargs = {"param_x": "E_B_std", "unit_x": "%", "transform_x": lambda x: x*1e2, "n_xticks": 3,
                       ...
                       "metrics": ["NL", "MC", "PC"],
                       "colormap": "magma", "pad": 0, "subplots_adjust_kwargs": dict(wspace=0.25, top=0.85)}
