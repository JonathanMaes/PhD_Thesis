""" IN THIS SCRIPT, THE THERMAL RELAXATION OF OOP_SQUARE ASI IS SIMULATED, WHEN STARTING FROM A UNIFORM STATE.
    ...
    The main parameters that are varied within this simulation are:
        - J: The exchange coupling constant between nearest neighbouring magnets
    ...
    Other parameters which can be adjusted, but are not 'swept' throughout this simulation, are:
        - E_EA: The OOP anisotropy of each magnet.
...
def run(t_max: float = 1e4, MCsweeps: float = 4., samples: int = 20):
    ...
    """
    mm_kwargs = {'E_EA_ratio': 40, 'E_B_std': 0.05, 'size': 11, 'J_ratio': 0}
    ...
    varx_name, vary_name = "E_MC_ratio", "J_ratio" # NN Dipole-dipole interaction, and energy barrier, in multiples of kBT
    varx_values, vary_values = [0, 1.25, 2.5], [-8]
...
def plot(data_dir=None):
    ...
    # Setup figure
    figsize = (thesis_utils.page_width, thesis_utils.page_width*.38)
    fig, axes = plt.subplots(vary_values.size, varx_values.size, figsize=figsize, sharex=SAME_XAXIS_RANGE, sharey=True)
    fig.subplots_adjust(left=0.19, top=0.75, bottom=0.2, right=0.89, wspace=0.12, hspace=0.2) #! Already here, because insets require knowledge of final aspect ratio
    ...
    for i, vary in enumerate(vary_values):
        for j, varx in enumerate(varx_values):
            ...
            phase_1_finished = np.mean(m_avg, axis=0) < 0.14
    ...
    ## SAVE PLOT
    hotspice.utils.save_results(figures={"OOP_relaxation_exchange": fig}, outdir=data_dir, copy_script=False)
