...
def plot(data_dir=None):
    ...
    # Setup figure
    figsize = (thesis_utils.page_width, thesis_utils.page_width*.85)
    fig, axes = plt.subplots(vary_values.size, varx_values.size, figsize=figsize, sharex=SAME_XAXIS_RANGE, sharey=True)
    fig.subplots_adjust(left=0.19, top=0.86, bottom=0.11, right=0.89, wspace=0.12, hspace=0.25) #! Already here, because insets require knowledge of final aspect ratio
