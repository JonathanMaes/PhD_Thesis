...
def show_all_lattices():
    ...
    fig.text(x, (cutoff - 0.01 + margin)/2, "Out-of-plane", fontsize=48, rotation='vertical', horizontalalignment="center", verticalalignment="center")
    fig.add_artist(lines.Line2D([0.045, 0.045], [margin, cutoff - 0.01 + margin], **linekwargs))
    # fig.add_artist(lines.Line2D([0.05, 0.99], [cutoff, cutoff], **linekwargs | dict(linestyle=':')))
...
#         thesis_utils.label_ax(ax, i, fontsize=32, offset=(0.01, 0)) # To move some labels up, use: form="(%s)" + ("\n" if i < 4 else "")
#     # Add in-plane and out-of-plane titles
...
#     # fig.add_artist(lines.Line2D([cutoff]*2, [1 - (1-y)*2 - 0.01, 0.01], **linekwargs | dict(linestyle=':', marker=None, color='grey')))
#     # fig.add_artist(lines.Line2D([0.05, 0.99], [cutoff, cutoff], **linekwargs | dict(linestyle=':')))
#     hotspice.utils.save_results(figures={"ASIs": fig}, timestamped=False)
