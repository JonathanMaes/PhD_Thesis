...
def main_plot():
    ...
    figparams = [
        ...
        dict(n=8,  lw=1, ASI_type=hotspice.ASI.IP_Kagome,   colors = {(1,2): 'C0', (6,7): 'C1'})
    ]
    for style_kwargs in figparams:
...
def plot_kernel(ax: plt.Figure, x, y, w, n=5, lw=1, pos: tuple = (0, 0), magnet_enlargement=1, ASI_type: type[hotspice.Magnets] = hotspice.ASI.IP_Pinwheel, colors: dict|str = None):
    ...
    if kernel_unitcell_index < 0 or pos[0]>=ASI.nx or pos[1]>=ASI.ny:
        raise ValueError(f"There is no magnet at {pos=}")
    ...
    ax.add_artist(patches.Rectangle(xy=(x + (ASI.nx-1)*d, y0), width=d, height=d*n_kernel, color=color, alpha=0.25))
