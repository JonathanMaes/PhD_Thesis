...
class Hysteresis:
    ...
    def plot(*hystereses: 'Hysteresis', labels: list[str] = None, labels_snapshots: list[str] = None, original_data: str|Path = None, colors: list[str] = None, legend_title: str = None, H_SIprefix: str = 'm', FIELD_POLAR_ZERO: bool = False, SHOW_SNAPSHOTS: bool = None, _vertical: bool = None, show: bool = True, save: bool|Path = False):
        ...
        # ax_dummy_top = fig.add_subplot(gs_top[1], projection='polar')
        # ax_dummy_top.set_visible(False)
        ...
        for j, data in enumerate(hystereses):
            ...
            # for y in data.thresholds.thresholds: ax1.axhline(y, linestyle="--", color=color, linewidth=1)
            # POLAR PLOT
            ...
            for j, data in enumerate(hystereses):
                for i, state in enumerate(data.threshold_states):
                    ...
                    if _vertical:
                        ...
                        separator = ' ' if n_thresholds < 9 else '\n'
                        show_number = (j == 0) or (separator == ' ')
                        ax.set_title((r"$\bf{(%d)}$" % (i+1))*show_number + f"{separator}{H/H_factor:.1f}{H_SIprefix}T", pad=0, fontsize=fs, color=c)
        ...
        fig.tight_layout()
        if save:
            for ext in ['png', 'pdf', 'svg']: hotspice.plottools.save_plot(save, ext=ext, dpi=600, transparent=True)
        if show:
            plt.show()
        else:
            plt.close(fig=fig)
...
if __name__ == "__main__":
    ...
    """
    save = True
    ONLY_REPLOT = True #! only_replot=True because we now have 9 threshold states in the .pkl (sometimes it only gives 7)
    outdir_base = Path(Path(__file__).parent / (Path(__file__).stem + ".out"))
    ...
    def run_hysteresis_sweep(varname: Literal["angle", "E_B", "E_B_std", "m_perp_factor"], /, dirname=None,
        ...
        Hysteresis.plot(*hystereses, SHOW_SNAPSHOTS=show_thresholds, _vertical=show_thresholds, labels=labels, labels_snapshots=labels_snapshots, legend_title=legend_title, save=outdir/"Hysteresis_Pinwheel.pdf" if save else False, show=(not save), **kwargs)
        return hystereses, reversaltests
    
    run_hysteresis_sweep("m_perp_factor", angle=30, E_B_std=0.07, E_B=60, m_perp_factor=[0, 0.4, 1], dirname="thesis_fig", original_data=outdir_base / 'Li-data.csv', show_thresholds=True, only_replot=ONLY_REPLOT)
