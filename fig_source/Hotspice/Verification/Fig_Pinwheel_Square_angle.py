...
def inset_ax(ax: plt.Axes, fig: plt.Figure = None, ASI_type: hotspice.ASI.IP_ASI = hotspice.ASI.IP_Square):
    ...
    l, y, n = 0.025, 0.62, 2
    if on_left:
        draw_lattice(0.2, y, l, n=n, angle=0)
    else:
        draw_lattice(0.47, y, l, n=n, angle=30)
        draw_lattice(0.89, y, l, n=n, angle=45)
...
def plot(data_dir=None):
    ...
    for ax in (ax1, ax2):
        for k, monopole in enumerate(monopoles):
            ...
            ax.errorbar(data['angles'], np.mean(AFM_fraction[k,:,:], axis=1), yerr=np.std(AFM_fraction[k,:,:], axis=1), fmt=f"{thesis_utils.marker_cycle[k]}-", label=label)
    ...
    fig.supxlabel(r"Magnet rotation angle $\alpha$ [°]", fontsize=10, x=0.55, y=0.03)
