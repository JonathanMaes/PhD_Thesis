...
def draw_recurrent_network(ax: plt.Axes,
    ...
    - Hidden nodes placed via Poisson-disc-like sampling in [0.2,0.8]×[0.1,0.9]
    ...
    if data is not None:
        edges_in_hid, edges_hid_out, edges_rec, hidden_positions, rads = data["edges_in_hid"], data["edges_hid_out"], data["edges_rec"], data["hidden_positions"], data["rads"]
    else:
        ...
        rads = rng.uniform(-0.3, 0.3, size=len(edges_rec))
    ...
    for i, (src, dst) in enumerate(edges_rec):
        start, end = hidden_positions[src], hidden_positions[dst]
        ax.add_patch(FancyArrowPatch(start, end, arrowstyle='-|>', mutation_scale=12, connectionstyle=f"arc3,rad={rads[i]}", color=edge_color, lw=1.5))
