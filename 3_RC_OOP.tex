...
Note that ``non-volatile'' does imply a non-interacting system; a sufficiently high magnetostatic coupling will still induce local AFM order.
...
Instead, there are only two regimes for non-volatile ASI: one where the magnetostatic interaction is sufficiently strong to overcome the energy barrier $\EEA$, and another where it is not.
