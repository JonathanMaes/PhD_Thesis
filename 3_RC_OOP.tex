...
Furthermore, the magnets only respond within a narrow range of fields near the coercive field, leading to a very steep hysteresis loop.
...
Such a two-step clocking protocol therefore allows domain wall boundaries to advance one step at a time, while avoiding avalanches that would result from a stronger input. \par
The main takeaway from this concept is that two key factors are necessary to achieve controlled domain wall movement in any ASI~\cite{MAES-24}.
\begin{itemize}
...
Any subsequent input cycle of opposite sign would either be too weak to have any additional effect, or be strong enough for the previously switched magnets to switch back, after which the same process can repeat.
