...
\subsubsection{Two-stage relaxation}\indexlabel{relaxation stages}
...
\xfig{3_RC_OOP/OOP_relaxation_continuous.pdf}{
	...
	Five regions can be distinguished: $\mathrm{I}$ -- frozen state, $\mathrm{II}$ -- transient states, $\mathrm{III}$ -- state with domains and domain boundaries, $\mathrm{IV}$ -- checkerboard ordering, $\mathrm{V}$ -- \indexlabel{superparamagnetism}superparamagnetic state.
...
The equilibrium magnetisation depends only on the input field magnitude $vB$, resembling the behaviour of a \xlabel{leaky integrator}~\cite{jaeger2001echo}.
...
The particular signal we will be attempting to predict with the thermally active OOP ASI is the chaotic \xlabel{Mackey-Glass oscillator} $\mathrm{MG}(t)$~\cite{MackeyGlass}, as this is a popular benchmark in literature~\cite{RotatingNeuronsRC,AdaptiveProgrammableRC,TaskAdaptivePRC,Moon_2021,JaegerHaasWireless,ArchitecturalMarkovianESN}.
...
We refer to this method as the \idx{AFM clocking protocol}.
