...
The main factor that motivated our research of OOP ASI for \xref{reservoir computing} (RC) was that these systems allow very efficient and relatively simple input and read-out methods to be used.
...
By recording the reservoir state $\vc{x}(t_i)$ and the target output $y(t_i)$ at discrete intervals $t_i = i t_\mathrm{max}$ (with $i = 1,\dots,n$), the readout layer can be trained on several periods of the input signal via ordinary least-squares (OLS) regression\footnote{
...
\vspace{-1em} % TODO: change legend labels to be consistent between both panels
...
\xfig{3_RC_OOP/Nonvolatile/Clocking_protocol.pdf}{
	...
	Illustration of the \xref{AFM clocking protocol} for controlled domain wall movement in ASI.
...
\makeshiftfig{ % TODO: different color scheme, this looks awful in print. Also different legend for domains, everyone is confused by it (try a triangle instead, with ... diagonally down).
...
\makeshiftfig{ % TODO: highlight leaking in this figure, e.g. with red dots at leaked magnets. Refer to leaking in caption.
