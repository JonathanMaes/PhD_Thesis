...
The main factor that motivated our research of OOP ASI for \xref{reservoir computing} (RC) was that this kind of system allows very efficient and relatively simple input and read-out methods to be used.
...
For example, the \xlabel{RKKY interaction}~\cite{RKKY_RK,RKKY_K,RKKY_Y} either promotes a parallel or anti-parallel magnetisation of successive \ce{Co} layers, depending on the precise \ce{Pt} layer thickness.
...
\link{avalanches}{Avalanches} can be avoided by instead applying two sub-steps for each applied input value.
...
\xfig{3_RC_OOP/Nonvolatile/Clocking_protocol.pdf}{
	...
	Each input cycle consists of two sub-steps that each affect a different set of magnets, arranged in a checkerboard pattern: as such, only half of the magnets are stimulated at once.
