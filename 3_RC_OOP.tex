\chapter{Reservoir computing in out-of-plane artificial spin ice}\label{ch:Applications} % TODO END: link to index terms
...
The relaxation process can be tracked by the normalised average magnetisation $\mavg = \abs{\llangle s \rrangle}$ and the local \xref{antiferromagnetic} parameter $\qNN = (1 - \langle s_i s_{i+1} \rangle)/2$.
...
The particular signal we will be attempting to predict with the thermally active OOP ASI is the chaotic \xlabel[tosection]{Mackey-Glass oscillator} $\mathrm{MG}(t)$~\cite{MackeyGlass}, as this is a popular benchmark in literature~\cite{RotatingNeuronsRC,AdaptiveProgrammableRC,TaskAdaptivePRC,Moon_2021,JaegerHaasWireless,ArchitecturalMarkovianESN}.
...
\vspace{-1em} % TODO FIG: change legend labels to be consistent between both panels (and do the same for all other waveforms in previous figures)
