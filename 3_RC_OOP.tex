...
To achieve this, a low MS coupling $\EMC=2.5\kBT$ will be used --- chosen to be as low as possible while avoiding the superparamagnetic regime.
As can be seen in~\cref{fig:3:OOP_relaxation}, systems with this level of coupling relax over timescales suitable for computation (i.e., $\mavg \rightarrow 0$ within $\approx \SI{1}{\milli\second}$) when $\EEA \leq 20\kBT$.
...
Therefore, all subsequent simulations of reservoir computing in thermally active ASI will use the standard values $\EEA=20\kBT$ and $\EMC=2.5\kBT$ (unless otherwise specified) while varying other parameters to determine the optimal input conditions for such a system. \\\par
...
This means that the external field providing the input can take any random magnitude between 0 and $B$, and each value is maintained for a period determined by the input frequency $f$.
...
\xfig{3_RC_OOP/OOP_relaxation_input.pdf}{
	...
	The NN magnetostatic interaction is $\EMC=2.5\kBT$ and the net OOP anisotropy is $\EEA = 40\kBT$.
...
Logically, the lower the frequency $f$ and the higher the input magnitude $B$, the easier the system reaches saturation --- the uniform state, characterised by $\mavg=1$ and $\qNN=0$.
Optimal reservoir computing can be expected when the system is never saturated, but uses as much of the available $\mavg$-range as possible to minimise the relative impact of the stochastic nature of the relaxation process.
...
This is most striking for high-frequency input --- for instance, the value of $\mavg$ remains statistically unaltered during input cycles 7--9 in panels 4--6 of~\cref{fig:3:OOP_relaxation_input}, regardless of the input magnitude $B$, while the input values applied during those cycles differ significantly between $0.54$ and $0.73$.
Therefore, the stochastic nature of the relaxation can, in some cases, prevent the perceptron from distinguishing between closely spaced input values.
...
Throughout this section, we will examine various input methods for such non-volatile ASI and develop a protocol that demonstrates promising memory capacity.

\paragraph{Magnetostatic interaction}
% TODO END: is this correct after everything I wrote?
...
The main issue is that the AFM input applies a global stimulus, which can easily cause avalanches of switching magnets and drive the system to saturation.
...
In this regard, the AFM input method is not fundamentally different from the uniform input strategy we previously dismissed.
...
This allows it to achieve saturation already with a far weaker input, since it only needs to affect domain wall magnets which have just $\leq 3$ oppositely magnetized NN and are therefore stable with $\EBeff \propto \EEA + 2\EMC$, as compared to $\EEA + 4\EMC$ for bulk magnets.
...
The fact that the AFM input can selectively influence domain walls is crucial, as it enables us to avoid saturation by introducing one last change to the input strategy.
...
This way, we avoid the avalanches that plagued the global input strategies. \par
...
To use this clocking protocol for RC, input values must be mapped to a certain cycle.
Since we are using the growth and shrinking of domains, and we intend to move domain walls by only one lattice unit at a time, this two-step input protocol requires binary input data.
...
This clocking protocol, coupled with this definition of the input, automatically provides basic memory in the system, as the previous few input bits are encoded in the size of the domains.
Since this clocking protocol relies on deterministic switching, the duration over which the sub-steps are applied is irrelevant; \qty{1}{\second} will be used in all simulations. \par
...
The top half of this figure shows the magnetisation state of each magnet, while the bottom half shows the same states but instead visualises the two domain types as black and white.
...
\makeshiftfig{
	...
	Evolution of a $16 \times 16$ OOP square-lattice ASI when using the two-step AFM clocking protocol (for seven cycles $A$ and seven opposite cycles $B$).
...
These domains nucleate at the edges of the ASI: this occurs both during the first $A$-cycle as well as the first $B$-cycle.
Any existing domains of the type promoted by the applied cycle also grow at the expense of the opposite domain type: in the figure, this occurs for example in the centre of the ASI when the $B$-cycles are applied. \par
A very important feature of this input method that can be noted from the figure, is that the states visited by cycle $B$ were not previously visited by cycle $A$.
In other words, cycle $B$ and cycle $A$ are not each other's inverse, even though they simply encourage the opposite magnetisation directions as the other cycle does.
This is of great importance for RC, because this enables the often desired fading memory as information is always retained from the previous input cycles. \par
Note also that, even though this system is non-volatile and $\sigma(\EEA) = 0$, some stochasticity can still be present and induce asymmetry.
A clear example of this is found when the $B$-cycles are applied: during the first $B$-cycle, the magnets at the bottom right do not switch, even though they did switch symmetrically during the first $A$-cycle.
This occurs because the strength of the input must strike a delicate balance to encourage domain wall magnets to switch, without causing other magnets to switch.
...
Furthermore, as soon as this asymmetry occurs, the ASI state after subsequent steps becomes increasingly asymmetric as the magnetostatic interaction is now also asymmetric, indicating that this clocking protocol in OOP ASI also exhibits some non-linearity as required for RC. \par
Note that the average magnetisation indicates what the last applied input value was, but it provides very little on previous inputs and therefore is not used as an output value.
...
When disorder is introduced in the system --- once again in the form of a random normally distributed anisotropy for each magnet --- the first steps of the clocking protocol will no longer be as clean and symmetric as they were during the $A$-cycles in~\cref{fig:3:Clocking_clearly_EBstd0}.
If the disorder is significant, the difference between low- and high-barrier magnets in the system can become substantial, making it harder to selectively address domain wall magnets.
Therefore, the field magnitude now has to be chosen carefully such that the domain walls can still propagate through all the defects in the system, but without making the stimulus so strong that domain walls can propagate through most of the non-stimulated magnets during each sub-step of the clocking protocol.
This is the same balance that already had to be struck in the earlier example, but which is now further complicated by the random distribution of $\EEA$. \par
It is rarely possible to find an input magnitude that satisfies all these constraints, so one has to be content with some imperfections.
Either one makes the input rather weak, causing some high-barrier magnets to be non-switchable while guaranteeing that no unstimulated magnets will switch.
Alternatively, one can make the input relatively strong, to ensure that the input can switch any magnet in the system when needed, but at the cost of having some ``leaking'' through the unstimulated magnets. % TODO: what to call these magnets? Unstimulated? Non-addressed? 
The latter is preferable since non-switchable magnets do not provide any compute capability to the system and their frozen $\uparrow/\downarrow$ state depends on the initial state of the system, introducing an additional variable factor.
Leaking, on the other hand, is not a big issue as long as the domain wall does not move through too many unstimulated magnets per input step.
In fact, such ``leaking'' can be beneficial to the non-linearity of the system, as it readily provides asymmetry in the system.
An example with $\sigma(\EEA) = \SI{5}{\percent}$ is shown in~\cref{fig:3:Clocking_clearly_EBstd5}, where such leaking can be observed.
...
\makeshiftfig{
	...
	All magnets are initially magnetised in one of the antiferromagnetic ground states (white). The magnetic field used in the clocking cycles has a magnitude $B_\mathrm{ext} = \qty{5.5}{\milli\tesla}$, and each step is applied for \qty{0.5}{\second}.
}

The $\EEA$-defects indeed cause more disordered domain dynamics.
If these defects were even more pronounced, domain nucleation could also occur in the bulk of the system.

\paragraph{Strength of magnetostatic interaction}
One of the main factors affecting domain wall movement is the $\EMC/\EEA$-ratio, as long as $\EEA$ is sufficiently high to yield a non-volatile system.
In the previous example of~\cref{fig:3:Clocking_clearly_EBstd5}, a very high ratio $\EMC/\EEA = 2$ was used.
Typically, it is easier to manufacture systems with low $\EMC/\EEA$ values, but they behave more erratically since the \SI{5}{\percent} disorder becomes more influential in such cases relative to the magnetostatic interaction.
To illustrate this,~\cref{fig:3:Clocking_clearly_EBstd5_EMC} shows a system with $\EMC/\EEA = 0.2$ and $\sigma(\EEA) = \SI{5}{\percent}$ still.
...
\makeshiftfig{
	...
	\textbf{(a)} shows a system with high magnetostatic interaction $\EMC = 400 \kBT$, requiring an external input magnitude $B_\mathrm{ext} = \qty{11.2}{\milli\tesla}$ for controlled domain wall movement.
	\textbf{(b)} shows a system with $\EMC = 40 \kBT$ and $B_\mathrm{ext} = \qty{3.78}{\milli\tesla}$ instead.
}

\subsection{Reservoir computing}
Due to the binary character of the clocked input protocol --- each cycle either grows or shrinks domains of either type --- it is not possible to apply scalar input in a straightforward manner.
% TODO: The limitation of binary input will be explored further in~\cref{sec:3:clocking_input_encoding}.
This limits the available options of which RC tasks can be used as benchmarks for this non-volatile system, as many tasks --- like the previously used signal transformation --- can not be performed with binary input.
Instead, we opt to determine more general RC metrics for this clocked non-volatile OOP ASI.
In the introduction (\cref{sec:1:RC_metrics}), two such classes of metrics were discussed: metrics based on matrix ranks, and the task-agnostic metrics.
In the following, the clocking protocol on the OOP square-lattice ASI will be evaluated using both classes of metrics.

\paragraph{Baseline system}
We will initially calculate RC metrics for a baseline ASI, after which we will evaluate the effect of several system parameters by adjusting them individually.
Magnets in this baseline ASI have a very large perpendicular anisotropy $\EEA = \SI{110}{\electronvolt} \approx 4255\kBT (\pm \SI{5}{\percent})$ and have the same magnetic moment as used for the thermally active ASI ($\mu = \SI{2.5e-16}{\ampere\metre\squared}$), which was based on $D_\mathrm{NM} = \SI{170}{\nano\metre}$ magnets.
Based on this geometry, the magnetostatic coupling $\EMC$ can be controlled by varying the lattice parameter $a$.
To avoid an unintentionally interconnected ASI as previously encountered in~\cref{sec:3:MFM}, a lower limit $a > D_\mathrm{NM} + \SI{25}{\nano\metre} = \SI{195}{\nano\metre}$ is imposed.
The best RC performance was achieved for systems where the MS coupling $\EMC$ is far smaller than the anisotropy $\EEA$\footnote{
	The values for $\EEA$ and $\EMC$ used here are not expressed as multiples of $\kBT$ because the research of non-volatile ASI was actually conducted before thermally active ASI were considered, so the notion of $\EEA$ and $\EMC$ as a multiple of $\kBT$ was not yet in use.
	Therefore, here, $\EEA$ is set in \SI{}{\electronvolt} and metrics are determined as a function of the lattice parameter $a$ rather than the magnetostatic coupling $\EMC$.
}, resulting in clocking dynamics similar to \crefSubFigRef{fig:3:Clocking_clearly_EBstd5_EMC}{b}. \par
To determine the optimal system parameters to perform RC with this clocking protocol, the input magnitude (i.e., external field magnitude $B$) and magnetostatic interaction (i.e., lattice spacing $a$) were varied, while the anisotropy was kept constant at a high value to ensure a non-volatile system.
Note that the absolute value of the external field magnitude $B$ is mostly irrelevant, as it depends on the specific values of $\EEA$ and $\EMC$ which affect the effective energy barrier $\EBeff$ of domain wall magnets.
A different choice of $\EEA$ and/or $\EMC$ will necessitate a different input magnitude $B$, but will not significantly alter the general trend of the metrics as function of the input magnitude.
Since the input strength has a significant effect on the clocking dynamics, it was varied to ensure that the ``sweet spot'' was hit for every value of the lattice spacing $a$. \par
Accounting for the finite size of magnets was found to have a negligible impact on the reservoir performance, apart from shifting the whole metric map slightly towards larger lattice constants $a$ by about \SIrange{20}{30}{\nano\metre}. % TODO: do we want to show this as figures? (Sweep20250325131714 vs. Sweep20250325105313)

\subsubsection{Metrics}
\paragraph{Matrix rank-based metrics}
Three matrix rank-based quantities can be calculated: the kernel-quality $K$, generalisation-capability $G$ and computing capacity $Q=K-G$.
The latter is the main metric of interest, as it can be used as an indication of how suitable a system is for RC purposes.

\xfig{3_RC_OOP/Nonvolatile/Sweeps/Sweep20230208082817.pdf}{
	\label{fig:3:Clocking_Sweeps_binary_KQ}
	Kernel-quality $K$, generalisation-capability $G$ and computing capacity $Q$ of a $20 \times 20$ OOP square-lattice ASI with $\EEA = \SI{110}{\electronvolt} \pm \SI{5}{\percent}$, over a range of lattice constants $a$ and input field magnitudes $B$.
	The corresponding magnetostatic coupling $\EMC$ ranges from $\approx 146\kBT$ (at $a=\SI{220}{\nano\metre}$) to $\approx 88\kBT$ (at $a=\SI{260}{\nano\metre}$) --- this is the optimum range, as the computing capacity decreases outside this range.
	The magnets had the same magnetic moment as the thermally active ASI, i.e. $\mu = \SI{2.5e-16}{\ampere\metre\squared}$.
	The readout used the average domain type of each $2 \times 2$ set of magnets, for a total of 100 readout values.
	Hence, the maximum value of $K$, $G$ and $Q$ is 100.
}

The matrix rank-based metrics require as many input sequences as there are readout nodes, with each input sequence consisting of a certain number of input values.
Specifically, we applied 100 random input values for every input sequence, and applied 100 such input sequences since we have 100 readout nodes.
For further insight, we also determined task-agnostic metrics in the same range.
% TODO: explain what I say in the powerpoint presentation of 20230207, or at least check if everything useful has been said
...
% TODO SIM: sweep of E_B_std and input magnitude at a well-chosen lattice spacing to showcase the effect of E_B_std?
