...
} in~\cref{fig:3:Clocking_Sweeps_binary_TA}.
...
That the parity check reaches higher values than the memory capacity was to be expected, as it is specifically tailored to binary input data whereas memory capacity is more generally applicable to scalar data. % TODO: what does PC actually measure?
...
\subsubsection{Alternative input encoding} % Binary vs. byte etc. Sigmoid thingy?
...
The first concept is to use an analog-to-digital conversion that encodes analog values as integers represented as bytes consisting of 8 bits.
...
The non-linearity, on the other hand, was found to be mostly unaffected when a little-endian byte representation (i.e., applying the most significant bit last) is used, but all in all this is not a viable method. \par
...
\newpage
\section{Conclusions}
% TODO: expand with the thermally active conclusions
