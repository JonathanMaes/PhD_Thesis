...
\makeshiftfig{
	...
	Evolution of the spatial distribution of domain types in a $20 \times 20$ OOP square-lattice ASI when using the two-step AFM clocking scheme (for six cycles $A$ and six opposite cycles $B$).
	All magnets are initially magnetised in one of the antiferromagnetic ground states (white).
	The finite size of magnets was accounted for with $D_\mathrm{NM} = \SI{170}{\nano\metre}$ and $S_\mathrm{ASI} = \SI{200}{\nano\metre}$.
...
Due to the binary character of this input protocol, it is not possible to apply scalar input.
% TODO: The limitation of binary input will be explored further in~\cref{sec:3:clocking_input_encoding}.
This limits our options for RC tasks, as we can for example not perform the previously used signal transformation task with this non-volatile system.
Instead, we opt to determine more general RC metrics for this clocked non-volatile OOP ASI, like the matrix-rank based and task-agnostic metrics that were previously detailed in~\cref{sec:1:RC_metrics}. \\\par

We will determine metrics for an ASI with a very large perpendicular anisotropy $\EEA = \SI{110}{\electronvolt} \approx 4255\kBT (\pm \SI{5}{\percent})$ and whose magnets have the same magnetic moment as for the thermally active ASI ($\mu = \SI{2.5e-16}{\ampere\metre\squared}$), which was based on $D_\mathrm{NM} = \SI{170}{\nano\metre}$ magnets.
Based on this geometry, we may control the magnetostatic coupling $\EMC$ by varying the lattice parameter $a > \SI{195}{\nano\metre}$ --- this lower limit being $D_\mathrm{NM} + \SI{25}{\nano\metre}$ to avoid an interconnected ASI as previously encountered in~\cref{sec:3:MFM}.
...
\paragraph{Kernel-quality, generalisation-capability and computing capacity}

First, we can determine the matrix-rank based metrics as explained in the introduction (\cref{sec:1:RC_metrics_KQ}).
Three quantities can be calculated in this regard: the kernel-quality $K$, generalisation-capability $G$ and computing capacity $Q=K-G$.
The latter is the main metric that can be used as an indication of how usable a system is for reservoir computing.

\xfig{3_RC_OOP/Nonvolatile/Sweeps/Sweep20230208082817.pdf}{
	\label{fig:3:Clocking_Sweeps_binary_KQ}
	Kernel-quality $K$, generalisation-capability $G$ and computing capacity $Q$ of a $20 \times 20$ OOP square-lattice ASI with $\EEA = \SI{110}{\electronvolt} \pm \SI{5}{\percent}$, for a range of lattice constants $a$ and input field magnitudes $B$.
...
These matrix-rank based metrics mostly show where the system responds to different kinds of input patterns, but provide limited additional information and are relatively expensive to compute.
In contrast, the matrix-rank based metrics require as many input sequences as there are readout nodes, with each input sequence consisting of a certain number of input values.
