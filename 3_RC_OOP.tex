...
To more clearly illustrate this phenomenon,~\cref{fig:3:Clocking_massive_seeded} shows a far larger ASI which was initialised in one ground state, apart from one ``seed'' magnet at the centre which is of the opposite domain type as compared to all other magnets.
...
Recall that the maximum value of $K$, $G$ and $C$ is the number of \link{linear readout layer}{readout nodes}; here, a $20 \times 20$ ASI with $10 \times 10$ readout nodes --- each representing the average domain type of a $2 \times 2$ magnet cluster --- yields a maximum value of 100 for all three metrics. \\\par
...
While this shows that defects improve the \xref{memory capacity} of the system, it does not mean a defect rate of \SI{5}{\percent} would be optimal.
