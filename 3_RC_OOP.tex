...
% TODO: make sure this intro is general for both transformation and prediction
...
Since this system can respond to analog input signals $s: \mathbb{R} \rightarrow [0, 1]; t \mapsto s(t)$, an alluring test to evaluate the reservoir computing capability of this system is to have it perform a signal transformation or prediction task~\cite{gartside2022reconfigurable,AdaptiveProgrammableRC,RC_NNN,TaskAdaptivePRC}. % REF: AvalanchesChaosNNN also does signal transformation, but this is more of a sidenote.
%A nonlinear waveform transformation (regression) task (24) was performed.  \par
This task primarily relies on the non-linearity and high dimensionality of a reservoir, rather than its fading memory capacity~\cite{FewMoleculeReservoir}.
This task requires non-linearity and high dimensionality in the reservoir, by which the basic computational capability of the reservoir can be evaluated~\cite{FewMoleculeReservoir}. \par
...
The goal is to estimate a ``target'' function $y(t) \in [0,1]$ as well as possible based on the reservoir's response $\vc{r}(t)$.
...
In simulation, the continuous input signal $s(t)$ can not be captured at infinite resolution, since \hotspice assumes all aspects of the system remain static until the next switch occurs.
...
To approximate a continuous input in simulation, the input signal must be sampled at sufficiently short time intervals while imposing a maximum switching time $t_\mathrm{max}$ equal to this interval.
To adequately sample an input signal of frequency $f$, we impose $t_\mathrm{max} = 1/20f$.
This way, the external field $\vc{B}_\mathrm{ext}$ closely resembles the true input signal, as it is updated after each iteration of the first-switch method (\cref{alg:2:FirstSwitchSingle}), regardless of whether a magnet has switched within the short interval $t_\mathrm{max}$. \\\par

By recording the reservoir state $\vc{r}(t_i)$ and the target output $y(t_i)$ at discrete intervals $t_i = i t_\mathrm{max}$ (with $i = 1,\dots,n$), the perceptron can be trained on several periods of the input signal via ordinary least-squares (OLS) regression\footnote{
...
For a consistent comparison, the training of the perceptron (which is a deterministic process) is always performed on the first 12 periods of the input signal\footnote{
	In this section on signal transformation, periodic signals will be considered, so one period should suffice.
	However, since the reservoir uses thermal relaxation, which is a stochastic process, multiple periods are used to make the estimator more robust to such noise.
}.
After training, the performance of a reservoir will be evaluated based on its prediction on 8 more periods of the input signal, measured by the mean-squared error of the ASI prediction $\hat{y}(t)$ with respect to the target function $y(t)$ at the sampled times $t_i$:
\begin{equation}
...
To evaluate the reservoir performance, two estimators will be compared: one that is trained on the reservoir output $\vc{r}(t)$ as described before, and one that is trained only on the raw 1D input signal $s(t)$ (augmented with a constant to improve the fit)\footnote{
	...
	% Adding such a constant has the effect of offsetting the ``raw input prediction'' vertically, allowing it to more closely approximate the desired output by an appropriate offset and positive or negative scaling, rather than only the positive scaling possible without such a constant (because both input and output are assumed to be $\in [0,1]$ throughout this section.
...
% Why do signal transformation? (scalar, nonlinear, memory...)
...
A signal transformation task primarily tests the non-linearity of the reservoir~\cite{gartside2022reconfigurable,TaskAdaptivePRC}.
