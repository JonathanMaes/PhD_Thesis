...
For deterministic switching --- that is, to avoid region~$\mathrm{I}$ --- both the anisotropy $\EEA$ and magnetostatic coupling $\EMC$ must be sufficiently high, say $\gg 40 \kBT$.
...
Since this clocking protocol relies on deterministic switching, the duration over which the sub-steps are applied is irrelevant\footnote{
	As time is irrelevant, all simulations use a default duration of \qty{1}{\second} per input cycle.
}. \par
