...
Where in-plane (IP) magnets mostly rely on \xref{shape anisotropy} to create an \xref{easy axis}, out-of-plane (OOP) magnets are realised using \xref{interfacial anisotropy} between the \link{ferromagnetism}{ferromagnetic material} and the substrate.
...
}, this can be realised by adding more electrodes to the \xref{Hall bar} (i.e., to the conductive layer underneath the ASI through which the AHE is measured).
...
\begin{enumerate}
	\item \textbf{Perpendicular magnetic anisotropy}\indexlabel[nolabel]{perpendicular magnetic anisotropy} constitutes the largest contribution to the total OOP anisotropy.
...
The \xref{N\'eel-Arrhenius} equation~\eqref{eq:2:Néel} translates this to an exponentially increasing switching time. \\\par
...
\paragraph{Distinguishing between the degenerate ground states in OOP ASI}
...
Three \xref{matrix rank-based metrics} can be calculated (\cref{sec:1:RC_metrics_KQ}): the \xref{kernel-quality} $K$, \xref{generalisation-capability} $G$ and \xref{computing capacity} $C=K-G$.
...
\paragraph{Linear estimator-based metrics}
...
In comparison, \xref{linear estimator-based metrics} such as \xref{non-linearity} and \xref{memory capacity} can be determined using only several hundred random input values, making them considerably more efficient to compute.
...
Therefore, the \xref{parity check} metric is also shown, as it was specifically designed for binary input. % See ppt 20230207
