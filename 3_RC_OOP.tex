...
The main factor that motivated our research of OOP ASI for \xref{reservoir computing} (RC) was that this kind of system allows very efficient and relatively simple input and readout methods to be used.
...
\subsection{Structure of out-of-plane nanomagnets}\label{sec:3:OOP_nanomagnet_PMA}\indexlabel[nolabel]{single-domain nanomagnet} % Co/Pt multilayer, energy contributions
Where in-plane (IP) magnets mostly rely on \xref{shape anisotropy} to create an \xref{easy axis}, out-of-plane (OOP) magnets are realised using \indexlabel[nolabel]{interfacial anisotropy}\xref{interfacial anisotropy} between the \link{ferromagnetism}{ferromagnetic material} and the substrate.
...
This \idx{perpendicular magnetic anisotropy} (PMA) acts in the immediate vicinity of the interface, where the \xref{magnetisation} then prefers to align perpendicular to said interface. \par
...
For example, the \xlabel{RKKY interaction}~\cite{RKKY_RK,RKKY_K,RKKY_Y} either promotes a parallel or antiparallel magnetisation of successive \ce{Co} layers, depending on the precise \ce{Pt} layer thickness.
Fortunately, the RKKY coupling is likely not dominant in a \ce{Co}-\ce{Pt} stack, as \ce{Pt} nearly satisfies the \xlabel{Stoner criterion}~\cite{PtMagneticOrder}: since the \ce{Pt} is combined with a ferromagnetic material (\ce{Co}), it is thought that this induces a ferromagnetic coupling between \ce{Co} layers regardless --- at least for reasonable \ce{Pt} thicknesses up to a few \SI{}{\nano\metre}~\cite{PerpendicularMagnetizationASI}.

\subsection{ASI ground state}\indexlabel[tosection]{checkerboard state}
...
For most OOP spin ices, the ground state exhibits some form of \xlabel{antiferromagnetic} (AFM) ordering due to the magnetostatic interaction, which always encourages opposite magnetisation between two neighbouring OOP magnets.
...
If \xlabel[nolabel]{frustration} is present, the global ground states of the system are hard to reach as not all local interactions can be satisfied simultaneously.
Otherwise, the symmetry of the system imposes two opposite ground states, which may give rise to \textit{domain walls}\indexlabel{domain wall} in the ASI --- not to be confused with domain walls within a magnet. \par % Locally, vertices where magnets meet will have a degenerate ground state, giving rise to mobile domains dictated by frustration.
...
We focus on this lattice because it is the simplest example of a non-frustrated OOP ASI.
...
The reason for this is that lattices with the same nearest-neighbour (NN) spacing can only differ in their \xref{magnetostatic interaction} between next-nearest neighbours and beyond, which decays rapidly with distance.
As a result, the ground state is largely determined by NN interactions, which prefer antiparallel alignment.
Therefore, OOP lattices in which every closed NN loop contains an even number of magnets (e.g., square/honeycomb lattices) are unlikely to exhibit frustration.
...
\paragraph{Input by \xlabel[nolabel]{spin--orbit torque}}
...
Indeed, SOT in the presence of broken symmetry results in a preferential magnetisation direction: in \hotspice, we will model this preference for one of the two magnetisation states using an additional \xref{external magnetic field} $B_z$.
Note that this field is not intended to model the \link{spin--orbit torque}{field-like SOT}~\cite{SOT_firstprinciplesCoPt}: it is merely an approximation to capture the overall effect of SOT within the confines of the two-state Ising-like model.
...
\paragraph{Readout by \xlabel[nolabel]{anomalous Hall effect}}
In our paper~\cite{KUR-24}, \textit{A. Kurenkov} experimentally demonstrated electrical readout of the average magnetisation $\llangle s \rrangle$ using the \xref{anomalous Hall effect}~\cite{AHE}.
...
}, this can be realised by adding more electrodes to the \indexlabel[nolabel]{Hall bar}\xref{Hall bar} (i.e., to the conductive layer underneath the ASI through which the AHE is measured).
...
In the context of \xref{reservoir computing}, such a readout approach with multiple electrodes can therefore be computationally equivalent to knowing the exact magnetic configuration of the system.
...
\begin{enumerate}
	...
	Its interfacial origin and characteristics have been discussed earlier in this chapter (see~\cref{sec:3:OOP_nanomagnet_PMA}). % Anisot E: \approx\SI{110}{\atto\joule}
	...
	This contribution was also discussed earlier, see~\cref{sec:2:shape_anisotropy}~and~\ref{sec:3:OOP_nanomagnet_PMA}. % Demag E: \approx\SI{-160}{\atto\joule}
	\item \textbf{Inter-layer \xlabel[nolabel]{magnetostatic interaction}}.
	...
	This contribution aids the PMA in stabilising the OOP anisotropy, counteracting the \xref{demagnetising field}, but is smaller in magnitude. % Cross MS E: \approx\SI{50}{\atto\joule}
	...
	All aforementioned energy contributions are internal to individual OOP magnets, making the \xref{magnetostatic interaction} the sole reason for the AFM ground state of OOP ASI. % E_MS: \lesssim\SI{11}{\atto\joule}
...
\end{enumerate}
For a system to exhibit spontaneous switching, the first three of these contributions --- which together constitute the \idx{net out-of-plane anisotropy} --- should mostly cancel out, to the level where their combined influence is of the same order of magnitude as the inter-magnet \xref{magnetostatic interaction}, which is usually at least an order of magnitude weaker than the other contributions separately.
...
They are placed in an ASI where the lateral spacing between magnets is $S_\mathrm{ASI}$ --- resulting in a lattice parameter $a=D_\mathrm{NM}+S_\mathrm{ASI}$ in a square lattice.
...
Note that the \xref{magnetostatic interactions} depend on the square of the ferromagnetic volume $V \propto n_{\ce{Co}} t_{\ce{Co}} D_\mathrm{NM}^2$. \\\par
...
\begin{itemize}
	...
	This upper bound was experimentally determined~\cite{KUR-24} and is imposed by \xref{shape anisotropy}.
	...
	The separation only affects the strength of the \xref{magnetostatic interaction} between neighbouring magnets, as approximated in \hotspice by \cref{eq:2:E_MS} and the \link{finite dipole model}{second-order correction} of \cref{eq:2:E_MS_order2}.
	...
	Note that the number of layers does not affect the balance between PMA and \xref{shape anisotropy}, as the number of interfaces per amount of magnetic material remains constant.
	However, since \link{magnetostatic interaction}{MS interactions} grow $\propto n_{\ce{Co}}^2$, the inter-layer coupling --- and with it the net OOP anisotropy --- becomes increasingly significant for a large number of layers.
	\item \textbf{Diameter} $\boldsymbol{D_\mathrm{NM}} \lesssim \SI{200}{\nano\metre}$. \newline
	Magnets wider than this limit were observed to take on multi-domain states~\cite{KUR-24}, as their \link{demagnetising field}{demagnetisation energy} becomes dominant due to its rapid $\propto D_\mathrm{NM}^4$ growth.
	Even though in-plane magnets without uniform \xref{magnetisation} have been used for computation~\cite{gartside2022reconfigurable}, we did not intend to use this in the OOP systems.
	Furthermore, the \hotspice simulator requires a \link{single-domain nanomagnet}{single-domain} magnetisation state for its Ising-like model to be applicable. %\par The diameter greatly affects the inter-magnet MS coupling.
	...
	Note that $t_{\ce{Pt}}$ must still be chosen appropriately to promote ferromagnetic coupling between layers while maintaining the \xref{interfacial anisotropy}, accounting for effects like \link{RKKY interaction}{RKKY coupling}.
\end{itemize}
...
\subsection{Relaxation characteristics}\label{sec:3:relaxation}
...
\begin{itemize}[leftmargin=4.1em]
	\item[$\boldsymbol{\EEA}$ ---] the energy barrier of a single non-interacting OOP magnet due to the \link{net out-of-plane anisotropy}{net OOP anisotropy}.
	This is the combined contribution of PMA, shape anisotropy and the inter-layer magnetostatic interaction.
	\item[$\boldsymbol{\EMC}$ ---] the \xlabel{magnetostatic coupling} energy between nearest neighbours.
	This is responsible for the relaxation to the ground state.
\end{itemize}
Due to the \xref{N\'eel-Arrhenius} switching law~\eqref{eq:2:Néel}, it is more appropriate to write dimensionless energies ($\EEA/\kBT$ and $\EMC/\kBT$), as it is only this ratio --- not the absolute value of the energies or temperature --- that controls the switching times. \par
In real ASI, manufacturing inevitably introduces imperfections.
Slight geometrical variations can result in a significant variance of the \link{net out-of-plane anisotropy}{net OOP anisotropy} between magnets~\cite{Budrikis2012,DisorderGroundStateASI}.
...
The \xref{effective energy barrier} $\EBeff$ of a magnet $i$ can then symbolically be written as
\begin{equation}
	\label{eq:3:OOP_relaxation_EBeff}
	\EBeff{}_{,i}(t) = \EEA + \sigma(\EEA) \sampi_i + \EMC \sum_j s_i(t) D_{ij} s_j(t)
\end{equation}
with $\sampi$ a random value from a standard normal distribution and $D_{ij}$ a factor proportional to the \xref{magnetostatic interaction} energy between magnets $i$ and $j$. \\\par

The relaxation process can be tracked by the normalised average magnetisation $\mavg = \abs{\llangle s \rrangle}$ and the \indexlabel{local antiferromagnetic parameter}local \xref{antiferromagnetic} parameter $\qNN = (1 - \langle s_i s_{i+1} \rangle)/2$.
...
Note that, while for a random state $\qNN \approx 0.5$, a value of $\qNN=0.5$ does not necessarily imply a random state. \newpage
...
Each panel shows the mean, standard deviation and 1st/99th percentile of $\mavg$ and $\qNN$ over 200 relaxations --- this variability is both due to the randomness of \xref{N\'eel-Arrhenius} switching and because each relaxation used unique randomly sampled $\EEA$ for individual magnets, in accordance with the $\sigma(\EEA)/\EEA = \SI{5}{\percent}$ variation.
While the mean relaxation profile is essentially unaffected by the number of magnets $N$ in the system, the variability between relaxations (indicated by the shaded regions in the figure) decreases for larger systems as $\approx 1/\sqrt{N}$, in accordance with the \xlabel{central limit theorem}.
Each relaxation consisted of at most $40N$ switches using the \xref{first-switch method}, as this is sufficient for the system to achieve equilibrium.
...
This \idx[tosection]{logarithmic relaxation} implies that the first switches occur on a vastly different timescale than the last.
...
The cause of this is the \xref{magnetostatic interaction}\footnote{
	...
	This is the opposite of \xref{logarithmic relaxation}; quantities like $\mavg$ would only show up as straight lines when using a linear time axis and logarithmic y-axis.
	% Exponential decay would be much easier to work with for temporal input, but having no MS coupling defeats the point of using an ASI for some non-linearity.
}.
...
Whichever magnet randomly switches next will therefore experience a slightly higher \xref{effective energy barrier} $\EBeff$ than before.
...
The \indexlabel[nolabel]{N\'eel-Arrhenius}\xref{N\'eel-Arrhenius} equation~\eqref{eq:2:Néel} translates this to an exponentially increasing switching time. \\\par


\xfig{3_RC_OOP/OOP_relaxation.pdf}{
	\label{fig:3:OOP_relaxation}
	Dependence of the ordering timescale on the energy balance between \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$ and nearest-neighbour \xref{magnetostatic coupling} $\EMC$ in an $11 \times 11$ OOP square-lattice ASI.
	This system size was chosen because it matched the size of our fabricated systems discussed later.
	The ASI is initialised in a uniform magnetic state with all spins pointing `up' and is released at $t = 0$, after which the ASI relaxes to lower energies.
...
Following the previous discussion, we now understand that high \link{magnetostatic coupling}{coupling} results in a greater decrease of $\EBeff$ per switch, and consequently a larger exponential increase in the switching time.
Meanwhile, the \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$ only affects the timescale of the relaxation --- it does not affect the slope, but simply shifts the relaxation curves along the (logarithmic) time axis.
...
\subsubsection{Two-stage relaxation}\indexlabel[tosection]{relaxation stages}
While $\mavg$ follows a near-perfect \xref{logarithmic relaxation}, $\mavg \approx 0$ is reached earlier than the perfect checkerboard ordering $\qNN = 1$ --- most strikingly in panels 6 and 9 of~\cref{fig:3:OOP_relaxation}.
...
Instead, expanding domains are likely to be separated by \xref{domain walls}, as seen in panels 3 and 4 which have just barely reached stage~$\circled{2}$.
From this point on, a further increase in $\qNN$ can only\footnote{
	If the \xref{magnetostatic coupling} $\EMC$ is insufficient, a perfect checkerboard ordering will not be achieved and $\qNN$ may not increase much further --- in panel 7 of~\cref{fig:3:OOP_relaxation}, $\qNN$ settles around $\approx 0.8$.
} be achieved by switching magnets that are part of a domain wall.
However, even though domain wall magnets are the least stable magnets in the system, they still typically have 3 stabilising NN, resulting in a significant \xref{effective energy barrier} that depends on both $\EEA$ and $\EMC$.
...
This is because $\EMC$ is the only factor responsible for order in the system: low \link{magnetostatic coupling}{coupling} makes magnets less inclined to reside in the ground state, giving the opportunity for multiple magnets to be in an unstable state at once and drive $\mavg$ further from 0.
...
A stronger \link{magnetostatic coupling}{coupling} $\EMC$ causes faster decay of $\mavg \rightarrow 0$: in panels 4--6 of~\cref{fig:3:OOP_relaxation}, the time to reach $\mavg \sim 0$ decreases from $\approx \SIrange{e3}{10}{\second}$.
However, strong coupling does not necessarily result in faster ordering $\qNN \rightarrow 1$ --- as panels 8 and 9 clearly illustrate --- because \xref{domain wall} magnets typically have 3 oppositely magnetised nearest neighbours, resulting in a net stabilising effect proportional to $\EMC$ (and $\EEA$).
...
Systems in this region exhibit local domains, but their \xref{domain walls} remain immobile.
...
\xfig{3_RC_OOP/OOP_relaxation_continuous.pdf}{
	...
	Phase diagrams at $t=\SI{e4}{\second}$, of the normalised average magnetisation $\mavg$ and the local antiferromagnetic parameter $\qNN$, as a function of \xref{net out-of-plane anisotropy} $\EEA$ and nearest-neighbour magnetostatic coupling $\EMC$ for an $11 \times 11$ lattice.
...
Reducing $\EMC$ too much puts the system in region~$\mathrm{V}$ (panel 7), where the system is \indexlabel{superparamagnetism}superparamagnetic at this timescale.
For a magnet to have a mean switching time $t = \SI{e4}{\second}$, the \xref{N\'eel-Arrhenius} switching law states that it must have an anisotropy $\EEA/\kBT = \ln(t \nu_0) \approx 32$: this is the reason why region~$\mathrm{V}$ extends upwards to $\EEA \lesssim 32 \kBT$.
The main region of interest --- region~$\mathrm{IV}$ with $\qNN \approx 1$ --- is therefore constrained from the left by the superparamagnetic regime, from the top by the transient/frozen regime, and from the right by states in which domain walls are immobile at the observed timescale. \par
...
However, due to the exponential character of the \xref{N\'eel-Arrhenius} law, this shift will be minimal and region~$\mathrm{IV}$ will not grow by much more than $\approx 10 \kBT$ for any reasonable timescale, as compared to~\cref{fig:3:OOP_relaxation_continuous}~\cite{KUR-24}.
...
\subsubsection{Exchange-coupled system}\indexlabel[nolabel]{exchange coupling}
The presence of \xref{exchange coupling} in a system affects the relaxation behaviour, resulting in deviations from the usual \link{logarithmic relaxation}{logarithmic ordering} during stage $\circled{1}$.
This is depicted in~\cref{fig:3:OOP_relaxation_exchange} for a negative (AFM) exchange coupling $J=-8\kBT$ and a \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA = 40\kBT$. \par
Since a negative \xref{exchange coupling} further stabilises the checkerboard state, the relaxation generally occurs faster than in a system without exchange coupling (compare, for instance, panels 2--3 of \cref{fig:3:OOP_relaxation_exchange} with panels 1--2 of \cref{fig:3:OOP_relaxation}).
...
This is in stark contrast with the near-perfect \xref{logarithmic relaxation} previously observed in systems without exchange coupling.
The reason for this is that, contrary to the \link{magnetostatic interaction}{MS interaction}, the exchange coupling only depends on the local configuration of the nearest neighbours of each magnet.
...
\xfig{3_RC_OOP/OOP_relaxation_exchange.pdf}{
	...
	Dependence of the ordering timescale on the nearest-neighbour magnetostatic coupling $\EMC$ in an $11 \times 11$ exchange-coupled OOP square-lattice ASI with a \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA = 40\kBT$.
	A negative NN \xref{exchange coupling} $J=-8\kBT$ was used; a positive exchange would discourage the system from approaching the checkerboard ground state.
	The ASI is initialised in a uniform magnetic state with all spins pointing `up' and is released at $t = 0$, after which the ASI relaxes to lower energies.
	Blue traces show the absolute value of the normalised average magnetisation $\mavg$ while orange traces represent the local antiferromagnetic parameter $\qNN$.
	Each trace shows the mean (central line), standard deviation (central shaded area), and 1st/99th percentile (outer shaded area) of 200 repeated relaxations with randomly sampled $\EEA$ profiles.
	The hatched ($\times$) region highlights the frozen timescales, while the white and green shaded region highlight stage~$\circled{1}$~and~$\circled{2}$ of ordering, respectively.
...
The second stair (e.g., $t < \SI{e-2}{\second}$ in panel 1 of~\cref{fig:3:OOP_relaxation_exchange}) is caused by magnets at the edge of the ASI.
Their available energy levels are distinct from those of bulk magnets, since magnets at the edge only have 3 NN: for them, $E_\mathrm{exch} \in \{\pm 3J, \pm J\}$, as compared to $E_\mathrm{exch} \in \{\pm 4J, \pm 2J, 0\}$ for bulk magnets.
...
As the system size increases, the height of the second stair --- which is caused by the switching of magnets at the edge --- shrinks and becomes increasingly obscured by the surrounding stairs. \par
From then on, further relaxation towards the perfect checkerboard state can only be achieved in the same way as for systems without \xref{exchange coupling}: domain walls must perform a random walk to move out of the ASI.
...
When the absolute value of the exchange coupling is small, as in \cref{fig:3:OOP_relaxation_exchange} with $J=-8\kBT$, separate stairs will blend together to some extent as the tails of their \xlabel{exponential relaxation} overlap.
This effect becomes more pronounced in the presence of \xref{magnetostatic interactions}, which increase the variation in the interactions between magnets --- for instance leading to some magnets with 3 parallel NN randomly switching even before all magnets with 4 parallel NN have done so.
...
The state of an OOP ASI is defined by the \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$, \xref{magnetostatic coupling} $\EMC$ and elapsed time $t$.
...
For \xref{reservoir computing} purposes, a sufficiently low $\EEA$ and $\EMC$ are required --- but not too low, to maintain OOP anisotropy and a ground state checkerboard ordering, respectively.
...
Based only on the magnet geometry --- determined by the various geometrical parameters discussed earlier in~\cref{sec:3:OOP_geometry} --- it is hard to predict the \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$ and \xref{magnetostatic coupling} $\EMC$ of a manufactured ASI.
For typical nanomagnet dimensions (7 layers of $t_{\ce{Co}} \approx \SI{1.45}{\nano\metre}$ with $D_\mathrm{NM} \approx \SI{170}{\nano\metre}$), some of the aforementioned energy contributions are on the order of several $1000 \kBT$, as we discuss in more detail in~\ccite[Supp. 4]{KUR-24}. % --- especially PMA and shape anisotropy, which have opposite sign and therefore mostly cancel out.
Due to manufacturing defects, the discrepancy between theoretical estimates of these energy contributions and their real magnitudes can easily exceed $100 \kBT$. % Theoretically, $\EEA$ of an isolated magnet (as manufactured by Alex and observed to be part of a thermally relaxing OOP ASI) is $\approx\SI{-3.6}{\atto\joule} = \SI{-22.5}{\electronvolt} = -871\kBT$ which is far too large to get thermal switching. Furthermore, it is negative, implying an IP magnetisation. This discrepancy between Mathematica and reality could be due to non-coherent reversal, lowering the real energy barrier.
...
For this reason, \textit{A. Kurenkov} manufactured several $11 \times 11$ OOP square-lattice ASIs, initialised them in the `up' state by a \SI{0.9}{\tesla} OOP field and measured their state using \link{magnetic force microscopy}{MFM}\indexlabel[nolabel]{magnetic force microscopy} at different time intervals since initialisation.
...
Assuming a normal distribution\footnote{
	Since the random relaxation is governed by the \xref{N\'eel-Arrhenius} equation~\eqref{eq:2:Néel}, the distribution of quantities bounded in the range $[0,1]$ --- like $\mavg$ and $\qNN$ --- will be similar to a binomial distribution.
...
As \indexlabel{fit quality} $\eta$, we will use the product of these probability densities, multiplied over each experimental $\mavg$ and $\qNN$ at each measured time $t$.
...
We will also allow the \xref{exchange coupling} $J$ to be varied during fitting --- if the fit yields a significantly non-zero value of $J$, this can indicate that the system contains defects in the form of interconnected magnets. \par % \\\par
...
More extensive parameter searches may benefit from using other optimisation techniques like the more recently proposed Lipschitz Optimisation\phantomsection\label{LIPO} (LIPO) algorithm~\cite{LIPO,LIPO_dlib}.
...
Due to the \xref{logarithmic relaxation}, an accurate fit requires measurements separated by multiple orders of magnitude in time.
...
After verifying this, fitting could be performed using two measurements: one soon after initialisation ($\approx \SI{e3}{\second}$, since \link{magnetic force microscopy}{MFM} measurements are not instantaneous) and another after approximately 2 years $\approx \SI{7e7}{\second}$.
...
Note that more switches occur in closely-packed lattices (low $S_\mathrm{ASI}$) due to the stronger \xref{magnetostatic coupling} between magnets.
...
This figure shows the optimal fits through these points --- based on $\gtrsim 1000$ iterations of \xref{Bayesian optimisation} --- with the corresponding values of $\EEA$, $\EMC$ and $J$ listed in the top right corner of each panel.
...
For this reason, \cref{fig:3:MFM_fitquality} is also provided, which shows the \xref{fit quality} $\eta$ for all guesses made by the optimiser, as a function of these three parameters.
...
The reason for this is unclear; see Supp. 5 of our paper~\cite{KUR-24} for a further discussion of the $S_\mathrm{ASI} = \SI{40}{\nano\metre}$ and similarly inconclusive $\SI{35}{\nano\metre}$ samples. \\\par

The three other systems (20, 25 and \SI{30}{\nano\metre}) all end up with a similar estimate of the \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA \approx 60 \kBT$.
Such similarity is not unexpected since they were all deposited in the same manner, but these values are far lower than the analytically predicted $\EEA \gg 100 \kBT$, as calculated under the assumption of \link{coherent rotation}{coherent reversal} in a sinusoidal energy landscape formed by the various energy contributions outlined in~\cref{sec:3:E_contributions}~\cite[Supp. 4]{KUR-24}.
Hence, it is likely that \link{non-coherent magnetisation reversal}{non-coherent reversal} occurs, leading to a far smaller energy barrier than predicted. %This indicates that the anisotropy might be easier to control than initially thought, or is at least reproducible for identically processed samples.

\vspace{-1em}
\xfig{3_RC_OOP/MFM_grid.pdf}{
	\label{fig:3:MFM_grid}
	Determining system parameters (\link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$, \link{magnetostatic coupling}{NN MS coupling} $\EMC$ and \xref{exchange coupling} $J$) for lattices with different magnet separation $S_\mathrm{ASI}$.
	All magnets have the same geometry ($t_{\ce{Co}} = \SI{1.45}{\nano\metre}$ and $D_\mathrm{NM} = \SI{170}{\nano\metre}$) and were initialised in a uniform magnetic state by an \SI{0.9}{\tesla} OOP \link{external magnetic field}{magnetic field} at $t=0$.
	\textbf{(a)} \link{magnetic force microscopy}{MFM} images by \textit{A. Kurenkov}~\cite{KUR-24} of the lattices after they were kept at room temperature for $t \sim \SI{1000}{\second}$ and $t \sim \SI{7e7}{\second}$ (27 months).
	...
	The reported uncertainties represent the weighted standard deviations of the respective distributions in~\cref{fig:3:MFM_fitquality}.
	Each trace shows the average of 200 simulated relaxations (central line) and their standard deviation (shaded area).
}
\vspace{-0.5em}

The reason for allowing the optimiser to adjust the \xref{exchange coupling} $J$ is now also clear: for $S_\mathrm{ASI} = \SI{20}{\nano\metre}$, the optimal fit yields a clear non-zero $J = 6.5 \kBT$.
...
For systems with larger spacing, a near-zero \xlabel[nolabel]{exchange coupling} is found.
...
Such defects could explain why a better \xref{fit quality} $\eta$ could be achieved for $S_\mathrm{ASI} = \SI{25}{\nano\metre}$ and \SI{30}{\nano\metre} than for $S_\mathrm{ASI} = \SI{20}{\nano\metre}$.
...
\xfig{3_RC_OOP/MFM_fitquality.pdf}{
	...
	Each dot represents one guess by the \link{Bayesian optimisation}{Bayesian optimiser}.
	The \xref{fit quality} $\eta$ indicates how well the Monte Carlo simulation fits the experimental data at a given $\EEA$, $\EMC$ and $J$ value: brighter dots represent better guesses.
	The greyed-out area in the right panel represents \xref{antiferromagnetic} coupling ($J<0$), which is considered non-physical in this system.
}
\vspace{-1em}
...
For the interconnected system ($S_\mathrm{ASI} = \SI{20}{\nano\metre}$), the fitted \link{magnetostatic coupling}{MS coupling} $\EMC$ is also much larger, because it is hard to distinguish between exchange coupling and MS coupling.
...
One cluster is concentrated near $J=0$ with high MS coupling, while a second cluster with slightly lower fit quality $\eta$ exists at a non-physical\footnote{
	Given our interpretation of the \xref{exchange coupling} as being an indicator of defects in the form of interconnected magnets, we consider a negative coupling to be non-physical as interconnections would promote parallel ordering ($J > 0$) through the \link{ferromagnetism}{ferromagnetic} exchange interaction.
...
While $\EEA$ is already far smaller than analytical models predict, further control of the total energy barrier is needed to achieve a system that spontaneously reaches the checkerboard ground state in a reasonable timeframe. \\\par
...
Fits using the exact barrier $\EBeff$ of~\cref{eq:2:EB_exact} were also attempted, but these yielded less conclusive results.
They still more or less agreed on the approximate values of $\EEA$, $\EMC$ and the requirement of a non-zero \xref{exchange coupling} for $S_\mathrm{ASI} = \SI{20}{\nano\metre}$.
...
Because of this, we may conclude that the exact barrier is less accurate than the \xref{mean-barrier model} in this case, possibly indicating that reversal occurs through a \link{non-coherent magnetisation reversal}{non-coherent} process.
This would also explain why $\EEA$ is on the order of $60 \kBT$, rather than the $\approx 1000 \kBT$ predicted by analytical calculations that also assume a \link{coherent rotation}{coherent reversal}~\cite[Supp.4]{KUR-24}.

\newpage
\section{Reservoir computing in thermally active out-of-plane ASI}
\subsection{Concept}\indexlabel[nolabel]{reservoir computing}
In order to use this thermally active OOP ASI as a \link{reservoir computing}{reservoir}, a method must be devised that generates a non-linear response with fading memory.
One option to achieve this, is to use the transient regime between the AFM ground state and the uniform state: the previously discussed \link{logarithmic relaxation}{spontaneous relaxation} makes the system evolve towards the ground state, while an external stimulus can instead push the system towards the uniform state.
...
The relaxation towards the ground state, on the other hand, is governed by parameters intrinsic to the ASI itself, like the \link{net out-of-plane anisotropy}{OOP anisotropy} $\EEA$ and \link{magnetostatic coupling}{MS coupling} $\EMC$.

\paragraph{Input}
Practically, such an input method can be realised through \xref{spin--orbit torque} (SOT) in the presence of a symmetry-breaking in-plane field, as discussed earlier in~\cref{sec:1:ASI_IO}~and~\ref{sec:3:IO_exp_to_sim}.
...
Here, the OOP \link{external magnetic field}{magnetic field} $B_z(t)$ that models the SOT in \hotspice was scaled between $B_0$ (minimum) and $B_1$ (maximum).
...
\link{reservoir computing}{Reservoir computing} benefits from having multiple simultaneous readout values, as this increases the available information for the \xref{linear readout layer} to train on.
However, in a normal OOP square-lattice ASI, the relaxation dynamics do not differ much between various areas throughout the ASI --- only the magnets near the edge respond somewhat differently than those in the bulk of the system, as they experience a different total \link{magnetostatic interaction}{MS interaction}.
Without more significant spatial differences, multiple readouts will provide no large benefit. \par
...
Here, this will be accomplished by gradually increasing the \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$ and \xref{magnetic moment} $\mu$ of magnets along the $x$-axis --- as if the magnets were larger on one side of the lattice than on the other --- causing different sides of the lattice to relax to the ground state over different timescales.
...
\subsection{Influence of input on dynamics}\indexlabel[tosection]{leaky integrator}
...
Hence, a low \link{magnetostatic coupling}{NN MS coupling} $\EMC=2.5\kBT$ will be used as a baseline, as this is the lowest value that fully avoids the \link{superparamagnetism}{superparamagnetic} regime, though its relaxation already spans $>5$ orders of magnitude.
...
While a further reduction of $\EEA$ would accelerate the relaxation, accurate control over the anisotropy is challenging to achieve in practice.
Therefore, all subsequent simulations of \xref{reservoir computing} in thermally active ASI will use the standard values $\EEA=20\kBT$ and $\EMC=2.5\kBT$ (unless otherwise specified) while varying other parameters to determine the optimal input conditions for such a system. \par
...
More specifically, the upwards-pointing \link{external magnetic field}{external field} $B_z$ that provides the input can take any random magnitude between 0 and $B$, with each value being maintained for a period determined by the input frequency $f$. \par % \\\par

\vspace{-1.5em}
\xfig{3_RC_OOP/OOP_relaxation_input.pdf}{
	\label{fig:3:OOP_relaxation_input}
	Dependence of system dynamics on the input frequency $f$ and magnitude $B$ in an $11 \times 11$ \link{exchange coupling}{exchange-coupled} OOP square-lattice ASI.
	At any time, the normalised input magnitude is indicated by the grey bars.
	The nearest-neighbour \xref{magnetostatic coupling} is $\EMC=2.5\kBT$ and the \link{net out-of-plane anisotropy}{net OOP anisotropy} is $\EEA = 40\kBT$.
	The ASI is initialised in an \link{antiferromagnetic}{AFM} ground state.
	A uniform \link{external magnetic field}{external field} of arbitrary strength is then applied for a duration determined by the input frequency $f$.
	Blue traces show the absolute value of the normalised average magnetisation $\mavg$ while orange traces represent the local antiferromagnetic parameter $\qNN$.
	Each trace shows the mean (central line), standard deviation (central shaded area), and 1st/99th percentile (outer shaded area) of 200 repeated relaxations with randomly sampled $\EEA$ profiles.
}
\vspace{-1em}

Larger input values $v$ drive the system further from its ground state, while smaller values applied for a sufficiently long time allow a gradual \link{logarithmic relaxation}{relaxation} back towards $\mavg \approx 0$.
...
Optimal \link{reservoir computing}{reservoir} performance can be expected when the system never saturates, yet still uses as much of the available $\mavg$-range as possible to minimise the relative impact of the stochasticity intrinsic to the relaxation process. \par
...
Therefore, the stochastic nature of the relaxation can, in some cases, prevent the \xref{linear readout layer} from distinguishing between closely spaced input values. \par
...
The transient dynamics of thermally active OOP ASI, as were illustrated in~\cref{fig:3:OOP_relaxation_input}, can be harnessed to perform \xref{reservoir computing}.
While we first attempted to evaluate reservoir performance using \xref{linear estimator-based metrics}, these proved largely uninformative as the \xref{logarithmic relaxation} results in a high non-linearity and low memory capacity overall.
However, since this system can respond to analog input signals $s: \mathbb{R} \rightarrow [0, 1]; t \mapsto s(t)$, a signal transformation or prediction task forms an alluring test to nonetheless demonstrate and evaluate its reservoir computing capability~\cite{gartside2022reconfigurable,NeuromorphicFewShot,RC_NNN,TaskAdaptivePRC}. \par % REF: AvalanchesChaosNNN also does signal transformation, but it is more of a sidenote there.
Applying a signal $s(t)$ to the reservoir, here in the form of an \link{external magnetic field}{external field} $B_z(t)$, yields a response $\vc{x}(t) \in \mathbb{R}^p$ from the $p$ readout nodes the reservoir provides.
The goal is to estimate a ``target'' function $y(t) \in [0,1]$ as accurately as possible based on the reservoir's response $\vc{x}(t)$.
As explained in~\cref{sec:1:RC}, the reservoir computing paradigm includes a \xref{linear readout layer} that converts the response $\vc{x}(t)$ into an output $\hat{y}(t) = \vc{x}^\mathrm{T}(t) \cdot \vc{w}$.
...
This can be done by imposing a maximum \link{switching rate}{switching time} $t_\mathrm{max} = 1/20f$ and updating the \link{external magnetic field}{external field} $\vc{B}_\mathrm{ext}$ after each iteration of the \xref{first-switch method} (\cref{alg:2:FirstSwitchSingle}) --- regardless of whether a magnet has switched within the short interval $t_\mathrm{max}$ --- resulting in adequate sampling of the true input signal. \\\par

By recording the reservoir state $\vc{x}(t_i)$ and the target output $y(t_i)$ at discrete intervals $t_i = i t_\mathrm{max}$ (with $i = 1,\dots,n$), the \link{linear readout layer}{readout layer} can be trained on several periods of the input signal via \xref{ordinary least-squares} (OLS) regression\footnote{
...
The performance is then quantified by the \xlabel{mean squared error} ($\MSE$) of the ASI prediction $\hat{y}(t)$ with respect to the target function $y(t)$ at the sampled times $t_i$:
\begin{equation}
	\label{eq:3:MSE}
...
To interpret this value, the mean squared error of two estimators will be compared: one that is trained on the \link{reservoir computing}{reservoir} output $\vc{x}(t)$ as described before, and one that is trained only on the raw 1D input signal $s(t)$ (augmented with a constant\footnote{
	Training an \link{ordinary least-squares}{OLS estimator} on a 1D signal is equivalent to an optimal scaling to fit the signal to the target function.
	Including a constant factor enables a vertical offset of the input signal for a better fit (yielding up to $10 \times$ lower baseline MSE).
	Some studies do not use such a constant factor~\cite{gartside2022reconfigurable}, thereby lowering the quality of the raw-input baseline and elevating the relative performance of the \link{reservoir computing}{reservoir}.
...
Signal transformation primarily relies on the \xref{non-linearity} and \xref{high dimensionality} of a \link{reservoir computing}{reservoir}, rather than its \xref{fading memory} capacity, though the balance between these can differ depending on the specific transformation~\cite{FewMoleculeReservoir,gartside2022reconfigurable,TaskAdaptivePRC}.
...
Hence, the reservoir can not only rely on the input signal at the current time, but must also retain information on, e.g., the derivative of the input signal, in order for the \link{linear readout layer}{linear estimator} to be able to accurately predict the sawtooth during both the increasing and decreasing parts of the sine wave.
The thermally active OOP ASI reservoir should readily have such information available, since its operation resembles that of a \xref{leaky integrator}.
...
For a consistent comparison, the \xref{linear readout layer} will always be trained on the first 12 periods of the input signal, after which the \link{reservoir computing}{reservoir} performance is evaluated on 8 more periods.
...
Since both sides appear very similar, we may conclude that the \xref{linear readout layer} is not \xref{overfitting} on the limited data (12 periods) it has available. \par
...
The reason for this is twofold: at low frequencies, the system has sufficient time to reach the end of stage $\circled{1}$ of \link{logarithmic relaxation}{relaxation} ($\mavg \approx 0$), while a relatively strong input easily saturates the system into a uniform magnetic state. % when the sine wave exceeds a certain threshold, as the strongest magnetic fields are applied around that time.
...
\makeshiftfig{
	...
	\vspace{-2em} % Looks more natural for this figure
}{
	\label{fig:3:Transformation_freq-magn_baseline}
	Monte Carlo simulations of a sine-to-sawtooth signal transformation performed by a thermally active $11 \times 11$ OOP square-lattice ASI \link{reservoir computing}{reservoir}.
	The top panel shows the inverse \xref{mean squared error} $1/\MSE$ as a function of the input \link{external magnetic field}{magnetic field} frequency $f$ and amplitude $B_1$, with constant $B_0 = \SI{-0.35}{\milli\tesla}$ (as derived from~\cref{fig:3:Transformation_magn-magnmin}), averaged over 5 runs with random $\sigma(\EEA)/\EEA=\SI{5}{\percent}$ defects.
	Higher (lower) values in yellow (purple) indicate better (worse) performance.
	The black contours indicate $1/\MSE \sim 30$, encompassing regions where the reservoir outperforms a linear transformation of the original input signal.
	Roman numerals in the top panel indicate the points corresponding to the bottom panels, which show a temporal view of the transformation.
...
Such a broad range of usable frequencies was to be expected considering the \link{logarithmic relaxation}{logarithmic} nature of the relaxation dynamics.
...
The minimal \link{external magnetic field}{field} magnitude $B_0$ for this system without \link{property gradient}{gradient} was set to \SI{-0.35}{\milli\tesla}.
...
The results of this parameter sweep are presented in~\cref{fig:3:Transformation_magn-magnmin}, from which the optimal value of $B_0=\SI{-0.35}{\milli\tesla}$ was deduced. \par
...
While the optimal input frequency often remains of a similar order of magnitude regardless of the transformation --- since the frequency is mostly dictated by the relaxation dynamics --- the optimal range of input \link{external magnetic field}{field} magnitudes is highly dependent on the desired transformation.
...
\sidefig[0.53]{3_RC_OOP/Thermally active/Transformation/magn-magnmin/averaged.pdf}{
	...
	Inverse \xref{mean squared error} $1/\MSE$ of a \SI{100}{\hertz} sine-to-sawtooth signal transformation using a thermally active $11 \times 11$ OOP square-lattice ASI, as a function of the input \link{external magnetic field}{magnetic field} range $B_0 \rightarrow B_1$.
	Averaged results are shown over 5 runs with random $\sigma(\EEA)/\EEA=\SI{5}{\percent}$ defects.
	Higher (lower) values in yellow (purple) indicate better (worse) performance.
	The black contours indicate $1/\MSE \sim 30$, encompassing regions where the \link{reservoir computing}{reservoir} outperforms a linear transformation of the original input signal.
}
\vspace{-1.5em}

\paragraph{Gradient}
The previous discussion on input frequencies and magnitudes used a system without a \xref{property gradient}.
By making the magnets on one side of the system bigger than on the other side, the \link{logarithmic relaxation}{relaxation timescales} can be spread out throughout the system, introducing more variation between the single-column readout nodes.
Quantitatively, a gradient $\Gamma$ means that the \link{net out-of-plane anisotropy}{net OOP anisotropy} $\EEA$ and \xref{magnetic moment} $\mu$ on the right (left) side of the lattice are a factor $\Gamma$ higher (lower) than their average value throughout the ASI.
The varying magnetic moment $\mu$ also affects the \link{magnetostatic coupling}{NN MS coupling}: the previously stated constant value of $\EMC = 2.5 \kBT$ now corresponds to the MS coupling between NN if their moment $\mu$ would be equal to the average magnetic moment $\llangle \mu \rrangle$ throughout the ASI.
...
\sidefigs[0.53]{3_RC_OOP/Thermally active/Transformation/grad-magn/freq1/averaged.pdf}{3_RC_OOP/Thermally active/Transformation/grad-magn/freq1/details.pdf}{
	...
	Impact of the \xref{property gradient} $\Gamma$ on the performance of a \SI{1}{\hertz} sine-to-sawtooth signal transformation by an $11 \times 11$ OOP square-lattice ASI.
	The top panel shows the inverse \xref{mean squared error} $1/\MSE$ as a function of the gradient $\Gamma$ and amplitude $B_1$, with constant $B_0 = \SI{0}{\milli\tesla}$, averaged over 5 runs with random $\sigma(\EEA)/\EEA=\SI{5}{\percent}$ defects.
	Higher (lower) values in yellow (purple) indicate better (worse) performance.
	The black contour encompasses $1/\MSE \gtrsim 30$, i.e., regions where the reservoir outperforms a linear transformation of the input signal.
	The bottom panel provides a temporal view of the transformation for $\Gamma = \SI{52}{\percent}$ and $B_1=\SI{0.45}{\milli\tesla}$, showing the input (grey dotted sine), target (black dashed sawtooth) and the prediction with (blue) and without (red) reservoir.
}
\vspace{-0.5em}

The impact of the \link{property gradient}{gradient} is illustrated in~\cref{fig:3:Transformation_grad-magn_freq1} as a function of the input magnitude $B_1$\footnote{
	$B_0 = \SI{0}{\milli\tesla}$ was chosen to show it can be deduced from the figure by a constant very low $1/\MSE$ at $B_1 = B_0$.
}.
...
For even stronger gradients, the performance drops again, as the range of \link{net out-of-plane anisotropy}{anisotropies} $\EEA$ in the system becomes too wide for an input signal to effectively induce dynamics throughout the whole system.
It can also be noted that the range of suitable input magnitudes $B_1$ expands as the gradient becomes more severe.
The reason for this is that the range of \xref{coercive fields} of individual magnets in the system gets wider due to the property gradient.
...
The \xlabel[nolabel]{central limit theorem} states that the standard deviation of a mean scales as $1/\sqrt{N}$, so the value of $\mavg$ during relaxation will be more consistent in larger systems.
...
However, as $p$ increases, the danger of \xlabel{overfitting} increases as well~\cite{DeepRC_IonGating_Overfitting,lukovsevivcius2009reservoir}.
Overfitting is a general \xref{machine learning} phenomenon that occurs when a model has too many trainable weights $\vc{w}$ for the limited data it has been given.
...
This is a clear indication of \xref{overfitting}. \par
...
Contrary to the single-column readout, the performance is now equal between the training and test sets, reaching a maximum of $1/\MSE \approx 60$ for $N_x=N_y=30$, beyond which enlarging the system no longer significantly affects the performance. \par
...
\makeshiftfig{
	...
	\vspace{-1em} % Looks more natural for this figure
}{
	\label{fig:3:Transformation_size-magn}
	Impact of system size on a sine-to-sawtooth signal transformation by an OOP ASI \link{reservoir computing}{reservoir} as measured by the inverse \xref{mean squared error} $1/\MSE$ as a function of the input \link{external magnetic field}{magnetic field} magnitude $B_1$, averaged over 5 runs with random $\sigma(\EEA)/\EEA=\SI{5}{\percent}$ defects.
	The input frequency and minimal magnitude were kept constant at $f=\SI{800}{\hertz}$ and $B_0=\SI{0}{\milli\tesla}$, respectively.
	Higher (lower) values in yellow (purple) indicate better (worse) performance.
	The black contours indicate $1/\MSE \sim 30$, encompassing regions where the reservoir outperforms a linear transformation of the original input signal.
	In panel \textbf{(a)}, each column of the ASI constitutes an individual \link{linear readout layer}{readout node}, while in panel \textbf{(b)} the number of readout nodes was kept constant at 11 by averaging multiple columns together.
...
Only the weights $\vc{w}$ of the \link{linear readout layer}{readout layer} have to be adjusted to reflect this new target.
...
\sidefigs[0.53]{3_RC_OOP/Thermally active/Transformation/freq-magn/target_SINESQ/Sweep20231023131601.pdf}{3_RC_OOP/Thermally active/Transformation/freq-magn/target_SINESQ/details.pdf}{
	...
	Transformation of a sine wave input signal into a double-period sine wave by an $11 \times 11$ OOP square-lattice ASI \link{reservoir computing}{reservoir}.
	The top panel shows the inverse \xref{mean squared error} $1/\MSE$ as a function of the input frequency and amplitude $B_1$, with constant $B_0 = \SI{0}{\milli\tesla}$.
	Higher (lower) values in yellow (purple) indicate better (worse) performance.
	The black contours indicate $1/\MSE \sim 8$, encompassing regions where the reservoir outperforms a linear transformation of the original input signal.
	The panel at the bottom provides a temporal view of the transformation for $f=\SI{0.5}{\hertz}$ and $B_1=\SI{0.25}{\milli\tesla}$, showing the input sine-wave signal (grey dotted curve) and target double-frequency sine (black dashed line) alongside the prediction with (blue) and without (red) the reservoir.
}
\vspace{-1em}

For this ASI \link{reservoir computing}{reservoir} to accurately produce a double-frequency sine, the input frequency must not exceed a threshold frequency $f_\mathrm{c} \approx \SI{10}{\hertz}$.
...
It is no coincidence that this value is similar to the timescale at which $\mavg \approx 0$ in panel 8 of~\cref{fig:3:OOP_relaxation}, which shows the \link{logarithmic relaxation}{relaxation} in the absence of external stimuli for this system.
...
Such a prediction task benefits from the \xref{fading memory} property of a \link{reservoir computing}{reservoir} to better infer the future evolution of the signal, while it relies less on a reservoir's \xref{non-linearity}~\cite{TaskAdaptivePRC,NeuromorphicFewShot}.
The particular signal we will be attempting to predict with the thermally active OOP ASI is the chaotic \xlabel[tosection]{Mackey-Glass oscillator} $\mathrm{MG}(t)$~\cite{MackeyGlass}, as this is a popular benchmark in literature~\cite{RotatingNeuronsRC,NeuromorphicFewShot,TaskAdaptivePRC,Moon_2021,JaegerHaasWireless,ArchitecturalMarkovianESN}.
...
The \xref{linear readout layer} is then trained over a timespan of $60\tau$, after which the test set spans $40\tau$. \par
...
For the systems with a gradient, $\Gamma$ was also optimised during the Bayesian optimisation procedure, typically yielding $\Gamma \approx \SIrange{25}{30}{\percent}$, though smaller gradients also perform decently. \\\par
...
The performance at $h=0$ presents the limit of what the thermally active ASI \link{reservoir computing}{reservoir} can achieve, due to the stochastic nature of the \link{logarithmic relaxation}{relaxation process} it employs.
Unsurprisingly, this limit improves by adding a \link{property gradient}{gradient} and increasing the system size. \par
The $11 \times 11$ ASI reservoir without gradient (blue) only outperforms a linear scaling of the input (red) for $h > 0.4$.
Including a \link{property gradient}{gradient} and enlarging the system improves the $\MSE$, though the gradient appears more impactful than the subsequent enlargement.
...
The smaller ASI without gradient struggles to reproduce the peaks of the target waveform, as its magnets only respond to a narrow \link{external magnetic field}{field} range.
On the other hand, the larger ASI with an appreciable gradient $\Gamma \approx \SI{25}{\percent}$ contains magnets with various \xref{coercive fields}, enabling it to capture both the peaks and valleys, as these are likely to respectively coincide with low and high input fields.
...
\xfig{3_RC_OOP/Thermally active/Prediction/MG.pdf}{
	...
	Performance of a thermally active OOP square-lattice ASI on a prediction task for the \xref{Mackey-Glass oscillator}, for different lattice sizes and \link{property gradient}{gradients} $\Gamma$.
	The number of \link{linear readout layer}{readout nodes} is fixed at $p=11$ for the $11 \times 11$ systems and $p=10$ for the $20 \times 20$ ASI.
	\textbf{(a)} \link{mean squared error}{Mean squared error} as a function of the offset $h$, for the reservoir prediction (blue) and a linear transformation of the original input signal (red).
...
Generally speaking, our thermally active OOP ASI achieves an $\MSE$ of roughly 0.015 on both the sine-to-sawtooth transformation and the \link{Mackey-Glass oscillator}{Mackey-Glass} prediction task.
...
\newpage
\subsection{Summary}
By harnessing the spontaneous \link{logarithmic relaxation}{relaxation dynamics} of thermally active OOP ASI, this type of system can be used as a \link{reservoir computing}{reservoir} thanks to its gradual response to external stimuli.
...
The computational capability of this system can be further boosted by including a \xref{property gradient}.
...
Still, manufacturing thermally active ASI remains challenging, as this requires great control over the \link{net out-of-plane anisotropy}{effective OOP anisotropy}, which must not exceed several tens of $\kBT$.
...
\newpage
\section{Reservoir computing in non-volatile out-of-plane ASI}\indexlabel[nolabel]{reservoir computing}\label{sec:3:Nonvolatile}
...
When the \link{net out-of-plane anisotropy}{energy barrier} $\EEA$ of a magnet is significantly higher than the thermal energy $\kBT$, the \xref{N\'eel-Arrhenius} switching law implies that it will remain stable over any reasonable timescale in the absence of external perturbations.
For example, a magnet with $\EEA > 40 \kBT$ will most likely remain stable for multiple years.
Therefore, non-volatile ASI is easier to construct than thermally active ASI, as great control over the \link{net out-of-plane anisotropy}{perpendicular anisotropy} is not required --- the anisotropy just has to be sufficiently large.
Throughout this section, we will develop, examine and discuss a ``clocked'' input protocol for non-volatile ASI that exhibits promising \xref{memory capacity}.

\paragraph{Magnetostatic interaction}
Note that a ``non-volatile'' system is not the same as a non-interacting system.
While a low \xref{magnetostatic coupling} $\EMC \ll \EEA$ results in a frozen system, a sufficiently high coupling will still induce spontaneous ordering, albeit only locally as the high anisotropy renders \xref{domain walls} immobile.
Recall in this regard the five regions previously outlined in~\cref{fig:3:OOP_relaxation_continuous}.
Because of the high anisotropy $\EEA$, the \link{superparamagnetism}{superparamagnetic} region~$\mathrm{V}$ and perfectly ordered region~$\mathrm{IV}$ are not relevant.
Instead, non-volatile ASI only exhibits two regimes: one where the magnetostatic coupling is sufficiently strong to overcome the \link{net out-of-plane anisotropy}{energy barrier} $\EEA$, and another where it is not.
...
% Nomenclature: thermally active ASI has low energy barriers that allow the domain walls to move by thermal fluctuations, as all interactions are on the order of several $\kBT$. \par
...
While the thermally active ASI typically had a MS coupling $\EMC \lesssim 10\kBT$, as was determined from the \link{magnetic force microscopy}{MFM} images in~\cref{fig:3:MFM_grid}, such low coupling will be insufficient in non-volatile ASI.
...
Increasing the magnetostatic coupling beyond $10\kBT$ should not present a significant practical issue, as the inclusion of a separate \link{ferromagnetism}{ferromagnetic} layer above the magnets can significantly increase the MS coupling by focusing the stray fields onto nearby magnets~\cite[Supp. 10]{KUR-24}.

\paragraph{Uniform external field}
While it was possible to perform signal transformation with thermally active OOP ASI by using a uniform \link{external magnetic field}{external field}, this approach is no longer viable in non-volatile ASI.
It lacks the intrinsic \link{logarithmic relaxation}{relaxation dynamics} of thermally active ASI, making the temporal dimension largely irrelevant: a magnet will either switch within a reasonable timeframe, or it will not.
Furthermore, the magnets only respond within a narrow field range near the \xref{coercive field}, leading to a very steep \xref{hysteresis} loop.
...
To control the domain structure, we can take inspiration from the ``clocking'' protocol proposed by Jensen~\etal~\cite{clocking-protocol}, which can move \xref{domain walls} in IP pinwheel ASI in discrete steps~\cite{SnakesInThePlane}.
While our square-lattice OOP ASI is very different from the pinwheel lattice, both exhibit domains, though these are \link{superferromagnetism}{superferromagnetic} in pinwheel ASI rather than \xref{antiferromagnetic}.
...
To achieve \xlabel{stepwise domain wall motion} in any ASI, the input method must promote one domain type over the other(s), while avoiding \xlabel{avalanches} that would otherwise saturate the ASI into one of its global ground states. % in which case only a single domain would remain.
Indeed, systems with little disorder are prone to avalanches, since most of their domain wall magnets all experience a similar \xref{effective energy barrier} $\EBeff$, regardless of their location.
...
Therefore, Jensen~\etal~\cite{clocking-protocol} applied an \xref{external magnetic field} in a well-chosen direction to selectively affect only one of the two pinwheel sublattices at a time, and then changed the field direction to affect the other sublattice.
...
\begin{itemize}
	...
	In pinwheel ASI, a global in-plane \link{external magnetic field}{field} readily distinguishes between the four types of \link{superferromagnetism}{superferromagnetic} domains.
	\item At least two independently addressable sublattices should exist, to avoid \xref{avalanches}. \newline
...
\paragraph{Distinguishing between the degenerate ground states in OOP ASI}\indexlabel[nolabel]{antiferromagnetic}
Since OOP ASI always has a more or less \xref{antiferromagnetic} (AFM) ground state, a uniform \link{external magnetic field}{external field} can never be used for controlled domain wall motion since it does not distinguish between the doubly degenerate domains --- thereby violating the first requirement.
% If a uniform field were to be used, it would simply cause the magnetisation of low-$\EBeff$ magnets to align with it. Any subsequent input cycle of opposite sign would either be too weak to have any additional effect, or be strong enough for the previously switched magnets to switch back, after which the same process can repeat. Regardless, a uniform field will not obtain the much coveted fading memory, precluding its use for RC in this system.
%Hence, for an input protocol to induce desirable dynamics in non-volatile ASI, it must be specifically tailored to the lattice geometry. \par
...
Since it is rather impractical to apply local magnetic fields, a lithographic realisation of such an AFM stimulus could instead be based on \link{spin--orbit torque}{SOT} induced by diagonal current lines: the required checkerboard pattern is then achieved when successive diagonals carry oppositely oriented currents.
Care must be taken, however, that this input is compatible with the \link{anomalous Hall effect}{AHE} readout, which requires a full conducting underlayer rather than individual diagonal current lines.
...
Applying an AFM ``checkerboard'' field is, however, not yet sufficient to achieve controlled domain wall movement.
Since it applies a global stimulus, it still violates the second requirement of having at least two independently addressable sublattices, and therefore easily triggers \xref{avalanches} of switching magnets rather than providing stepwise domain wall motion.
...
This way, since only half of the magnets are stimulated simultaneously, the other half is prevented from switching at the same time --- provided their \link{net out-of-plane anisotropy}{anisotropy} $\EEA$ is sufficiently high compared to the \link{magnetostatic coupling}{NN MS coupling }$\EMC$.
The avalanches that plagued the global input strategies are thereby avoided. \par
For this to result in controlled \xref{domain wall} motion, the strength of the input is of great importance.
If it is too weak, the magnets will not respond.
If the input is too strong, though, all magnets will follow the \link{external magnetic field}{applied field} and a global ground state will be reached within a single input cycle nonetheless.
...
Hence, by combining an appropriate choice of input strength with the \link{AFM clocking protocol}{two-step protocol} that prevents half of the magnets from switching, it is possible to move domain walls by only a few lattice sites at once. % by deterministically switching only domain wall magnets towards the applied field.

\xfig{3_RC_OOP/Nonvolatile/Clocking_protocol.pdf}{
	\label{fig:3:Clocking_protocol}
	Illustration of the \xref{AFM clocking protocol} for controlled \xref{domain wall} movement in square-lattice OOP ASI.
	The symbols $\bigodot$ and $\bigotimes$ respectively indicate that the external stimulus is such, that the magnet at that location would prefer an `up' or `down' \xref{magnetisation}.
	This example shows a $5 \times 5$ lattice, but the concept is analogous for other lattice sizes.
	Each input cycle consists of two substeps that each affect a different set of magnets, arranged in a checkerboard pattern: as such, only half of the magnets are stimulated at once.
...
This clocking protocol, coupled with this definition of the input, readily provides basic \link{fading memory}{memory} in the system, as the previous few input bits become encoded in the size and location of the domains.
...
In the remainder of this section, we will illustrate the behaviour of the clocking protocol and show that it yields desirable dynamics for \xref{reservoir computing}.
Before we can do that, though, we still need an appropriate readout method.

\paragraph{Readout of domain states}
...
Recall that it is theoretically possible to determine the state of all magnets in a small ASI from \link{anomalous Hall effect}{AHE} measurements~\cite[Supp. 7]{KUR-24}.
...
To turn these individual magnetic states into a robust output vector, we will once again use averaging to avoid \xref{overfitting}.
Since the $x$- and $y$-axes are equivalent, this is done via ``\xlabel{squinting}'' --- effectively dividing the ASI into a coarser square lattice and averaging the states of $k \times k$ magnets for each \link{linear readout layer}{readout} value.
Hence, absolute values $<1$ indicate the presence of a \xref{domain wall} and it is even possible to distinguish between straight domain walls, corners and other features based on these averaged readout values.
...
To show that the \xref{AFM clocking protocol} does indeed move \xref{domain walls} step by step, independent of their orientation or precise location, we will now consider a few example systems.
...
The absence of defects results in a far more orderly domain wall motion than expected in a real system, where domain walls would be likely to remain pinned at high-\link{net out-of-plane anisotropy}{anisotropy} magnets.
...
\makeshiftfig{
	...
	Evolution of a $16 \times 16$ OOP square-lattice ASI when using the \xref{AFM clocking protocol} (for seven cycles $A$ and seven opposite cycles $B$).
	The moment orientation (\textbf{top}) and domain type (\textbf{bottom}) of each magnet are shown after each cycle was applied.
	The \link{finite dipole model}{finite size} of magnets was accounted for with $D_\mathrm{NM} = \SI{170}{\nano\metre}$ and $S_\mathrm{ASI} = \SI{200}{\nano\metre}$.
	System parameters were chosen such that $\EMC = 40 \kBT$ and $\EEA = 200 \kBT$, without defects ($\sigma(\EEA) = 0$).
	All magnets are initially magnetised in one of the \xref{antiferromagnetic} ground states (white).
	The \link{external magnetic field}{magnetic field} used in the clocking cycles has a magnitude $B_\mathrm{ext} = \SI{3.78}{\milli\tesla}$.
}
...
These domains nucleate at the corners of the ASI, because those magnets only have two neighbours so their \xref{effective energy barrier} $\EBeff$ is at least as low as that of a \xref{domain wall} magnet.
...
Since some information from previous input cycles is often retained in this manner, this property provides the much coveted \xref{fading memory} that is of great importance to RC. \par
...
The reason for this is the relatively low \link{magnetostatic coupling}{MS coupling} $\EMC=40\kBT$, due to which the input strength must strike a delicate balance to avoid switching magnets that are not part of a domain wall.
...
Note also that, during the $B$-cycles in~\cref{fig:3:Clocking_clearly_EBstd0}, the central white-coloured domain does not re-grow with the same Petit-Beurre-shape\footnote{
	The ``\xlabel{Petit Beurre}'' is a French biscuit decorated with serrated edges and four corners in the shape of ears, schematically resembling the central white-coloured domain that appears during the $A$-cycles of~\cref{fig:3:Clocking_clearly_EBstd0}.
...
This is part of the general phenomenon that, in the \xref{AFM clocking protocol}, domain growth preferentially occurs at the corners of a domain. \par
...
\makeshiftfig{
	...
	Evolution of a $141 \times 141$ OOP square-lattice ASI when using the \xref{AFM clocking protocol} (for 36 cycles $A$ and 36 opposite cycles $B$).
	The domain type of each magnet is shown every six cycles.
	All magnets are initially magnetised in one of the \xref{antiferromagnetic} ground states (white), apart from one magnet at the centre which started in the opposite state (black).
	The \link{finite dipole model}{finite size} of magnets was accounted for with $D_\mathrm{NM} = \SI{170}{\nano\metre}$ and $S_\mathrm{ASI} = \SI{200}{\nano\metre}$.
	System parameters were chosen to result in $\EMC = 40 \kBT$ and $\EEA = 200 \kBT$, without defects ($\sigma(\EEA) = 0$).
	The \link{external magnetic field}{magnetic field} used in the clocking cycles has a magnitude $B_\mathrm{ext} = \SI{3.78}{\milli\tesla}$.
}
\vspace{-1em}
...
This is a stochastic process: while the left side of the central domain in~\cref{fig:3:Clocking_massive_seeded} randomly forks between the second and third panels, the right half does not during this particular run. \par % During this particular run
...
Any experimental realisation of OOP ASI will exhibit some degree of manufacturing defects, which are once again modelled by a normally distributed random \link{net out-of-plane anisotropy}{anisotropy} for each magnet in the ASI.
This results in less symmetric states during successive steps of the \link{AFM clocking protocol}{clocking protocol} as compared to the defect-free system from~\cref{fig:3:Clocking_clearly_EBstd0}.
...
This often makes it impossible to find an input magnitude for which \xref{domain walls} can propagate through any defect in the system without affecting magnets that are not part of domain walls.
Therefore, ``\xlabel{leaking}'' can occur through low-barrier unstimulated magnets, which can spontaneously respond to the changing state of their neighbours even when they themselves are not stimulated~\cite{DisorderGroundStateASI}. % DisorderGroundStateASI says: "As expected, disorder allows dynamics to start inside the array at sites where ``loose'' spins with smaller switching fields are located."
This can be observed in~\cref{fig:3:Clocking_clearly_EBstd5} for an example with $\sigma(\EEA)/\EEA = \SI{5}{\percent}$ and high \link{magnetostatic coupling}{MS coupling} $\EMC = 200 \kBT$ compared to the anisotropy $\EEA = 100 \kBT$ to highlight leaking.
...
\makeshiftfig{
	...
	Evolution of a $20 \times 20$ OOP square-lattice ASI when using the \xref{AFM clocking protocol} (for five cycles $A$ and five opposite cycles $B$).
	The domain type of each magnet is shown after each cycle.
	Magnets that have switched through ``\xref{leaking}'' are indicated by red dots.
	The \link{finite dipole model}{finite size} of magnets was accounted for with $D_\mathrm{NM} = \SI{170}{\nano\metre}$ and $S_\mathrm{ASI} = \SI{200}{\nano\metre}$.
	System parameters were chosen to result in $\EMC = 200 \kBT$ and $\EEA = 100 \kBT$, with $\sigma(\EEA)/\EEA = \SI{5}{\percent}$.
	All magnets are initially magnetised in one of the \xref{antiferromagnetic} ground states (white).
	The \link{external magnetic field}{magnetic field} used in the clocking cycles has a magnitude $B_\mathrm{ext} = \SI{5.5}{\milli\tesla}$.
}
...
This is to be avoided, as frozen magnets do not contribute to the \xref{fading memory} of the system.
...
It can even enhance RC performance, as it introduces asymmetry which is beneficial to the \xref{non-linearity} of the system.
...
As the ratio $\sigma(\EEA) / \EMC$ increases, so does the range of fields where low-\link{net out-of-plane anisotropy}{anisotropy} magnets can be switched in the bulk of a domain without significant \xref{leaking} during stepwise \xref{domain wall} propagation.
Such \indexlabel{domain nucleation}nucleation is beneficial for RC, in contrast to defect-free ASI where domain walls could only be injected at corners of the ASI.
%Fortunately from a fabrication perspective, systems with low $\EMC$ are typically easier to produce.
To illustrate the impact of $\EMC$,~\cref{fig:3:Clocking_clearly_EBstd5_EMC} compares a strongly-coupled system (a) with a weakly-coupled system (b). % a system with $\sigma(\EEA) / \EMC = 0.025$ (\crefSubFigRef{fig:3:Clocking_clearly_EBstd5_EMC}{a}) against one with $\sigma(\EEA) / \EMC = 0.25$ (\crefSubFigRef{fig:3:Clocking_clearly_EBstd5_EMC}{b}).
...
\makeshiftfig{
	...
	Evolution of the spatial distribution of domain types in a $20 \times 20$ OOP square-lattice ASI when using the \xref{AFM clocking protocol} (for six cycles $A$ and six opposite cycles $B$).
	All magnets are initially magnetised in one of the \xref{antiferromagnetic} ground states (white).
	The \link{finite dipole model}{finite size} of magnets was accounted for with $D_\mathrm{NM} = \SI{170}{\nano\metre}$ and $S_\mathrm{ASI} = \SI{200}{\nano\metre}$.
	System parameters were chosen to result in $\EEA = 200 \kBT$, with $\sigma(\EEA)/\EEA = \SI{5}{\percent}$.
	\textbf{(a)} System with high \xref{magnetostatic coupling} $\EMC = 400 \kBT$, requiring an input magnitude $B_\mathrm{ext} = \SI{11.2}{\milli\tesla}$ for controlled domain wall movement.
	\textbf{(b)} System with low MS interaction $\EMC = 40 \kBT$ and $B_\mathrm{ext} = \SI{3.78}{\milli\tesla}$.
}
\vspace{-1em}
...
Due to the binary nature of the \link{AFM clocking protocol}{clocked input protocol} --- as each cycle either grows or shrinks domains of a particular type --- it is most straightforward to supply binary input. % The limitation of binary input will be further explored in~\cref{sec:3:clocking_input_encoding}.
...
As introduced in~\cref{sec:1:RC_metrics}, these include \link{matrix rank-based metrics}{metrics based on matrix ranks} and those \link{linear estimator-based metrics}{based on linear estimators}.
...
The ASI we consider here as our baseline system contains magnets with a very large \link{net out-of-plane anisotropy}{perpendicular anisotropy}\footnote{
	...
	For this reason, $\EEA$ is provided in \SI{}{\electronvolt} and the metrics will be presented as a function of the lattice parameter $a$ rather than the \xref{magnetostatic coupling} $\EMC$.
} $\EEA = \SI{110}{\electronvolt} \pm \SI{5}{\percent}$ and the same \xref{magnetic moment} as the thermally active ASI ($\mu = \SI{0.25}{\femto\ampere\metre\squared}$), which was based on magnets with a diameter $D_\mathrm{NM} = \SI{170}{\nano\metre}$.
As these parameters are kept constant, the \link{magnetostatic coupling}{MS coupling} $\EMC$ only depends on the lattice parameter $a$.
...
It is worth noting that the difference between using a \link{point dipole model}{point dipole} or \link{finite dipole model}{finite-size model} was found to be negligible, apart from a nearly uniform shift of the following metric maps toward larger lattice spacings, by about $\Delta a = 20$ to \SI{30}{\nano\metre}. \par % If we want to show this as figures, compare Sweep20250325131714 vs. Sweep20250325105313.
...
\paragraph{Matrix rank-based metrics}\indexlabel[nolabel]{matrix rank-based metrics}
Three \xref{matrix rank-based metrics} can be calculated (\cref{sec:1:RC_metrics_KQ}): the \indexlabel[nolabel]{kernel-quality}\xref{kernel-quality} $K$, \indexlabel[nolabel]{generalisation-capability}\xref{generalisation-capability} $G$ and \indexlabel[nolabel]{computing capacity}\xref{computing capacity} $C=K-G$.
...
Recall that the maximum value of $K$, $G$ and $C$ is the number of \link{linear readout layer}{readout nodes}; here, a $20 \times 20$ ASI with $10 \times 10$ readout nodes --- each representing the average domain type of a $2 \times 2$ magnet cluster --- yields a maximum value of 100 for all three metrics.

\vspace{-1em}
\xfig{3_RC_OOP/Nonvolatile/Sweeps/Sweep20230208082817.pdf}{
	\label{fig:3:Clocking_Sweeps_binary_KQ}
	\link{kernel-quality}{Kernel-quality} $K$, \xref{generalisation-capability} $G$ and \xref{computing capacity} $C$ of a $20 \times 20$ OOP square-lattice ASI with $\EEA = \SI{110}{\electronvolt} \pm \SI{5}{\percent}$.
	Beyond the displayed range of lattice constants $a$ and input field magnitudes $B$, the metrics decrease.
	The corresponding \xref{magnetostatic coupling} $\EMC$ ranges from $146\kBT$ (at $a=\SI{220}{\nano\metre}$) to $88\kBT$ (at $a=\SI{260}{\nano\metre}$), as the \xref{magnetic moment} of each magnet is $\mu = \SI{0.25}{\femto\ampere\metre\squared}$.
...
\paragraph{Linear estimator-based metrics}\indexlabel[nolabel]{linear estimator-based metrics}
Although the \xref{matrix rank-based metrics} provide insight into how the system responds to various input patterns, they are relatively expensive to compute --- requiring as many input sequences as there are readout nodes (here: 100), with each sequence comprising many (here: 100) input values.
In comparison, \xref{linear estimator-based metrics} such as \indexlabel[nolabel]{non-linearity}\xref{non-linearity} and \indexlabel[nolabel]{memory capacity}\xref{memory capacity} can be determined using only several hundred random input values, making them considerably more efficient to compute.
...
For example, the input method with a uniform field --- which was previously dismissed for non-volatile ASI --- achieves a moderate $C \approx 40$, but yields very low values for all linear estimator-based metrics like non-linearity and memory capacity. % Matrix rank-based metrics should rather be seen as indicators of which systems do NOT give decent RC performance.
...
Therefore, the \indexlabel[nolabel]{parity check}\xref{parity check} metric is also shown, as it was specifically designed for binary input. % See ppt 20230207

\xfig{3_RC_OOP/Nonvolatile/Sweeps/Clocking_binary_TA_averaged.pdf}{
	\label{fig:3:Clocking_Sweeps_binary_TA}
	\link{linear estimator-based metrics}{Linear estimator-based metrics} for the same input, readout and system parameters as in~\cref{fig:3:Clocking_Sweeps_binary_KQ} ($20 \times 20$ OOP square-lattice ASI with $10 \times 10 = 100$ \link{linear readout layer}{readout nodes}, $\EEA = \SI{110}{\electronvolt} \pm \SI{5}{\percent}$, $\mu = \SI{0.25}{\femto\ampere\metre\squared}$), over the same range of lattice spacings $a$ and input magnitudes $B$.
	The \xref{non-linearity} is bounded in the range $[0,1]$, while the other two metrics have no upper limit.
}

Systems with stronger \link{magnetostatic interaction}{MS interaction} behave more non-linearly, though only in a narrow range of input \link{external magnetic field}{field} magnitudes.
As envisioned, the \link{AFM clocking protocol}{clocking protocol} indeed enables the system to retain some information from previous inputs: the \xref{memory capacity} reaches values $>2$ over a wide range of the parameter space, while the \xref{parity check} reveals that (on average) the parity of the previous 3 input cycles can be recalled.
That the parity check reaches higher values than the memory capacity was to be expected, as it is specifically tailored to binary input data whereas memory capacity is more generally applicable to scalar data, though the parity check also includes a notion of \xref{non-linearity}.
These metrics provide more insight than the \xref{matrix rank-based metrics}, as they can directly be related to the performance of the system for certain tasks --- some tasks mostly rely on \xref{fading memory} while others mostly require non-linearity, and yet others prefer a mix of both. \par
While $<3$ bits of memory may sound low, it is comparable to the memory capacity and parity check of other magnetic reservoirs~\cite{NeuromorphicFewShot,hon2021numerical,tsunegi2019STOforcedsyncRC,Venkat_2024,Vidamour_2022}. % NeuromorphicFewShot is similar when using a normal square ASI, but width-modulated and pinwheel ASI perform better.
Higher performance can be achieved by using general RC architectures like the \xlabel{rotating neurons reservoir}~\cite{RotatingNeuronsRC} or the \xlabel{single dynamical node} paradigm~\cite{appeltant2011information}, as for example used in~\ccite{Venkat_2024,Vidamour2023}.
...
The reason for this is that different initial states require different minimal input \link{external magnetic field}{field} magnitudes --- it is easier to switch a magnet when the ASI is in a uniform state than when it is in the \link{antiferromagnetic}{AFM} ground state. % Compare Sweep_230220_1_binaryInput_randomInit.out/Sweep20230220132509 with Sweep_230119_2_binaryInput.out/Sweep20230119153543
This is further complicated by defects, which may impede \xref{domain wall} motion.
For example, when starting from the AFM ground state, domain walls must be injected from the edges and propagate throughout the system, whereas an initially uniform state will enable initial switches (i.e., \xref{domain nucleation}) to occur throughout the bulk of the system.
...
Therefore, to ensure consistency between the RC metric maps presented in this section, all simulations start from the AFM ground state\footnote{
	While the high \link{net out-of-plane anisotropy}{anisotropy} $\EEA$ prevents the system from spontaneously reaching the ground state, it can be forced to the ground state by applying a single clocking cycle with a stronger stimulus than used during computation.
}.
...
Now, RC metrics can be used to verify this claim: the \xref{linear estimator-based metrics} for a system without defects ($\sigma(\EEA) = 0$) are shown in~\cref{fig:3:Clocking_Sweeps_binary_TA_EBstd0}. \par
While the \xref{memory capacity} and \xref{parity check} are still definitively non-zero, they are at least $\approx \SI{40}{\percent}$ lower than for a system with $\sigma(\EEA)/\EEA = \SI{5}{\percent}$ defects.
Furthermore, while a region with near-unity \xref{non-linearity} now exists, the overlap between parameter combinations that yield decent non-linearity and those that yield decent memory capacity remains rather limited.

\xfig{3_RC_OOP/Nonvolatile/Sweeps/Sweep20230124100320.pdf}{
	\label{fig:3:Clocking_Sweeps_binary_TA_EBstd0}
	\link{linear estimator-based metrics}{Linear estimator-based metrics} for a defect-free system ($\sigma(\EEA) = 0$) with otherwise the same input, readout and system parameters as in~\cref{fig:3:Clocking_Sweeps_binary_TA} ($20 \times 20$ OOP square-lattice ASI with $10 \times 10 = 100$ readout nodes, $\EEA = \SI{110}{\electronvolt}$, $\mu = \SI{0.25}{\femto\ampere\metre\squared}$), over the same range of lattice spacings $a$ and input magnitudes $B$.
	The \xref{non-linearity} is bounded in the range $[0,1]$, while the \xref{memory capacity} and \xref{parity check} have no upper limit.
}

While this shows that defects improve the \xref{memory capacity} of the system, it does not prove that a defect rate $\sigma(\EEA)/\EEA = \SI{5}{\percent}$ would be optimal.
To assess this,~\cref{fig:3:Clocking_Sweeps_binary_TA_EBstd} shows \xref{linear estimator-based metrics} for a range of defect rates $\sigma(\EEA)/\EEA$ and input \link{external magnetic field}{field} magnitudes $B$ while the lattice spacing was kept constant.

\xfig{3_RC_OOP/Nonvolatile/Sweeps/Sweep20250327111551.pdf}{
	\label{fig:3:Clocking_Sweeps_binary_TA_EBstd}
	\link{linear estimator-based metrics}{Linear estimator-based metrics} as a function of disorder $\sigma(\EEA)/\EEA$ with otherwise the same input, readout and system parameters as in~\cref{fig:3:Clocking_Sweeps_binary_TA} ($20 \times 20$ OOP square-lattice ASI with $10 \times 10 = 100$ \link{linear readout layer}{readout nodes}, $\EEA = \SI{110}{\electronvolt}$, $\mu = \SI{0.25}{\femto\ampere\metre\squared}$), over the same range of input magnitudes $B$, with $a=\SI{230}{\nano\metre}$ kept constant.
	The \xref{non-linearity} is bounded in the range $[0,1]$, while the \xref{memory capacity} and \xref{parity check} have no upper limit.
}

The trade-off between \xref{non-linearity} and \xref{memory capacity}, which has been extensively discussed~\cite{dambre2012information,MemoryNonlinearityReservoirs,RC_BeyondMemoryNonlinearity,RC_unification} and reported in various systems~\cite{DynamicEmergence_NanomagneticSystem,RC_TaskAgnosticMetrics_v2,TaskAdaptivePRC}, is striking in this figure as well.
...
The memory capacity and \xref{parity check}, on the other hand, benefit greatly from disorder.
...
The optimal \link{external magnetic field}{field} range always remains centred around the same value, but broadens as the disorder increases.
This is a consequence of the greater spread of \xref{effective energy barriers} throughout the ASI due to the increased disorder, widening the range of field strengths at which any of the magnets can switch.

\subsubsection{Alternative input encoding}\label{sec:3:clocking_input_encoding}
The \link{AFM clocking protocol}{clocking protocol} naturally lends itself to binary input\footnote{
	...
	One could similarly imagine a quaternary input (4 values) for Pinwheel ASI, which exhibits \link{superferromagnetism}{superferromagnetic} domains with 4 possible average \xref{magnetisation} angles, though this is mere conjecture as binary input was used by Jensen~\etal~\cite{clocking-protocol} when demonstrating clocking in pinwheel ASI.
}.
...
While this is the natural habitat of conventional computers, it is not a viable solution here due to the limited linear \xref{memory capacity} of the system.
...
Furthermore, one must take care to regularly apply both positive and negative cycles to keep the \xref{domain wall} dynamics going, complicating the input encoding.
Ultimately, both of these analog input concepts were dismissed as they did not provide a desirable effect.

\subsection{Summary}
Achieving \xref{reservoir computing} in non-volatile OOP ASI posed several challenges.
A uniform global input could not be used for RC, as it does not distinguish between the degenerate ground states.
This was solved by devising an input with the same checkerboard pattern as the \link{antiferromagnetic}{AFM} ground state.
However, applying a global AFM input did not yield usable dynamics since uncontrolled \xref{domain wall} motion saturates the system in one of its ground states, erasing prior information present in the system.
This led us to formulate two basic requirements for using a non-volatile ASI as a reservoir: the input must lift the degeneracy of the ground states such that it can affect domain boundaries, and at least two independently addressable sublattices should exist to prevent \xref{avalanches}. \par
Hence, a \link{AFM clocking protocol}{clocking protocol} was developed that does not stimulate all magnets simultaneously.
More specifically, the system was divided into two interleaved sublattices, allowing the \link{stepwise domain wall motion}{controlled propagation of domain walls}.
...
This was reflected by an increased \xref{memory capacity} ($\approx 1.5 \rightarrow 2.5$ bits) and \xref{parity check} performance ($\approx 1.5 \rightarrow 3$ bits), though the maximum \xref{non-linearity} decreased. \par
...
\section{Conclusion}
We have numerically demonstrated the \xref{reservoir computing} capability of both thermally active and non-volatile OOP ASI, each of which required very different input methods.
The spontaneous \xref{logarithmic relaxation} of thermally active ASI towards the ground state results in a gradual response to external stimuli over a broad frequency range, which allowed us to use it as a sort of \xref{leaky integrator}.
However, non-volatile ASI requires a more intricate input method in order to avoid \xref{avalanches}, which led us to develop a \link{AFM clocking protocol}{clocking protocol} that results in \xref{stepwise domain wall motion}. \par
...
The binary character of the clocking scheme restricts its available tasks, so there we opted to use the \xref{linear estimator-based metrics} instead.
Both systems achieved a performance comparable to other single reservoirs on their respective tasks and metrics.
Introducing spatial variation --- in the form of a \xref{property gradient} in thermally active ASI and random disorder in non-volatile ASI --- was found to provide a richer output for both systems.
