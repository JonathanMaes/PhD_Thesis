...
When the energy barrier $\EEA$ separating the `up' and `down' states of a magnet is significantly higher than the thermal energy, the N\'eel-Arrhenius switching law says that it will remain stable over any reasonable timescale --- for example, a magnet with $\EEA > 40 \kBT$ will remain stable over multiple years in the absence of external perturbations.
Therefore, non-volatile ASI is easier to construct, as it does not require great control over the perpendicular anisotropy: it just requires a large anisotropy.
...
While we previously noted that the magnets in the thermally active system had a coupling $\EMC \approx 10 \kBT$, this will not suffice here: if the anisotropy is $\gg 40 \kBT$, the magnetostatic coupling between magnets must be increased.
This should not present a significant practical issue: as demonstrated in Supplementary Information 10 of our paper~\cite{KUR-24}, the inclusion of a separate ferromagnetic layer above the magnets can focus the stray fields onto nearby magnets, thereby significantly increasing the MS coupling.
...
They lack the intrinsic relaxation dynamics that we previously relied upon for RC in thermally active ASI, so a fundamentally different input method will be required.
In a non-volatile system, rather than pulling the system out of equilibrium and expecting it to return towards the ground state by itself, we must instead make use of the domain structure for RC.
...
A uniform external field does not fulfill the first requirement, i.e. that an input method should be able to distinguish between the two ground states if it is to be used in a clocking protocol.
...
\paragraph{Avoiding avalanches}
However, applying the aforementioned AFM input is insufficient to achieve a clocking protocol.
The main issue is that the AFM input provides a global stimulus.
This means that all magnets in the system will want to take on the ground state that the input promotes, which can easily saturate the system.
The only thing preventing saturation are the defects that cause variation on the anisotropy $\EEA$, as these can prevent magnets from switching to the opposite state.
However, this means that the AFM input is not fundamentally different from the uniform output we dismissed!
They both just pull magnets towards the direction of the external stimulus, with only high-anisotropy magnets resisting the switching.
Aside from the AFM input using the ground state of the system, their effects only differ by a $\pm 1$ sign on each magnet, which gets negated by their respective readout methods.
While the AFM and uniform input have different effects in reality, they are equivalent from a computational point of view.
The only difference is that the AFM input can achieve switching with lower fields, as it only has to switch domain wall magnets to make domain walls propagate through the system, whereas a uniform input requires a stronger field to pull magnets away from the ground state.
To make use of the natural ordering of an ASI, we must allow some magnets in the ASI to choose their state based on their neighbours. \par
Therefore, to avoid saturation, two sub-steps can be applied instead.
For this, we can divide the magnets into two groups, akin to the black and white squares on a checkerboard.
...
In this scheme, half of the magnets are not stimulated at any time, which means they can not switch if the anisotropy $\EEA$ is sufficiently high (i.e., $\gg 40\kBT$ for practical timescales).
...
By choosing the strength of the stimulus correctly, we can make it such that only magnets that are part of a domain wall will switch.
After all, any domain wall magnet is less stable than a magnet in the bulk of a domain, as domain wall magnets only have 3 or 2 oppositely-oriented NN.
In combination with the two-step method that prevents half of the magnets from switching, this means that we can move domain walls by only a few lattice sites at once. \par
To use this clocking scheme for RC, input values must be mapped to a certain cycle.
We will map an input of value `0' to the application of cycle $A$, and a value `1' to cycle $B$.
% For e.g. bit 0 we first apply a + stimulus on the first sublattice with 0 stimulus on the other sublattice, and then apply 0 stimulus on the other sublattice with - stimulus on the first sublattice.
In this two-step input scheme, it is not possible to apply scalar input, because we are using domain wall movement as the input, which limits the options.
Since there are only 2 domain types, we can only apply binary input.
Therefore, we can not perform the signal transformation task with this system.
This limitation of binary input is explored further in~\cref{sec:3:clocking_input_encoding}. \par
...
\paragraph{Example}
For a clean illustration of the clocking scheme, we will use an example with $\EEA=100\kBT$ and a very high coupling $\EMC=200\kBT=2\EEA$.
This way, the influence of the random defects in $\EEA$ is suppressed, as the magnetostatic interactions are far stronger.
This results in a far more deterministic domain wall movement.
%Besides this and the requirement for a stronger stimulus to counteract the strong magnetostatic coupling --- domain wall magnets typically have 3 oppositely-oriented NN and are therefore stable with $\EBeff \propto \EEA + \EMC$ --- not much changes.
