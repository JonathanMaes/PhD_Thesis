...
\begin{enumerate}
	...
	\item \textbf{Shape anisotropy} of each layer due to their \xref{demagnetising field}.
...
To better understand thermally active ASI and inform our choices for RC in this system, it is essential to familiarize ourselves with its relaxation dynamics.
...
In the Ising-like model used by \hotspice, two essential system parameters remain.
The first is the net OOP anisotropy --- as determined by the energy barrier $\EEA$ of a non-interacting OOP magnet --- which is the combined contribution of PMA, shape anisotropy and the inter-layer MS interaction.
Secondly, the magnetostatic coupling between nearest neighbours --- denoted as $\EMC$ --- is responsible for the relaxation to the ground state. \par
In real ASI, manufacturing variability may introduce defects, which will be modelled by a Gaussian spread on the anisotropy $\EEA$: unless mentioned otherwise, a standard deviation $\sigma(\EEA) = \SI{5}{\percent}$ will be used.
This provides pinning sites for domain walls, which can make the relaxation process more spatially reproducible.
Such consistent variation within a particular ASI could ultimately benefit RC by providing a richer output. \par
