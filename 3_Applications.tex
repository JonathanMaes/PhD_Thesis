...
This slope change marks the transition between what we call phase $\circled{1}$ and phase $\circled{2}$ of the relaxation.
In the figure, phase $\circled{2}$ is indicated by the green shaded area. \par
...
During phase $\circled{1}$, a significant amount of magnets still have three or four nearest neighbours (NN) which have not yet switched --- see for example the final state (at \SI{e4}{\second}) of panel 2, shown in its inset: most magnets still point `up' (white).
...
This is no longer the case in a system which has reached phase $\circled{2}$ --- in panel 3, which has barely reached the second phase, very few magnets with $\geq 3$ NN of the same orientation remain.
In phase $\circled{2}$, most magnets have reached a stable local checkerboard ground state.
...
Unlike the constantly changing global environment of phase $\circled{1}$, domain wall magnets typically have only 1 or 2 oppositely-oriented NN and are embedded within checkerboard domains.
...
\xfig{3_OOP/OOP_relaxation_continuous.pdf}{
	...
	Phase diagrams of the average magnetization $\mavg$ and the local antiferromagnetic parameter $\qNN$ as a function of net out-of-plane anisotropy $\EEA$ and nearest-neighbour magnetostatic coupling $\EMC$ at $t=\SI{e4}{\second}$.
	...
	The white (black) dotted line corresponds to $\mavg = 0.05$ ($\qNN = 0.95$).
