...
For this reason, \textit{A. Kurenkov} measured the state of manufactured ASI at different moments in time using MFM.
For each of these MFM images, the values of $\mavg$ and $\qNN$ can be determined.
The knowledge of these quantities at different moments in time can then be used to fit this data to simulated relaxations, like the ones in~\cref{fig:3:OOP_relaxation}, by varying simulation parameters like $\EEA$ and $\EMC$. \par
...
These distributions\footnote{
	Since the random relaxation is governed by the N\'eel-Arrhenius equation~\eqref{eq:2:Néel}, quantities like $\mavg$ which are bounded in the range $[0,1]$ most likely follow a binomial distribution, which can be well approximated by a normal distribution for a large number (here: $> 100$) of magnets in the system.
} are each characterised by a mean and standard deviation, which can be determined from repeated simulations --- typically, 20 relaxations suffice. % We embrace the Monte Carlo nature of \hotspice. 
Therefore, each experimental data-point is some amount of standard deviations away from the mean of this simulated distribution.
As fit metric $\eta$, we will use the square of this number of standard deviations, summed over each experimental $\mavg$ and $\qNN$ at each measured time $t$.
In a sense, this is related to the relative probability that the MFM measurement can be explained by a simulation using certain system parameters.
Mathematically,
\begin{equation}
	\eta = -\frac{1}{2} \bigsum_{t_i} \bigsum_{x \in \left\{\substack{\mavg\\\qNN\\\qNN[2]\\\qNN[3]}\right.} \ab(\frac{\langle x_\mathrm{sim}(t_i) \rangle - x_\mathrm{MFM}(t_i)}{\sigma\big(x_\mathrm{sim}(t_i)\big)})^2 \mathrm{,}
\end{equation}
...
With this fit metric $\eta$, the optimum combination of $\EEA$ and $\EMC$ can be determined.
...
It is an iterative method that starts from an initial guess --- estimated values of $\EEA$ and $\EMC$ can be determined based on the experimentally measured $\mavg(t)$ by using the inverse of~\cref{eq:3:relaxation_fit_ab}.
...
More extensive parameter searches may benefit from using other optimisation techniques like LIPO~\cite{LIPO}. \\\par % Good explanation in http://blog.dlib.net/2017/12/a-global-optimization-algorithm-worth.html
...
For this reason, the samples were measured soon ($\approx \SI{e3}{\second}$) after initialisation, and again after 2 years $\approx \SI{7e7}{\second}$, providing sufficiently distinct timescales for a successful fit.
While two measurements are insufficient to prove that the relaxation indeed follows a logarithmic behaviour as predicted by simulation, this was verified using multiple measurements over a shorter timescale as discussed in~\ccite{KUR-24} (Supp. 5.4).

\paragraph{Results} % EEA etc.
% TODO: present the results of this BO procedure, possibly in combination with the `simple' vs. `parabolic' energy barrier and the infinitesimal spin vs. the finite spin approximation in the DD energy calculation, and how these affect the best fit (and coupling this back to verifying which approximations are beneficial and which are not).
