...
\usepackage[small,bf,centerlast]{caption}[=v1] % Customize appearance of fig/table captions
...
\usepackage[latin1]{inputenc} % Support for Latin-1 encoding (UTF-8 is generally preferred)
...
\usepackage{backref} % Adds backreferences from bibliography to citing pages
...
\usepackage{hyperref} % Creates hyperlinks and metadata in PDFs
...
\usepackage{mdframed} % Creates framed content with custom styles
...
\usepackage{physics} % Simplifies common physics expressions and notation
\usepackage{soul} % For text decoration, e.g., underline (\ul{}), strikethrough (\st{}), highlighting...
% \usepackage{subcaption} % Subfigure captions
\usepackage{tikz}
\usepackage{titlesec} % Customizes the format and spacing of section titles (e.g. \setcounter{secnumdepth} to make subsubsections numbered in chapters)
\usepackage{tocbibind} % Adds bibliography and table of contents to ToC
\usepackage{tocloft} % Customizes the layout of the table of contents
...
\definecolor{xrefcolor}{HTML}{000040}
...
\bibliography{bib/bibliography.bib}\cleardoublepage
