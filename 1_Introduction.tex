...
The full internal state of the reservoir following this perturbation is often unknown, so a $p$-dimensional readout $\vc{r}(t) \in \mathbb{R}^p$ is assumed (i.e., the green nodes in~\cref{fig:1:NeuralNetworks}).
...
} $\hat{y}(t)$ of the system is then obtained by a weighted sum $\hat{y}(t) = \vc{r}^\mathrm{T}(t) \cdot \vc{w}$ of the components of the response vector $\vc{r}(t)$.
...
By sampling the readout and desired output at $n$ moments in time, $n$ vectors $\vc{r}_i$ and $\vc{y}_i$, $i = 1, \dots, n$ are obtained.
These can be arranged into an $n \times p$ readout matrix $\vc{R}$ (whose row $i$ is $\vc{r}_i^T$) and the $n$-dimensional desired output vector $\vc{y}$.
The optimal weights can then directly be calculated as
\begin{equation}
	\vc{w} = (\vc{R}^T \vc{R})^{-1} \vc{R}^T \vc{y} \mathrm{,}
\end{equation}
which minimises $\norm{\vc{y} - \hat{\vc{y}}}^2 = \norm{\vc{y} - \vc{R}\vc{w}}^2$~\cite{RC_NNN}.
...
By periodically sampling this $p$-dimensional vector at $n$ moments in time, an $n \times p$ matrix $\vc{R}$ can be constructed whose rows are these vectors at different moments in time.
...
A more efficient alternative method would be to record the response $\vc{r}$ after each input value, as this would also result in maximally distinct input sequences, though this method strays further from the original definition~\cite{RC_HierarchicalNeuroevolution,RCbenchmarksReview1}.
...
This yields the \idx{compute quality} $Q = K - G$, where higher values typically indicate better performance.
...
\begin{equation}
	\hat{r}_i(t) = c_i + \sum_{\tau=0}^{k} w_{i,\tau} s(t - \tau) \mathrm{,}
\end{equation}
is trained by adjusting the weights $w_{i,\tau}$ and $c_i$ based on the known discrete-time input-output relationship (on the training set).
...
\begin{equation}
	\mathrm{NL} = 1 - \frac{1}{p} \sum_{i=1}^p \mathrm{R}^2[\hat{r}_i;r_i] = 1 - \frac{1}{p} \sum_{i=1}^p \frac{\mathrm{cov}^2(\hat{r}_i; r_i)}{\sigma^2(\hat{r}_i) \sigma^2(r_i)} \mathrm{.}
\end{equation}

\paragraph{Linear memory capacity}
The linear memory capacity can also be calculated through by means of linear estimators, but by working the other way around: an estimator for the past input $s(t - \tau)$ is now trained based on the current state $\vc{r}$ of the reservoir~\cite{tsunegi2019STOforcedsyncRC,NeuromorphicFewShot}.
In other words, the estimator
\begin{equation}
	\hat{s}(t - \tau) = c_\tau + \sum_{i=1}^{p} w_{i,\tau} r_i(t)
\end{equation}
is now trained.
...
\begin{equation}
	\label{eq:1:MC}
	\mathrm{MC} = \sum_{\tau = 1}^{k} \mathrm{R}^2[\hat{s}(t - \tau); s(t - \tau)] = \sum_{\tau = 1}^{k} \frac{\mathrm{cov}^2(\hat{r}_i; r_i)}{\sigma^2(\hat{r}_i) \sigma^2(r_i)}
\end{equation}
...
Secondly, the energy associated with \xref{shape anisotropy} increases with volume, so a maximal number of layers for a given amount of ferromagnetic material is preferable for PMA to counteract the in-plane shape anisotropy.
...
% TODO END: a lot of this was copy-pasted from the Chapter 3 introduction, so have to revisit that. Must keep a brief explanation in Chapter 3 though, saying that OOP magnets use PMA and are constructed in stacks to counteract the shape anisotropy, and refer to this section and PMA using cref and xref.
...
A relatively common technique is \idx{magnetic force microscopy} (MFM)~\cite{MFM}, which scans a cantilever with a very sharp tip across the sample, similar to atomic force microscopy (AFM).
...
\subsubsection{\spinengine} % TODO: dont give a paragraph of attention to this
...
The RC potential of IP ASI has been demonstrated numerically~\cite{RC_ASI}, but the experimental application of input and state readout is not straightforward.
Often, an external field was used to provide input, but it would be more convenient to use SOT~\cite{SOT_switching_IP}.
Efficient read-out of IP ASI remains challenging, due to the symmetry of the system and its discontinuous nature. \par % TODO END: cite roadmap and include more info
For OOP ASI, efficient input (SOT) and readout (AMR) mechanisms had been demonstrated experimentally, but their RC potential had not.
...
One example using such a macrospin approximation is the high-performance flatspin simulator~\cite{flatspin}, which implements deterministic spin flipping via a generalised Stoner-Wohlfarth model~\cite{StonerWohlfarth2008}.
This model revolves around the effective field experienced by each nanomagnet, whose switching is determined by an angle-dependent coercive field --- it is easier to switch a nanomagnet by applying a field at an oblique angle w.r.t. the easy axis, rather than parallel or perpendicular to it.
