...
This leads to the \xref{magnetostatic interaction} that will later be described by~\cref{eq:2:E_MS}. % TODO: this last sentence is weird. Either explain it here already with equations or just don't mention it. I feel like I should somehow mention that moments like to align in the same direction, unless something prevents them from moving.
% Since the torque implies that magnets prefer to align in the direction of the magnetic field, the form of the magnetostatic interaction implies that magnetic moments prefer parallel alignment when they are in-line, while they prefer anti-parallel alignment when they are placed side-by-side. \par % TODO: describe the field here already?
...
A non-vanishing net magnetic moment is the privilege of \textit{ferromagnetic}\indexlabel{ferromagnetism} materials, like \ce{Fe}, \ce{Ni} and \ce{Co} (at room temperature).
It is only in these materials that nearby atomic magnetic moments have a strong tendency to align due to the Heisenberg-Dirac exchange interaction~\cite{heisenberg1928theorie}, leading to a non-zero macroscopic \idx{magnetisation} within the material. \par
...
This is called the \idx{saturation magnetisation}, as it equals the magnetisation of the material if it were to be saturated into one direction by a strong external field.
...
Indeed, in a large piece of ferromagnetic material, it is more energetically favourable for the magnetisation to arrange into \idx{ferromagnetic domains} whose moments cancel each other out. % TODO: cite Weiss?
...
This results in a \idx{single-domain nanomagnet} with a nearly uniform magnetisation $\vc{M}(\vc{r}) \approx \vc{\mu}/V$, whose magnitude is roughly equal to the saturation magnetisation $M_\mathrm{sat}$~\cite{FRENKEL1930}. % Frenkel and Dorfman first conceived the idea of a single-domain particle based on energy considerations.
When this particle exhibits some form of anisotropy, a preferential magnetisation axis will emerge, referred to as the \idx{easy axis}.
...
One common type of anisotropy is \idx{shape anisotropy}, which originates from the tendency of magnetic moments to align in the same direction, resulting in the magnetisation generally aligning along the longest axis of the particle~\cite{MagneticCharge}.
Anisotropy can also originate from the crystal structure, but can be circumvented by an appropriate choice of material; for example, permalloy (\ce{Fe20Ni80}) is an alloy specifically designed to exhibit minimal crystal anisotropy.
...
% TODO: move discussion from chapter 3 to here?

\paragraph{Micromagnetic simulations}
% TODO: basic overview of micromagnetic simulations and how they work, so we can compare this with the \hotspice approach
