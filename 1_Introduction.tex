...
This research was performed in the context of the European \spinengine project, where three types of magnetic reservoirs were considered: magnetic nanorings~\cite{DynamicEmergence_NanomagneticSystem}, in-plane ASI~\cite{RC_ASI}, and perpendicular-anisotropy ASI~\cite{KUR-24}.
...
The latter are also referred to as task-agnostic metrics~\cite{RC_TaskAgnosticMetrics}. % Q: find a better name for them?
...
This requires them to use other types of anisotropy to favour a magnetisation direction perpendicular to the substrate.
A common solution is to use interfacial anisotropy between the ferromagnetic material and the substrate, as for example occurs at the interface between \ce{Co} and \ce{Pt}~\cite{PMA_PdCo_PtCo}.
...
\subsubsection{Input} % TODO: address bartel's comment here
In an experimental setting, input is often provided through external fields, but their inductance limits the attainable frequency.
For on-chip applications, faster and more efficient all-electrical methods exist that skip the magnetic field step: these exert a torque on the magnetisation by directly injecting spins into the magnet.
This avoids the generation of strong fields that may have unintended effects elsewhere in the system.
%While it is possible to miniaturise these fields by patterning striplines on-chip, allowing for local control, all-electrical control is more desirable.
% Compared to an \xref{external magnetic field}, this allows far greater control over the stimulus applied for each input bit, and can more easily be integrated on-chip without generating significant stray fields.
% While it is possible to use an external field to switch both IP and OOP nanomagnets, this is undesirable in a real device due to stray fields and the need for bulky magnets or strip lines.
...
X-ray based measurements like photo-electron emission microscopy (PEEM)~\cite{PEEM} and magnetic transmission X-ray microscopy (MTXM)~\cite{Imaging_MTXM} can make use of the \xlabel{X-ray magnetic circular dichroism} (XMCD) effect to achieve even higher resolution than MFM.
