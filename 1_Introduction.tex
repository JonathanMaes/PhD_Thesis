...
\begin{itemize}
	...
	\item Conventional computers are built according to the \xlabel{von Neumann architecture}, where computation (CPU) is separated from memory (RAM).
...
\vspace{-1.5em}
\sidefig[0.54]{1_Introduction/Ngrams.pdf}{
	\label{fig:1:Ngram}
...
Neuromorphic computing, on the other hand, does not seek exact solutions; in the words of Alan Turing, ``if a machine is expected to be infallible then it cannot also be intelligent.'' \par
...
A related concept is \idx{material computation}~\cite{NeglectedPillar}, which goes one step further: its goal is to find ways to exploit the inherent computing power present in physical substrates.
...
Among the various substrates being explored for material computation, magnetic and \indexlabel[nolabel]{spintronics}spintronic (spin transport electronic~\cite{Spintronics}) hardware are especially appealing~\cite{grollier2020neuromorphic,NeuromorphicSpintronicsProspect,QuantumNeuromorphicOpportunities}.
\link{single-domain nanomagnet}{Single-domain} \link{ferromagnetism}{ferromagnetic} \link{single-domain nanomagnet}{nanomagnets}, for example, have the ability to store information, respond non-linearly to stimuli~\cite{NeuromorphicSpintronics} and can dissipate very little energy during information processing~\cite{ThermodynamicLimitsComputation,SpintronicsEnergyEfficientComputing}.
...
ASI with out-of-plane \xref{magnetisation} offers particularly convenient electrical interfacing for both input and readout, but its computational capability had yet to be demonstrated. \newpage % In comparison to in-plane ASI, for which the computational capability had already been demonstrated numerically~\cite{ASI_computation,hon2021numerical}
...
\vspace*{\fill}
\xfig[1.0]{outline.pdf}{
	\label{fig:1:outline}
	Visual representation of the structure of this thesis.
	%Arrows highlight the relations between chapters.
}
\vspace*{\fill}
...
At the intersection of \xref{neuromorphic computing} and \xref{material computation} lies the paradigm of \idx[nolabel]{reservoir computing}.
...
\subsection{Origin}
\paragraph{Artificial neural networks\indexlabel{artificial neural network}}
...
To prevent these values from accumulating without limit, they are typically bounded to the range $[0,1]$ by applying an \xlabel{activation function} to the weighted sum --- usually a sigmoid, like the logistic map $e^x/(1+e^x)$ or hyperbolic tangent~\cite{SurveyUniversalApproximation}.
...
Some approaches to train RNNs take inspiration from FNN: \xlabel{backpropagation through time} (BPTT), for instance, unfolds the recurrent connections in time to obtain a FNN on which backpropagation can be applied~\cite{jaeger2002tutorial}.
...
\xfig{1_Introduction/NeuralNetworks.pdf}{
	...
	Schematic representation of \textbf{(a)} a \xref{feed-forward neural network} (FNN), \textbf{(b)} a \xref{recurrent neural network} (RNN) and \textbf{(c)} \xref{reservoir computing} (RC).
	In the latter, the reservoir is treated as a black box; it can be of mathematical origin, like a RNN, or a physical system like \xref{artificial spin ice} (ASI).
	Grey connections propagate information from left to right, unless indicated by an arrow.
	Blue (red) nodes represent input (output), while orange nodes constitute the hidden layers of the network.
}
\vspace{-1em}
...
Jaeger~\cite{jaeger2001echo} presented \xlabel{echo state network}s (ESNs), while Maass~\etal~\cite{maass_LSM} presented \xlabel{liquid state machine}s (LSMs)\footnote{
	...
	The ``echo'' of ESNs refers to the \xref{fading memory} provided by the internal dynamics of a recurrent network, where loops can retain information from past inputs.
}.
...
Instead, only a single \xlabel{linear readout layer} is trained via linear regression to obtain the desired output from this random network~\cite{D-LSM,D-ESN-Improved}.
...
From these principles, it is but a small step to \xref{reservoir computing} (RC), which generalises these ideas by treating the internal network as a black box~\cite{RC_unification,D-ESN-Improved,RC_Tensegrity}.
In the next section we will explore the reservoir computing framework in more detail.

\subsection{Reservoir computing framework\indexlabel{reservoir computing}}
...
A weighted sum of the components of (a lower-dimensional representation of) the reservoir's state vector (orange nodes) provides the output (red node) of the system as a whole. % REFs for RC, specifically training a readout layer: Dale et al., 2017; Jensen and Tufte, 2017; Sillin et al., 2013
...
Crucially, \textit{the properties of the reservoir itself are not modified during training} --- only the final \xref{linear readout layer} is~\cite{RC_ASI,DynamicEmergence_NanomagneticSystem}. \par
Since the reservoir acts as a black box, its properties can not be changed to obtain the desired response\footnote{
	It is possible to adjust a physical reservoir to exhibit the desired properties for a given task~\cite{NeuromorphicFewShot,gartside2022reconfigurable}, but this is different from mathematically tuning the weights of the \link{linear readout layer}{readout layer} via a simple procedure like linear regression.
}.
To solve temporal tasks, a reservoir should be a high-dimensional non-linear system with \indexlabel[nolabel]{fading memory}short-term memory~\cite{NeuromorphicAFMspintronics,RC_RecentAdvances}.
...
This is the concept of \idx{physical reservoir computing} (PRC)~\cite{PRC}, which allows the usage of physical systems without further mathematical abstractions --- apart from the \xref{linear readout layer} at the end~\cite{RC_RecentAdvances}. \par
While this avoids the many layers of abstraction present in conventional \xref{machine learning} methods, physical constraints can make it difficult to optimise a physical reservoir~\cite{RC_RecentAdvances}, in contrast to RNNs which have full mathematical freedom.
...
Nonetheless, PRC combines many advantages of all the previously discussed concepts: training is computationally very cheap due to the single \xref{linear readout layer}, enabling low energy usage due to the direct usage of a physical substrate for computation.
...
Fading memory\indexlabel{fading memory}~\cite{boyd1985ApproximatingVolterra} originates from the echo state property of ESNs, where recurrent connections preserve information of the past while it slowly fades out as new input values are applied.
...
\subsubsection{Matrix rank-based metrics}\label{sec:1:RC_metrics_KQ}\indexlabel[tosection]{matrix rank-based metrics}
...
In the extreme case where $K=n$, any desired output can be implemented by the reservoir in combination with its \xref{linear readout layer}~\cite{WhatMakesPowerful}.
...
\subsubsection{Linear estimator-based metrics}\indexlabel[tosection]{linear estimator-based metrics}
...
The \xlabel{non-linearity} of the reservoir can be measured by considering that its response consists of a linear and a non-linear contribution.
...
The linear \xlabel{memory capacity} can also be calculated by means of linear estimators, but by working the other way around: an estimator for the past input $s(t - \tau)$ is now trained based on the current readout $\vc{x}(t)$ of the reservoir~\cite{tsunegi2019STOforcedsyncRC,NeuromorphicFewShot}.
...
This metric measures \indexlabel[nolabel]{fading memory}short-term memory; long-term memory often refers to the knowledge engrained in the weights of an ANN, i.e., the \xref{linear readout layer} in the case of RC.
...
Closely related to the \xref{memory capacity} is the \xlabel{parity check} metric~\cite{hon2021numerical,tsunegi2019STOforcedsyncRC}.
...
This introduces a form of \xref{non-linearity} into it, though it still mostly serves as a measure of the memory capacity in binary systems.
...
\indexlabel{memristor}Memristors are a well-established example: they can store information in their resistance by changing their internal ion distribution, allowing them to act analogously to biological neurons~\cite{MemristorArtificialNeuron,MemristiveNN,RC_DiffusiveMemristors,RC_MemristorTemporal}.
...
They can be employed in a variety of architectures, for instance as a \xlabel{Josephson transmission line}~\cite{RC_SuperconductingElectronics}. \par % See also: analogue electronic circuits [TaskAdaptivePRC, Ref. 10], ferroelectrics [TaskAdaptivePRC, Ref. 13], VLSI
Photonic principles have also been exploited to construct reservoirs with very fast operation~\cite{RC_RecentAdvances}.
...
However, mechanical systems provide an extreme example of physical reservoir computing, as it has been shown that the body of a \xlabel{tensegrity} robot (tensile structures arranged in trusses, consisting of both springs and incompressible bars) can be used for both locomotion and computation~\cite{RC_Tensegrity}.
...
Finally, a wide variety of \link{spintronics}{magnetic and spintronic} systems have been proposed for use as physical reservoirs.
...
\end{equation}
at a point $\vc{r}$ relative to said object, giving rise to the \xref{magnetostatic interaction} through which magnetic moments influence each other. % (\cref{eq:2:E_MS})
Freely rotatable magnetic moments will therefore prefer a parallel alignment along their connecting axis.

\paragraph{Ferromagnetism\indexlabel{ferromagnetism}}
...
This magnetisation $\vc{M}$ --- the \xref{magnetic moment} per unit volume --- arises at the atomic level from two distinct sources: the angular momentum of electrons around the nucleus, and the intrinsic spin of the elementary particles comprising an atom~\cite{coey2010magnetism}.
...
The keyword regarding the \link{Heisenberg-Dirac exchange interaction}{exchange interaction} is ``nearby'': over longer distances, the \xref{magnetisation} direction can vary throughout the material as other interactions are also at play.
Most importantly, the \xref{magnetostatic interaction} makes it energetically favourable for the magnetisation to curl back on itself. % Minimising stray fields
...
\paragraph{Single-domain nanomagnets\indexlabel{single-domain nanomagnet}}
...
This results in a \emph{single-domain nanomagnet} with a nearly uniform magnetisation, giving it a magnetic moment $\vc{\mu}$ close to the upper limit of $M_\mathrm{sat}V$~\cite{FRENKEL1930,neel1949theorie}. % Frenkel and Dorfman first conceived the idea of a single-domain particle based on energy considerations.
...
The magnetisation can switch between these two stable states under the influence of nearby nanomagnets, \xref{external magnetic fields}, thermal fluctuations, electrical currents and a variety of other phenomena~\cite{SwitchingForced_EnergyEfficient,BrownThermalFluctuations,neel1949theorie}. \par
One common source of anisotropy is the geometry of a nanomagnet: this \idx[nolabel]{shape anisotropy} arises from the interaction of the magnetisation with its own \xlabel[nolabel]{demagnetising field}, resulting in a low-energy state when the magnetisation aligns with the longest axis of the geometry~\cite{MagneticCharge}.
Anisotropy can also originate from the crystal structure, but this can be suppressed by an appropriate choice of material; for example, \xlabel{permalloy} (\ce{Fe20Ni80}) is an alloy specifically designed to exhibit minimal \xlabel{magnetocrystalline anisotropy}~\cite{PyAnisotropyRevisited,PyAnisotropyQuantum}.
Finally, \xlabel{interfacial anisotropy} can occur at the interface between a \link{ferromagnetism}{ferromagnet} and an adjacent material.
For example, between \ce{Co} and \ce{Pt}~\cite{PMA_PdCo_PtCo}, a \idx[nolabel]{perpendicular magnetic anisotropy} (PMA) exists that acts in the immediate vicinity of the interface, where the magnetisation then prefers to align perpendicular to said interface.

\paragraph{In-plane and out-of-plane nanomagnets}\phantomsection\label{sec:1:IP_OOP} % Q: is this OK? What to do with the chapter 3 intro? Just remove 3.1.1?
Two types of \link{single-domain nanomagnet}{nanomagnets} are typically distinguished: in-plane (IP) and out-of-plane (OOP) nanomagnets.
IP nanomagnets can readily be obtained by tailoring the geometry of the \link{ferromagnetism}{ferromagnetic} particle: a flat geometry encourages an in-plane \xref{magnetisation} direction through the \xref{shape anisotropy}.
...
\subsubsection{Input}
In practice, using magnetic fields to switch \link{single-domain nanomagnet}{nanomagnets} is not ideal.
...
\paragraph{Spin-transfer torque\indexlabel{spin-transfer torque}}
A well-established method for switching \link{single-domain nanomagnet}{nanomagnets} is \emph{spin-transfer torque} (STT)~\cite{SlonczewskiSTT}, as has been widely used in, e.g., magnetic random-access memory (STT-MRAM)~\cite{brataas2012current}.
When a spin current polarised along $\vc{\sigma}$ enters a \link{ferromagnetism}{ferromagnetic} material, the injected spins will exert a torque $\vc{\tau}_\mathrm{STT} \propto \vc{\mu} \times (\vc{\mu} \times \vc{\sigma})$ on the local \xref{magnetisation} $\vc{\mu}$, causing it to align with the injected \xlabel{spin polarisation} $\vc{\sigma}$.
...
\paragraph{Spin--orbit torque\indexlabel{spin--orbit torque}}
...
When an in-plane electrical current passes through the underlayer, a \xref{spin polarisation} will accumulate at the interface with the ferromagnet as a result of the bulk \xlabel{spin Hall effect}~\cite{SHE} and/or interfacial \xlabel{Rashba-Edelstein effect}~\cite{SOT_Rashba}. % RE effect due to symmetry being broken at an interface. The bulk spin Hall effect often provides the dominant damping-like torque, while any interfacial Rashba effect contributes mainly to the field-like term. % SOT does not originate from a single underlying phenomenon, but is an umbrella term for various effects that all relate to spin--orbit coupling and end up generating a torque in a magnetic material.
These spins, polarised in-plane and perpendicular to the electrical current, can then diffuse into the ferromagnet and exert a torque on its \xref{magnetisation}~\cite{mumax3tutorial}.
...
The field-like torque $\vc{\tau}_\mathrm{FL} \propto \vc{\mu} \times \vc{\sigma}$ causes a precession of the \xref{magnetic moment} $\vc{\mu}$ around the spin polarisation $\vc{\sigma}$.
...
This can be achieved in a plethora of ways, e.g., by an in-plane magnetic field, an \xlabel{exchange bias} layer, a source of OOP spin polarisation, exploitation of crystal symmetries if possible, or geometrical asymmetry in the system~\cite{SOT_firstprinciplesCoPt,SOT_Roadmap}.
...
\paragraph{Anomalous Hall effect\indexlabel{anomalous Hall effect}}
Our method of choice to obtain electrical output from ensembles of out-of-plane \link{single-domain nanomagnet}{nanomagnets} is to make use of the \emph{anomalous Hall effect} (AHE).
In the ordinary Hall effect, an electrical current flowing through a conductor (the \xlabel{Hall bar}) generates a transverse voltage, since the current generates a magnetic field which exerts a Lorentz force on the electrons.
When the conductor is a \link{ferromagnetism}{ferromagnet}, an additional contribution appears that is proportional to the \xref{magnetisation} of the material.
...
\paragraph{Anisotropic magnetoresistance\indexlabel{anisotropic magnetoresistance}}
Another phenomenon that can provide readout is \emph{anisotropic magnetoresistance} (AMR): when an electrical current flows through a \link{ferromagnetism}{ferromagnetic} material, the resistance weakly depends on the angle $\theta$ between this current and the \xref{magnetisation}~\cite{AMR}.
...
It also requires the system to be electrically connected: this makes it an appropriate choice for the ring system in the \spinengine project~\cite{Vidamour2023,ArchitecturesNanoringRC,DynamicEmergence_NanomagneticSystem}, for example, but prevents it from directly being applied to ensembles of \link{single-domain nanomagnet}{nanomagnets}.
...
\paragraph{Ferromagnetic resonance\indexlabel{ferromagnetic resonance}}
...
When the frequency matches a resonance of the internal \xref{magnetisation} dynamics of a \link{single-domain nanomagnet}{nanomagnet}, an absorption peak appears.
The complexity of internal magnetisation dynamics, represented in the frequency domain, can provide a rich and non-linear \link{reservoir computing}{reservoir} output~\cite{NeuromorphicFewShot,Gomez-Iriarte_FMR}.
...
\paragraph{Magnetic force microscopy\indexlabel{magnetic force microscopy}}
...
Whereas the latter uses a non-magnetic tip to measure the sample's texture at the atomic scale, MFM uses a tip coated in \link{ferromagnetism}{ferromagnetic} material (e.g., \ce{Co}), giving it a vertical \xref{magnetic moment} to measure the local out-of-plane\footnote{
...
For example, an IP \link{single-domain nanomagnet}{nanomagnet} will appear as a pair of regions with bright and dark contrast, because the divergence (convergence) of the magnetic field at its north (south) pole results in opposite vertical magnetic fields on either end~\cite{NML_Carlton}.
An MFM image of OOP magnets (\cref{sec:3:MFM}) is easier to interpret, as there is a one-to-one correspondence between $\partial B_z/ \partial z$ and the magnetisation state. \par
...
X-ray based measurements like \xlabel{photo-emission electron microscopy} (PEEM)~\cite{PEEM} and \xlabel{scanning transmission X-ray microscopy} (STXM)~\cite{Imaging_MTXM} make use of the \xlabel{X-ray magnetic circular dichroism} (XMCD) effect.
...
Another option is to use optical techniques like the \xlabel{magneto-optical Kerr effect} (MOKE)~\cite{KerrFaraday_book}: while these do not require a synchrotron, their resolution is diffraction-limited to several hundred \SI{}{\nano\metre}. \par
Recently, scanning \indexlabel{nitrogen-vacancy microscopy}nitrogen-vacancy (NV) microscopy has emerged as a high-sensitivity scanning-probe technique.
Like \link{magnetic force microscopy}{MFM}, it uses a cantilever, but with a diamond tip in which one or more NV colour centres are embedded~\cite{NVprospects}.
...
\subsubsection{Overview of spintronic reservoirs}\indexlabel[tosection]{spintronics}
\indexlabel{spin-torque oscillator}Spin-torque nano-oscillators (STOs) are a common example of devices considered as spintronic reservoirs, owing to their small size and high frequency operation~\cite{tsunegi2019STOforcedsyncRC}.
...
This way, by applying time-multiplexed input, a single STO has successfully been used~\cite{tsunegi2019STOforcedsyncRC} as a \xref{single dynamical node} to, for instance, perform speech recognition~\cite{STO_RC_Riou2021,NeuromorphicOscillators}.
Arrays of STOs are also more broadly used in \xref{neuromorphic computing} to implement traditional neural networks more efficiently in hardware~\cite{VowelRecognition4STO}. \par
The \xref{magnetisation} within \link{ferromagnetism}{ferromagnetic} thin films can also exhibit a diverse range of magnetic textures, like domain walls, spin waves and skyrmions.
...
This thesis concerns itself with ensembles of magnetostatically coupled \link{single-domain nanomagnet}{nanomagnets}.
Due to competing \xref{magnetostatic interactions}, these ensembles often exhibit collective dynamics like phase transitions and long-range order, which can be exploited for \xref{reservoir computing}.
...
A class of ensemble of particular interest is \xref{artificial spin ice}~\cite{RC_ASI}.

\subsubsection{Artificial spin ice}\label{sec:1:ASI}\indexlabel[tosection]{artificial spin ice}
\textit{Artificial spin ice} (ASI) is a type of metamaterial where magnetostatically coupled \link{single-domain nanomagnet}{single-domain (bistable) nanomagnets} are arranged on an ordered lattice~\cite{RC_ASI,flatspin}.
The \xref{magnetisation} direction of the constituent nanomagnets can switch between two states under the influence of nearby nanomagnets, \link{external magnetic field}{externally applied magnetic fields}, thermal fluctuations, or any of the electronic input methods \link{sec:1:ASI_IO}{previously discussed}~\cite{CoerciveFieldReversal,BrownThermalFluctuations,SOT_FM_AFM,brataas2012current}. \par
...
The term \idx{spin ice} draws inspiration from water ice, where the oxygen atoms are positioned on a diamond lattice, so each has four nearest neighbours with which it shares a hydrogen bond. % TODO FIG: figure of water ice, then spin ice, then ASI?
However, each oxygen atom is closely bound to two hydrogen atoms, so the hydrogen atoms in the two other bonds must be more distant, leading to geometrical frustration with a ``two-in, two-out'' \xlabel{ice rule}~\cite{nisoli2013colloquium,ZeroPointEntropy,heyderman2013artificial,MagnetizationDynamicsASI}.
Similarly, natural spin ices have been found in certain crystal structures like pyrochlores, whose atomic \xref{magnetic moments} must obey similar ice rules that also lead to frustrated spin configurations~\cite{nisoli2013colloquium}.
From there, it is only a small leap to ASI, where a 2D lattice of \link{single-domain nanomagnet}{nanomagnets} mimics the frustration present in natural spin ices~\cite{heyderman2013artificial}.
...
Modern nano-fabrication methods such as electron beam \xlabel{lithography} allow any arbitrary geometry to be realised and many other aspects of the system to be engineered, offering enormous freedom to the designer~\cite{AdvancesASI,ASI_computation}.
...
The aforementioned \link{sec:1:ASI_IO}{electrical input methods} also enable precise control over these systems.
Hence, the controlled, tunable and easily measurable environment offered by ASI has enabled the study of complex phenomena such as phase transitions with ordered domains~\cite{ApparentFMpinwheel,sklenar2019field,MeltingASI,ImagingBridgedKagome,sendetskyi2019continuous,lou2023competing,branford2012emerging} or glass-like behaviour~\cite{wang2006artificial,ZeroPointEntropy}, vertex-based frustration~\cite{morrison2013unhappy,nisoli2018topologytetris} leading to emergent topological structures such as monopoles and \xref{Dirac strings}~\cite{ObservationMonopoleASI,mengotti2011kagome}, chiral dynamics~\cite{branford2012emerging,EmergentChiralityRatchet}...
ASI also holds significant potential for computational applications~\cite{heyderman2022spin}, both in conventional logic~\cite{ComputationalLogic_2018,Gypens_Balanced,EngineeringRelaxationComputation} and \xref{neuromorphic computing}~\cite{ASI_computation,RC_RecentAdvances}. \par
...
The \xref{reservoir computing} potential of IP ASI has been demonstrated numerically~\cite{RC_ASI}, but the experimental application of input and readout is not straightforward.
For OOP ASI, efficient input (\link{spin--orbit torque}{SOT}) and readout (\link{anomalous Hall effect}{AMR}) mechanisms are available, but its RC potential has not yet been demonstrated.
...
To determine whether an \link{artificial spin ice}{ASI} possesses desirable properties for it to be used as a \link{reservoir computing}{reservoir}, the impact of various system parameters and input methods on reservoir performance must be determined.
...
The internal \xref{magnetisation} dynamics of \link{ferromagnetism}{ferromagnetic} structures are the result of the interactions between millions of atomic \xref{magnetic moments}.
...
One particularly useful approximation for the simulation of \link{single-domain nanomagnets}{nanomagnets} is \idx[tosection]{micromagnetic theory}.
...
The main complication in micromagnetic simulations is the infinite range of the \xref{magnetostatic interaction}, which must therefore be calculated between all cells in the simulation.
...
Furthermore, to determine statistically valid \link{sec:1:RC_metrics}{RC metrics}, many input cycles must be applied --- on the order of 100 for the \xref{linear estimator-based metrics} --- during which many switches occur~\cite{RC_TaskAgnosticMetrics_v2}.
To address these limitations, specialised ASI simulation tools have been developed.

\subsection{Macrospin approximation}\label{sec:1:Modelling_macrospin}\indexlabel[nolabel]{point dipole model}
Since the magnetisation of \xref{single-domain nanomagnets} is often nearly uniform throughout, a \idx[tosection]{macrospin approximation} can be used where the magnet is represented as a \link{point dipole model}{point dipole}.
When a nanomagnet has a single \xref{easy axis} --- as is typically the case in ASI --- it can be approximated as a single Ising spin.
...
However, this approach comes at the cost that the internal \xref{magnetisation} structure of individual nanomagnets is no longer simulated in detail. \par
...
More recently, the importance of long-range \xref{magnetostatic interactions} has been emphasized to improve correspondence to experiments~\cite{Chioar2014,Rougemaille2011,Brunn2021}.
...
Beyond-NN interaction models, including full magnetostatically coupled models~\cite{ApparentFMpinwheel,mengotti2011kagome}, are also used to simulate square-~\cite{Brunn2021,Farhan2013,sklenar2019field} and triangular-based~\cite{Chioar2014,Rougemaille2011,Hofhuis2020} lattices, for example. \par % 'sklenar2019field': 3NN interactions calculated by mumax on quadrupole lattice. 'mengotti2011kagome' use a full dipole model with Ewald summation for PBC.
Here, we opt to develop our own simulation toolkit: \hotspice~\cite{MAES-24}.
This was driven by a desire to blend the approach of \texttt{flatspin}~\cite{flatspin} --- a general-purpose GPU-accelerated IP ASI simulator --- with the rather purpose-built Monte Carlo toolkits listed above, to obtain efficient simulation of \link{reservoir computing}{RC} in OOP ASI.
...
System dynamics are determined by the switching probability of each magnet, which is calculated based on the \link{effective energy barrier}{energy barrier} separating its two stable states.
...
The details of the \hotspice simulator will be extensively discussed in~\cref{ch:Hotspice}, after which it is used in~\cref{ch:Applications} to explore \xref{reservoir computing} in OOP ASI.
