...
\subsection{Reservoir computing\indexlabel{reservoir computing}}
...
Superconducting materials, on the other hand, offer low dissipation and extremely fast operation ($\gg \SI{}{\giga\hertz}$), with a number of possible architectures, for instance a \xlabel{Josephson transmission line}~\cite{RC_SuperconductingElectronics}. \par % See also: analogue electronic circuits [TaskAdaptivePRC, Ref. 10], ferroelectrics [TaskAdaptivePRC, Ref. 13], VLSI
...
Finally, \xlabel{interfacial anisotropy} can occur between the \link{ferromagnetism}{ferromagnetic} material and a substrate.
...
IP nanomagnets can readily be obtained by tailoring the geometry of the \link{ferromagnetism}{ferromagnetic} particle: a flat geometry guarantees an in-plane \xref{magnetisation} direction through the \xref{shape anisotropy}.
...
Whereas the latter uses a non-magnetic tip to measure the sample's texture at an atomic scale, MFM uses a tip coated in \link{ferromagnetism}{ferromagnetic} material (e.g., \ce{Co}), giving it a vertical \xref{magnetic moment}.
...
X-ray based measurements like \xlabel{photo-emission electron microscopy} (PEEM)~\cite{PEEM} and \xlabel{scanning transmission X-ray microscopy} (STXM)~\cite{Imaging_MTXM} can make use of the \xlabel{X-ray magnetic circular dichroism} (XMCD) effect to achieve even higher resolution than MFM.
