...
Some approaches to train RNNs take inspiration from FNN: \xlabel{back-propagation through time} (BPTT), for instance, unfolds the recurrent connections in time to obtain a FNN on which backpropagation can be applied~\cite{jaeger2002tutorial}.
...
Jaeger~\cite{jaeger2001echo} presented \xlabel{echo state networks} (ESNs), while Maass~\etal~\cite{maass_LSM} presented \xlabel{liquid state machines} (LSMs)\footnote{
...
X-ray based measurements like \xlabel{photo-electron emission microscopy} (PEEM)~\cite{PEEM} and \xlabel{scanning transmission X-ray microscopy} (STXM)~\cite{Imaging_MTXM} can make use of the \xlabel{X-ray magnetic circular dichroism} (XMCD) effect to achieve even higher resolution than MFM.
