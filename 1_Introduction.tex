...
At the intersection of \xref{neuromorphic computing} and \xref{material computation} lies the paradigm of \idx{reservoir computing}.
...
\subsection{Reservoir computing}\indexlabel[nolabel]{reservoir computing}
...
For a physical substrate to be suitable as a reservoir, several requirements need to be satisfied: \xlabel{high dimensionality}, non-linearity, \xlabel{fading memory}, and a separation/approximation property~\cite{appeltant2011information}.
...
\subsubsection{Linear estimator-based metrics}\indexlabel{linear estimator-based metrics}
...
A well-established method for switching \link{single-domain nanomagnet}{nanomagnets} is \idx{spin-transfer torque} (STT)~\cite{SlonczewskiSTT}, as has been widely used in, e.g., magnetic random-access memory (STT-MRAM)~\cite{brataas2012current}.
...
Our method of choice to obtain electrical output from ensembles of out-of-plane \link{single-domain nanomagnet}{nanomagnets} is to make use of the \idx{anomalous Hall effect} (AHE).
...
Another phenomenon that can provide readout is \idx{anisotropic magnetoresistance} (AMR): when an electrical current flows through a \link{ferromagnetism}{ferromagnetic} material, the resistance weakly depends on the angle $\theta$ between this current and the \xref{magnetisation}~\cite{AMR}.
...
Let us now return to our discussion of physical reservoir platforms from~\cref{sec:1:PhysicalRCplatforms}, where we previously left off at magnetic and \indexlabel{spintronics}spintronic reservoirs.
...
\textit{Artificial spin ice}\indexlabel{artificial spin ice} (ASI) is a type of metamaterial where magnetostatically coupled \link{single-domain nanomagnet}{single-domain (bistable) nanomagnets} are arranged on an ordered lattice~\cite{RC_ASI,flatspin}.
...
One particularly useful approximation for the simulation of \link{single-domain nanomagnets}{nanomagnets} is \idx{micromagnetic theory}.
...
Since the magnetisation of \xref{single-domain nanomagnets} is often nearly uniform throughout, a \idx{macrospin approximation} can be used where the magnet is represented as a \link{point dipole model}{point dipole}.
