...
Since the reservoir acts as a black box, its properties can not be changed to obtain the desired response\footnote{
	It is possible to adjust a physical reservoir to exhibit the desired properties for a given task~\cite{AdaptiveProgrammableRC,gartside2022reconfigurable}, but this is different from mathematically tuning the weights of the \link{linear readout layer}{readout layer} via a simple procedure like linear regression.
}.
...
The complexity of internal magnetisation dynamics, represented in the frequency domain, can provide a rich and non-linear \link{reservoir computing}{reservoir} output~\cite{AdaptiveProgrammableRC,Gomez-Iriarte_FMR}.
